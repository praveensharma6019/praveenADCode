...
# # Copy transforms, retaining directory structure
 RUN Invoke-Expression 'robocopy .\src C:\out\transforms /s /ndl /njh /njs *.xdt'

# Build the Sitecore main platform artifacts
 RUN msbuild .\src\Environment\platform\Adani.SuperApp.Realty.Environment.Platform.csproj /p:Configuration=$env:BUILD_CONFIGURATION /m /p:DeployOnBuild=True /p:DeployDefaultTarget=WebPublish /p:WebPublishMethod=FileSystem /p:PublishUrl=C:\out\sitecore
...
# COPY src/Foundation/Serialization/Serialization/Adani/ C:/unicorn/Adani/
