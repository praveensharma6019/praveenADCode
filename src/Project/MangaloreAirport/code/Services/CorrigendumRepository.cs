...
namespace Sitecore.MangaloreAirport.Website.Services
{
    public class CorrigendumRepository
    {
        ...
        public void ActivateCorrigendum(Guid id)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                Corrigendum corrigendumItem = (
                    from c in dbcontext.Corrigendums
                    where c.Id == id
                    select c).Single<Corrigendum>();
        ...
        public void DeleteCorrigendumDocument(Guid id, string DocumentPath)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                CorrigendumDocument doc = (
                    from x in dbcontext.CorrigendumDocuments
                    where x.Id == id
                    select x).Single<CorrigendumDocument>();
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                if (UserType != "Admin")
                {
                    IQueryable<CorrigendumModel> list =
                        from r in dbcontext.Corrigendums
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                IQueryable<CorrigedumEditModel> list =
                    ...
                    select new CorrigedumEditModel()
                    {
                        ...
                        CorrigendumDocument = r.CorrigendumDocuments.Where<CorrigendumDocument>((CorrigendumDocument x) => x.CorrigendumId == (Guid?)r.Id).ToList<CorrigendumDocument>(),
                        CorrigendumTenderMapping = r.CorrigendumTenderMappings.Where<CorrigendumTenderMapping>((CorrigendumTenderMapping x) => x.CorrigendumId == (Guid?)r.Id).ToList<CorrigendumTenderMapping>()
                    };
        ...
        public void InsertCorrigendumDocument(CorrigendumModel obj)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                ...
                dbcontext.CorrigendumDocuments.InsertOnSubmit(corrigendumDocument);
                dbcontext.SubmitChanges();
            }
        }
        ...
        public void InsertCorrigendumTenderMapping(Guid tenderId, Guid corrigId)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                CorrigendumTenderMapping corrigendumTenderMapping = new CorrigendumTenderMapping()
                {
                    Id = Guid.NewGuid(),
                ...
                dbcontext.CorrigendumTenderMappings.InsertOnSubmit(corrigendumTenderMapping);
                dbcontext.SubmitChanges();
            }
        }
        ...
        public void UpdateCorrigendumMapping(CorrigedumEditModel obj, List<Check> selectedRecords)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                (
                    ...
                    select x).ToList<CorrigendumTenderMapping>();
                IQueryable<CorrigendumTenderMapping> del =
                    from x in dbcontext.CorrigendumTenderMappings
                    where x.CorrigendumId == (Guid?)obj.Id
                    select x;
                dbcontext.CorrigendumTenderMappings.DeleteAllOnSubmit<CorrigendumTenderMapping>(del);
                dbcontext.SubmitChanges();
