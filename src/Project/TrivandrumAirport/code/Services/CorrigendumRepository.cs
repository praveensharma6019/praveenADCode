...
namespace Sitecore.TrivandrumAirport.Website.Services
{
    public class CorrigendumRepository
    {
        ...
        public void ActivateCorrigendum(Guid id)
        {
            ...
            {
                TRV_Corrigendum corrigendumItem = (
                    from c in dbcontext.TRV_Corrigendums
                    where c.Id == id
                    select c).Single<TRV_Corrigendum>();
        ...
        public void DeleteCorrigendumDocument(Guid id, string DocumentPath)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                TRV_CorrigendumDocument doc = (
                    from x in dbcontext.TRV_CorrigendumDocuments
                    where x.Id == id
                    select x).Single<TRV_CorrigendumDocument>();
                dbcontext.TRV_CorrigendumDocuments.DeleteOnSubmit(doc);
                try
                {
                    dbcontext.SubmitChanges();
                }
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                if (UserType != "Admin")
                {
                    IQueryable<CorrigendumModel> list =
                        from r in dbcontext.TRV_Corrigendums
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                IQueryable<CorrigedumEditModel> list =
                    ...
                    select new CorrigedumEditModel()
                    {
                        ...
                        CorrigendumDocument = r.TRV_CorrigendumDocuments.Where<TRV_CorrigendumDocument>((TRV_CorrigendumDocument x) => x.CorrigendumId == (Guid?)r.Id).ToList<TRV_CorrigendumDocument>(),
                        CorrigendumTenderMapping = r.TRV_CorrigendumTenderMappings.Where<TRV_CorrigendumTenderMapping>((TRV_CorrigendumTenderMapping x) => x.CorrigendumId == (Guid?)r.Id).ToList<TRV_CorrigendumTenderMapping>()
                    };
        ...
        public Guid InsertCorrigendum(CorrigendumModel obj)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                ...
                TRV_Corrigendum list = new TRV_Corrigendum()
                {
                    Id = Guid.NewGuid(),
                    Title = obj.Title,
                ...
                dbcontext.TRV_Corrigendums.InsertOnSubmit(list);
                dbcontext.SubmitChanges();
                id = list.Id;
            }
            return id;
        ...
        public void InsertCorrigendumDocument(CorrigendumModel obj)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                TRV_CorrigendumDocument TRVCorrigendumDocument = new TRV_CorrigendumDocument()
                {
                    Id = Guid.NewGuid(),
                ...
                dbcontext.TRV_CorrigendumDocuments.InsertOnSubmit(TRVCorrigendumDocument);
                dbcontext.SubmitChanges();
            }
        }
        ...
        public void InsertCorrigendumTenderMapping(Guid tenderId, Guid corrigId)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                TRV_CorrigendumTenderMapping TRVCorrigendumTenderMapping = new TRV_CorrigendumTenderMapping()
                {
                    Id = Guid.NewGuid(),
                ...
                dbcontext.TRV_CorrigendumTenderMappings.InsertOnSubmit(TRVCorrigendumTenderMapping);
                dbcontext.SubmitChanges();
            }
        }
        ...
        public void UpdateCorrigendumMapping(CorrigedumEditModel obj, List<Check> selectedRecords)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                (
                    ...
                    select x).ToList<TRV_CorrigendumTenderMapping>();
                IQueryable<TRV_CorrigendumTenderMapping> del =
                    from x in dbcontext.TRV_CorrigendumTenderMappings
                    where x.CorrigendumId == (Guid?)obj.Id
                    select x;
                dbcontext.TRV_CorrigendumTenderMappings.DeleteAllOnSubmit<TRV_CorrigendumTenderMapping>(del);
                dbcontext.SubmitChanges();
