  ...
  "July", "August", "September", "October", "November", "December"};
            ...
            try
            {
                ...
                if (model.IsMobile)
                {
                    try
                    {
                        ...
                        var apiurl = string.Format("https://vknr1.api.infobip.com/sms/1/text/query?to=91{0}&username=Carbon-OTP&from=ATRNSL&password=India@123!456&text=Dear%20User,%0AThe%20OTP%20for%20ATL%20Carbon%20Calculator%20is%20{1}.%0AThanks,%0AAdani%20Transmission%20Team%20-ADANI%20TRANSMISSION%20LIMITED%27", model.OTPFor, generatedotp);
        ...
        public ActionResult CarbonCalculatorRegistration(CarbonCalculatorRegistrationModel pm)
        {
            ...
            try
            {
                ...
                {
                    int regUserMobileOTP = rdb.Transmission_Carbon_CalculatorOTPHistories.Where(x => x.OTPFor == pm.MobileNumber && x.OTP == pm.OTP).OrderByDescending(x => x.CreatedDate).Count();
                    int regUserEmailOTP = rdb.Transmission_Carbon_CalculatorOTPHistories.Where(x => x.OTPFor == pm.EmailId && x.OTP == pm.EmailOTP).OrderByDescending(x => x.CreatedDate).Count();
                    if (regUserMobileOTP >= 1 && regUserEmailOTP >= 1)
                    {
                        pm.Id = Guid.NewGuid();
                        pgr.Id = pm.Id;
