...
{
    public class AdaniCapitalBranchListModel
    {
    ...
    public class AdaniCapitalGetBranchDetails
    {
        ...
        public AdaniCapitalBranchListModel GetBranchLocation(string State, string City)
        {
            var Location = dbContext.AdaniCapitalBranchLists.Where(t => t.State == State && t.City == City).Select(i => new AdaniCapitalBranchListModel { Latitude = i.Latitude, Longitude = i.Longitude, Address = i.Address }).FirstOrDefault();
            if (Location != null)
            {
                return Location;
            }
