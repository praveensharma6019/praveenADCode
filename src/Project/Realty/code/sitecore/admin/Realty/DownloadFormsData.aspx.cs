...
{
    public partial class DownloadFormsData : System.Web.UI.Page
    {
            ...
            try
            {
                ...
                else 
                {
                    var datarecord = from rc in rdb.ConsolidateFormDatas
                                     where ((rc.FormSubmitOn.Value.Date >= fromdate) && (rc.FormSubmitOn.Value.Date <= todate) && rc.PageInfo.ToLower().Trim() ==drptype.SelectedValue.ToLower().Trim())
                                     select rc;

                    if (System.Convert.ToDateTime(TextBoxTo.Text) == System.Convert.ToDateTime(TextBoxFrom.Text))
                    {
                        datarecord = from rc in rdb.ConsolidateFormDatas
                                     where rc.FormSubmitOn.Value.Date == fromdate && rc.PageInfo.ToLower().Trim() == drptype.SelectedValue.ToLower().Trim()
                                     select rc;
                    }
