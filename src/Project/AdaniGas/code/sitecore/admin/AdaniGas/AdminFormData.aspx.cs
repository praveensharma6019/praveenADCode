...
{
    public partial class AdminFormData : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    var datarecord = from rc in rdb.PaymentHistoryDatas
                                     where ((rc.Created_Date.Value.Date >= fromdate) && (rc.Created_Date.Value.Date <= todate))
                                     select rc;

                    if (System.Convert.ToDateTime(TextBoxTo.Text) == System.Convert.ToDateTime(TextBoxFrom.Text))
                    {
                        datarecord = from rc in rdb.PaymentHistoryDatas
                                     where rc.Created_Date.Value.Date == fromdate
                                     select rc;
                    }
