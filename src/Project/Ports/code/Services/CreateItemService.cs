...
namespace Sitecore.Ports.Website.Services
{
    public class CreateItemService
    {
        public bool CreateItem(TemplatedModel templateModel)
        {
            try
            {
                Log.Info("In CreateItem method templateModel received:" + templateModel, templateModel);
                    ...
                    var postData = new List<KeyValuePair<string, string>>
                {
                    ...
                    Log.Info("response received from createItem Api Response:" + response, response);
                    Log.Info("response received from createItem Api ResponseContent:" + response.Content, response);
                    Log.Info("response received from createItem Api ResponseRequestMessage:" + response.RequestMessage, response);
                    Log.Info("response received from createItem Api ResponseReasonPhrase:" + response.ReasonPhrase, response);
                    Log.Info("response received from createItem Api ResponseStatusCode:" + response.StatusCode, response);
                    if (response.IsSuccessStatusCode)
                    {
                        Log.Info("response received from createItem Api ResponseIsSuccessStatusCode:" + response.IsSuccessStatusCode, response);
                        return true;
                    }
                    return false;
                }
