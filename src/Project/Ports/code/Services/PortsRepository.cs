    ...
    public class PortsRepository
    {
        ...
        public string PortsGMSStoreGeneratedOtp(PortsGMSOTP model)
        {
            try
            {
                ...
                Log.Info($"connection cretaed Ports: {pgd.Connection}", pgd.Connection);
                pgd.PortsGMSOTPHistories.InsertOnSubmit(entity);
                Log.Info($"otp inserted Ports: {entity}", entity);
                pgd.SubmitChanges();
                return sRandomOTP;
            }
            catch (Exception ex)
            {
                Log.Info($"execption thrown in  PortsGMSStoreGeneratedOtp: {ex.InnerException}",ex);
                return null;
            }
