$("#formRealtyBookNow").on('submit', function () {
    ...
    var country = $("#BookCountry").val();
    if (country == "") {
        message = message + "Please select your country </br>";
        error = true;
    }
	var country = $("#BookState").val();
    if (country == "") {
        message = message + "Please select/enter state </br>";
        error = true;
    }
...
function validatePAN(number) {
 ...
 $("#BookCountry").change(function () {
        loadStateDrpBook('#BookState');
    });



function loadStateDrpBook(statedrp) {
            var parTag = '';
    if ($("#BookCountry").val() == 'Others') {
        parTag = $("#BookState").parent();
        $(parTag).text('');
		$(parTag).append('<label for="">State / Province</label>');		
        $(parTag).append('<input class="form-control" id="BookState" name="State" required="required" type="text" value="">');

    }
    else {
        parTag = $("#BookState").parent();
		$(parTag).text('');
		$(parTag).append('<label class="control-label" for="">State / Province</label>');
        $(parTag).append('<select class="form-control" id="BookState" name="State"><option value="">Select State</option></select>');
        statedrp = $(statedrp);
        statedrp.html('');
        statedrp.append($('<option value="">Select State</option>'));
        if ($("#BookCountry").val() == 'AU') {
		...
		if ($("#BookCountry").val() == 'AE') {
            statedrp.append($("<option />").val('AE-AZ').text('Abu Dhabi'));
            statedrp.append($("<option />").val('AE-DU').text('Dubai'));
            statedrp.append($("<option />").val('AE-SH').text('Sharjah'));
            statedrp.append($("<option />").val('AE-UQ').text('Umm al-Qaiwain'));
            statedrp.append($("<option />").val('AE-FU').text('Fujairah'));
            statedrp.append($("<option />").val('AE-AJ').text('Ajman'));
            statedrp.append($("<option />").val('AE-RK').text('Ra’s al-Khaimah'));            
        }
		if ($("#BookCountry").val() == 'OM') {
            statedrp.append($("<option />").val('OM-DA').text('Ad Dakhliyah'));
            statedrp.append($("<option />").val('OM-ZA').text('Ad Dhahirah'));
            statedrp.append($("<option />").val('OM-BS').text('Al Batinah North'));
            statedrp.append($("<option />").val('OM-BJ').text('Al Batinah South'));
            statedrp.append($("<option />").val('OM-BU').text('Al Buraimi'));
            statedrp.append($("<option />").val('OM-WU').text('Al Wusta'));
            statedrp.append($("<option />").val('OM-SS').text('Ash Sharqiyah North'));
            statedrp.append($("<option />").val('OM-SJ').text('Ash Sharqiyah South'));
            statedrp.append($("<option />").val('OM-ZU').text('Dhofar'));
            statedrp.append($("<option />").val('OM-MA').text('Muscat'));
            statedrp.append($("<option />").val('OM-MU').text('Musandam'));
        }
		if ($("#BookCountry").val() == 'QA') {
            statedrp.append($("<option />").val('QA-DA').text('Doha'));
            statedrp.append($("<option />").val('QA-KH').text('Al Khor and Al Thakhira'));
            statedrp.append($("<option />").val('QA-WA').text('Al Wakrah'));
            statedrp.append($("<option />").val('QA-RA').text('Al Rayyan'));
            statedrp.append($("<option />").val('QA-MS').text('Al Shamal'));
            statedrp.append($("<option />").val('QA-SH').text('Al-Shahaniya'));
            statedrp.append($("<option />").val('QA-ZA').text('Al Daayen'));
            statedrp.append($("<option />").val('QA-US').text('Umm Salal'));            
        }
		if ($("#BookCountry").val() == 'KW') {
            statedrp.append($("<option />").val('KW-AH').text('Ahmadi'));
            statedrp.append($("<option />").val('KW-JA').text('Al-Jahra'));
            statedrp.append($("<option />").val('KW-KU').text('Al Asimah'));
            statedrp.append($("<option />").val('KW-FA').text('Farwaniya'));
            statedrp.append($("<option />").val('KW-HA').text('Hawalli'));
            statedrp.append($("<option />").val('KW-MU').text('Mubarak Al-Kabeer'));            
        }
    }
    }