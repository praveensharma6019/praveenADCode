...
function Set_Cookie(name, value, expires, path, domain, secure) {
    ...
    document.cookie = name + "=" + escape(value) + ((expires) ? ";expires=" + expires_date.toGMTString() : "") + ((path) ? ";path=" + path : "") + ((domain) ? ";domain=" + domain : "") + ((secure) ? ";secure" : "") + ";HttpOnly";
}
...
function Delete_Cookie(name, path, domain) {
    if (Get_Cookie(name))
        document.cookie = name + "=" + ((path) ? ";path=" + path : "") + ((domain) ? ";domain=" + domain : "") + ";expires=Thu, 01-Jan-1970 00:00:01 GMT" + ";HttpOnly";
}
...
$(document).ready(function() {
    ...
    if(isSet(Channel)) {
     ...
     var campaigncode = $.getQueryString('campaigncode');
    if(isSet(campaigncode)) {
        Delete_Cookie('campaigncode');
        Set_Cookie('campaigncode', campaigncode);
    }
