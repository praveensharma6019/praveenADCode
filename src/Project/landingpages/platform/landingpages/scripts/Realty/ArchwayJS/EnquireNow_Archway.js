...
function getCaptchaResponseForm(e) {
    grecaptcha.ready(function () {
        grecaptcha.execute('6Le0pu0gAAAAAD9pAHmh_CiRwBo6VZCNEvhMMnes', { action: 'ArchwayEnquiryNow' }).then(function (token) {
    ...
    grecaptcha.ready(function () {
        grecaptcha.execute('6Le0pu0gAAAAAD9pAHmh_CiRwBo6VZCNEvhMMnes', { action: 'ArchwayEnquiryNow' }).then(function (token) {
            ...
            $.ajax({
                type: "POST",
                data: JSON.stringify(savecustomdata),
                url: "/api/Realty/ArchwayEnquiryNow",
                contentType: "application/json",
                success: function (data) {
                    if (data.status === "101") {

                        $(this).prop('disabled', true);
                        window.location.href = "/archway/thankyou";
                        document.getElementById("price_form").reset();
                ...
                success: function (data) {
                    if (data.status === "101") {
                        ...
                        if (userAgent.match(/iPad/i) || userAgent.match(/iPhone/i) || userAgent.match(/iPod/i)) {
                            ...
                            link.href = "images/Realty/ArchwayImages/docs/Archway-Brochure.pdf";
                            link.download = "Archway Brochure";
                            link.click();
                            link.remove();
