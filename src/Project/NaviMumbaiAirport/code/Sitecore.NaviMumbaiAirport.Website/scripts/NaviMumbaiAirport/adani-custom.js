...
$('.stop').on('click',function(){
	...
	.owlCarousel({
		...
		$('.airport-section__carousel-3items')
	.on('changed.owl.carousel initialized.owl.carousel', function(event) {
    $(event.target)
      .find('.owl-item').removeClass('last')
      .eq(event.item.index + event.page.size - 1).addClass('last');
...
var concat = '&';
var query = {'location': 'ahmedabad,in', 'format': 'json','u':'c'};
var oauth = {
   ...
   // if (TravelDate.setHours(0,0,0,0) < CurrentDate.setHours(0,0,0,0)) { alert("Travel Date should not be past date."); $("#TravelYear").focus(); $('#BtnAPASubmit').removeAttr("disabled"); return false; }
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.status == "1") {
                window.location.href = "https://www.adani.com/svpia-ahmedabad-airport/assistance-thankyou";
        ...
        grecaptcha.ready(function() {
         grecaptcha.execute('6LdoDj4hAAAAAD7gqiPJEsAy96GJBkiNRu9qohiz', {action: 'AirportContactUs'}).then(function(token) {
			...
			$.ajax({
					...
					url: "/api/NaviMumbaiAirport/ContactUs",
					contentType: "application/json",
					success: function (data) {
						if (data.status == "1") {
							window.location.href = "/navi-mumbai-airport/thank-you";
						}
