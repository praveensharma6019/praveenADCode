...
namespace Sitecore.BelvedereClubGurgaon.Website.sitecore.admin.BelvedereClubGurgaon
{
    public partial class ClubGurgaonEnquiryRecords : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var datarecord = from rc in rdb.ClubGurgaonEnquiries
                                 where ((rc.FormSubmitOn.Value.Date >= fromdate) && (rc.FormSubmitOn.Value.Date <= todate))
                                 select rc;
                if (System.Convert.ToDateTime(TextBoxTo.Text) == System.Convert.ToDateTime(TextBoxFrom.Text))
                {
                    datarecord = from rc in rdb.ClubGurgaonEnquiries
                                 where rc.FormSubmitOn.Value.Date == fromdate
                                 select rc;
                }
                DataTable dt = ToDataTable(datarecord.ToList());
                string savefilepath = Server.MapPath("~/sitecore/admin/BelvedereClubGurgaon/SampleFile/ClubGurgaon-EnquiryRecord.csv");
        ...
        public void DownLoad(string FName)
        {
            ...
            {
                Response.Clear();
                Response.AddHeader("Content-Disposition", "attachment; filename=ClubGurgaon-EnquiryRecord.csv");
