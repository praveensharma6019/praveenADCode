...
namespace Sitecore.GuwahatiAirport.Website.Services
{
    public class CorrigendumRepository
    {
        ...
        public void ActivateCorrigendum(Guid id)
        {
            ...
            {
                GAU_Corrigendum corrigendumItem = (
                    from c in dbcontext.GAU_Corrigendums
                    where c.Id == id
                    select c).Single<GAU_Corrigendum>();
        ...
        public void DeleteCorrigendumDocument(Guid id, string DocumentPath)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                GAU_CorrigendumDocument doc = (
                    from x in dbcontext.GAU_CorrigendumDocuments
                    where x.Id == id
                    select x).Single<GAU_CorrigendumDocument>();
                dbcontext.GAU_CorrigendumDocuments.DeleteOnSubmit(doc);
                try
                {
                    dbcontext.SubmitChanges();
                }
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                if (UserType != "Admin")
                {
                    IQueryable<CorrigendumModel> list =
                        from r in dbcontext.GAU_Corrigendums
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                IQueryable<CorrigedumEditModel> list =
                    ...
                    select new CorrigedumEditModel()
                    {
                        ...
                        CorrigendumDocument = r.GAU_CorrigendumDocuments.Where<GAU_CorrigendumDocument>((GAU_CorrigendumDocument x) => x.CorrigendumId == (Guid?)r.Id).ToList<GAU_CorrigendumDocument>(),
                        CorrigendumTenderMapping = r.GAU_CorrigendumTenderMappings.Where<GAU_CorrigendumTenderMapping>((GAU_CorrigendumTenderMapping x) => x.CorrigendumId == (Guid?)r.Id).ToList<GAU_CorrigendumTenderMapping>()
                    };
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                if (userType != "Admin")
                {
                    IQueryable<Check> list =
                        from r in dbcontext.GAU_TenderLists
                        select new Check()
                        {
                            Id = r.Id,
                            NITNo = r.NITNo,
        ...
        public Guid InsertCorrigendum(CorrigendumModel obj)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                ...
                GAU_Corrigendum list = new GAU_Corrigendum()
                {
                    Id = Guid.NewGuid(),
                    Title = obj.Title,
                ...
                dbcontext.GAU_Corrigendums.InsertOnSubmit(list);
                dbcontext.SubmitChanges();
                id = list.Id;
            }
            return id;
        ...
        public void InsertCorrigendumDocument(CorrigendumModel obj)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                GAU_CorrigendumDocument GAUCorrigendumDocument = new GAU_CorrigendumDocument()
                {
                    Id = Guid.NewGuid(),
                ...
                dbcontext.GAU_CorrigendumDocuments.InsertOnSubmit(GAUCorrigendumDocument);
                dbcontext.SubmitChanges();
            }
        }
        ...
        public void InsertCorrigendumTenderMapping(Guid tenderId, Guid corrigId)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                GAU_CorrigendumTenderMapping GAUCorrigendumTenderMapping = new GAU_CorrigendumTenderMapping()
                {
                    Id = Guid.NewGuid(),
                ...
                dbcontext.GAU_CorrigendumTenderMappings.InsertOnSubmit(GAUCorrigendumTenderMapping);
                dbcontext.SubmitChanges();
            }
        }
        ...
        public List<TenderDetails> NITRNCorrigendum(Guid corrigenudmID)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                ...
                List<Guid?> data = (
                    from x in dbcontext.GAU_CorrigendumTenderMappings
                ...
                foreach (Guid? nullable in data)
                {
                    List<TenderDetails> tendersNITList = (
                        from x in dbcontext.GAU_TenderLists
        ...
        public void UpdateCorrigendumMapping(CorrigedumEditModel obj, List<Check> selectedRecords)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                (
                    ...
                    select x).ToList<GAU_CorrigendumTenderMapping>();
                IQueryable<GAU_CorrigendumTenderMapping> del =
                    from x in dbcontext.GAU_CorrigendumTenderMappings
                    where x.CorrigendumId == (Guid?)obj.Id
                    select x;
                dbcontext.GAU_CorrigendumTenderMappings.DeleteAllOnSubmit<GAU_CorrigendumTenderMapping>(del);
                dbcontext.SubmitChanges();
        ...
        public void UpdateCorrrigendum(CorrigedumEditModel obj)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                GAU_Corrigendum CorrigenList = (
                    from x in dbcontext.GAU_Corrigendums
                    where x.Id == obj.Id
                    select x).Single<GAU_Corrigendum>();
