...
{
    public class GuwahatiAirportRepository
    {
        public GuwahatiAirportRepository()
        {
        }
        ...
        public void ActivateTender(Guid id)
        {
            ...
            {
                GAU_TenderList tenderlist = (
                    from x in dbcontext.GAU_TenderLists
                    where x.Id == id
                    select x).Single<GAU_TenderList>();
        ...
        public void DeleteTenderDocument(Guid id, string DocumentPath)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                GAU_TenderDocument docFile = (
                    from x in dbcontext.GAU_TenderDocuments
                    where x.Id == id
                    select x).Single<GAU_TenderDocument>();
                dbcontext.GAU_TenderDocuments.DeleteOnSubmit(docFile);
                try
                {
                    dbcontext.SubmitChanges();
                }
        ...
        public int DeleteUserTenderDocument(Guid id, string userId, string tenderId)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                GAU_UserTenderDocument docFile = (
                    from x in dbcontext.GAU_UserTenderDocuments
                    where x.Id == id
                    select x).Single<GAU_UserTenderDocument>();
                ...
                {
                    dbcontext.GAU_UserTenderDocuments.DeleteOnSubmit(docFile);
                    try
                    {
                        dbcontext.SubmitChanges();
                        num = 0;
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                IQueryable<TenderEditModel> list =
                    ...
                    select new TenderEditModel()
                    {
                        ...
                        Location = r.Location,
                        Status = r.Staus,
                        TenderDocuments = r.GAU_TenderDocuments.Where<GAU_TenderDocument>((GAU_TenderDocument x) => (x.TenderId == (Guid?)r.Id) && x.IsPQDoc == (bool?)false).ToList<GAU_TenderDocument>(),
                        PQDocuments = r.GAU_TenderDocuments.Where<GAU_TenderDocument>((GAU_TenderDocument x) => (x.TenderId == (Guid?)r.Id) && x.IsPQDoc == (bool?)true).ToList<GAU_TenderDocument>()
                    };
        ...
        public void InsertDocumentList(TenderCreateModel obj)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                ...
                {
                    GAU_TenderDocument GAUTenderDocument = new GAU_TenderDocument()
                    {
                        Id = Guid.NewGuid(),
                    ...
                    dbcontext.GAU_TenderDocuments.InsertOnSubmit(GAUTenderDocument);
                }
                dbcontext.SubmitChanges();
            }
        }
        ...
        public Guid InsertTenderList(TenderCreateModel obj)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                GAU_TenderList Tlist = new GAU_TenderList()
                {
                    Id = Guid.NewGuid(),
                    NITNo = obj.NITNo,
                ...
                dbcontext.GAU_TenderLists.InsertOnSubmit(Tlist);
                dbcontext.SubmitChanges();
                id = Tlist.Id;
            }
            return id;
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                ...
                for (int j = 0; j < (int)pQFiles.Length; j++)
                {
                    ...
                    if (file != null && this.CheckExtension(file.FileName))
                    {
                        ...
                        Path.GetFileNameWithoutExtension(file.FileName);
                        Path.GetExtension(file.FileName);
                        GAU_TenderDocument GAUTenderDocument1 = new GAU_TenderDocument()
                        {
                            Id = Guid.NewGuid(),
                            TenderId = new Guid?(tenderlist.Id),
                            FileName = file.FileName,
                        ...
                        dbcontext.GAU_TenderDocuments.InsertOnSubmit(GAUTenderDocument1);
                        dbcontext.SubmitChanges();
                    }
                }
            }
