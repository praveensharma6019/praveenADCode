...
$(document).ready(function () {
    ...
    function hasScrolled() {
        ...
        $('.header--top_bar').addClass('header--top_bar--fixed');
        ...
        if (st < 150) {
            ...
            $('.header--top_bar').removeClass('header--top_bar--fixed');
...
//    jQuery.ajax(
//        {
//            url: "/api/GuwahatiAirport/GetTenderDocument",
//            method: "GET",
//            data: {
//                Id: id
//            },
...
function loadWeather() {
    var url = "https://api.weatherapi.com/v1/current.json?key=e6f018f1071b4a2c9c392224212408&q=guwahati&aqi=no";
    $.ajax({
        url: url,
...
$("#BtnAPASubmit").click(function () {
    ...
    */
    ...
    $.ajax({
        ...
        url: "/api/GuwahatiAirport/BookAirportAssistance",
        contentType: "application/json",
        success: function (data) {
            //////////////

            if (data.status == "1") {
                window.location.href = "https://stage.adaniuat.com/lgbia-guwahati-airport/assistance-thankyou";
...
$("#DateArrival").on("change", function () {
    ...
    $.ajax({
        ...
        url: "/api/GuwahatiAirport/GuwahatiFlightDetailsArrivalUpdate",
        contentType: "application/json",
        success: function (data) {
            $('#arrival2').html(data);
        }
...
$('span#submit').click(function () {
    ...
    if (confirm("Do you really want to delete this User")) {
        $.ajax({
            type: 'POST',
            data: { id: Id },
            url: "/api/GuwahatiAirport/DisableEnvelopeUser",
            success: function (data) {
                location.reload();
...
$('span#submits').click(function () {
    ...
    if (confirm("Do you really want to delete this User")) {
        $.ajax({
            type: 'POST',
            data: { id: Id },
            url: "/api/GuwahatiAirport/DisableAdminUser",
            success: function (data) {
                location.reload();
...
$("#DateDeparture").on("change", function () {
    ...
    $.ajax({
        ...
        url: "/api/GuwahatiAirport/GuwahatiFlightDetailsUpdateDeparture",
        contentType: "application/json",
        success: function (data) {
...
$("#DateDepartureInter").on("change", function () {
...
$("#btnSearch").click(function () {
    ...
    if ($("#txtSearch").val().length > 0 || $("#txtSearch").val() != "") {
        $.ajax({
            ...
            url: "/api/GuwahatiAirport/GuwahatiFlighSearchDetailsArival",
            contentType: "application/json",
            success: function (data) {
                if (data == "") {
...
$("#btnSearchDeparture").click(function () {
    ...
    if ($("#txtSearches").val().length > 0 || $("#txtSearches").val() != "") {
        $.ajax({
            ...
            url: "/api/GuwahatiAirport/GuwahatiFlighSearchDetailsDeparture",
            contentType: "application/json",
            success: function (data) {
                if (data == "") {
...
$("#BtnASFSubmit").click(function () {
    ...
    $.ajax({
        type: "POST",
        data: JSON.stringify(model),
        url: "/api/GuwahatiAirport/AirportSurveyFeedback",
        contentType: "application/json",
        success: function (data) {
            //////////////
...
$("#Message").blur(function () {
    var MessageRegex = /^[A-Za-z0-9;,.?)()=~+_`\-!@#$%^&*{}|\][:""'?\/\\ ]{1,1000}$/g;
    if (!MessageRegex.test($("#Message").val())) {
        $("#Message").next().html("Please enter a valid Message!");
        return false;
    } else {
        $("#Message").next().html("");
    }
    ...
    grecaptcha.ready(function () {
        grecaptcha.execute('6LdoDj4hAAAAAD7gqiPJEsAy96GJBkiNRu9qohiz', { action: 'AirportContactUs' }).then(function (token) {
            ...
            $.ajax({
                ...
                url: "/api/GuwahatiAirport/AirportContactUs",
                contentType: "application/json",
                success: function (data) {
                    if (data.status == "1") {
                        window.location.href = "/lgbia-guwahati-airport/thankyou";
                    }
...
/* Shop Filter new Script Start */
$(document).ready(function () {
    $('#comming_Soon').hide();
    $("select.filterByNew").change(function () {
        if ($('.sddir-row:visible').length != 0) {
            $('#comming_Soon').hide();
        }

        var filters = $.map($("select.filterByNew").toArray(), function (e) {
            return $(e).val();
        }).join(".");
        $("div#FilterContainer").find(".sddir-row").hide();
        $("div#FilterContainer").find("div." + filters).show();
        if (!$('#terminalDDLNew').val().localeCompare('All')) {
            $("div#FilterContainer").find(".sddir-row").show();
        }
        if (!$('#terminalDDLNew').val().localeCompare('Landside')) {
            $('.Both').hide();
        }
        else {
            $('.Both').show();
