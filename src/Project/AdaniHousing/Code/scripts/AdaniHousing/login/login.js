...
var onloadCallback = function () {
    ...
    if ($('#ReqCallbackrecaptcha').length) {
	 ...
	 if ($('#LoginFormRecaptcha').length) { 
        LoginFormRecaptcha = grecaptcha.render('LoginFormRecaptcha', {
            'sitekey': '6Lcql9QZAAAAAKGvvD4LZKHKys2Dwh0kFUVE5MGc',
            'theme': 'light'
        });
    }
...
$("#login").on("click", function () {
    var response = grecaptcha.getResponse(LoginFormRecaptcha);
    if (response.length === 0) {
        alert("Captcha required.");
        return false;
    }
 ...
 * @return {void}     
 */
function downloadPDF(pdf,customfilename) {
...
$(document).ready(function () {
    ...
    $(".GetSOA").on('click', function (event) {
		...
		var customFileName = invoiceval + "_SOA_report";
        $.ajax({
            type: 'GET',
            url: '/api/AdaniHousing/Get_SOA_Report',
            dataType: 'json',
            ...
            success: function (response) {
                ...
                if (isMobile()) {
					downloadPDF(response.PDF_report, customFileName);
    ...
    $(".GetIntCertificate").on('click', function (event) {
        event.preventDefault();
        var invoiceval = $(this).attr('data-document');
		var customFileName = invoiceval + "_IntersetCertificate";
        $.ajax({
            type: 'GET',
            url: '/api/AdaniHousing/Get_InterestCertificate',
            dataType: 'json',
            data: { LAN: $(this).attr('data-document') },
            success: function (response) {
                if (isIE()) {
    ...
    $(".GetBalConfLetter").on('click', function (event) {
        event.preventDefault();
        var invoiceval = $(this).attr('data-document');
		var customFileName = invoiceval + "_BalanceConfiramtionLetter";
        $.ajax({
            type: 'GET',
            url: '/api/AdaniHousing/Get_BalConfLetter',
            dataType: 'json',
            data: { LAN: $(this).attr('data-document') },
            success: function (response) {
                if (isIE()) {
    ...
    $(".GetWelcome").on('click', function (event) {
        event.preventDefault();
        var invoiceval = $(this).attr('data-document');
		var customFileName = invoiceval + "_WelcomeLetter";
        $.ajax({
            type: 'GET',
            url: '/api/AdaniHousing/Get_WelcomeLetter',
            dataType: 'json',
            data: { LAN: $(this).attr('data-document') },
            success: function (response) {
                if (isIE()) {
                    ...
                    $("#main-pdf").html("<iframe id='iframe1' src='data:application/pdf;base64, " + response.PDF_report + "' width='100%' style='height:100%'></iframe>");
                    $("#printPDFModal").modal('show');
                    $("#pdftodownload").attr('href', "data:application/pdf;base64, " + response.PDF_report, customFileName);
                }
                if (isMobile()) {
					downloadPDF(response.PDF_report, customFileName);
                    $("#printPDFModal").modal('hide');
    ...
    $(".GetRepaySchedule").on('click', function (event) {
        event.preventDefault();
        var invoiceval = $(this).attr('data-document');
		var customFileName = invoiceval + "_RepaymentSchedule";
        $.ajax({
            type: 'GET',
            url: '/api/AdaniHousing/Get_RepaymentSchedule',
            dataType: 'json',
            data: { LAN: $(this).attr('data-document') },
            success: function (response) {
                if (isIE()) {
