...
{
    public class AdaniHousingCollectionCentreListModel
    {
    ...
    public class AdaniHousingGetCCDetails
    {
        ...
        public List<AdaniHousingCollectionCentreListModel> GetCentreLocation(string State, string City)
        {
            var Location = dbContext.AdaniCapitalHousingCollectionCentres.Where(t => t.State.ToUpper() == State.ToUpper() && t.City.ToUpper() == City.ToUpper()).Select(i => new AdaniHousingCollectionCentreListModel { OfficeType = i.OfficeType, Latitude = i.Latitude, Longitude = i.Longitude, Address = i.Address, MerchantOrShop= i.MerchantOrShop,ContactNo=i.ContactNo }).Distinct().ToList();
            if (Location != null)
            {
                return Location;
            }
