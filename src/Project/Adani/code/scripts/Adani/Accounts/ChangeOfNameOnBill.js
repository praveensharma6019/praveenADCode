...
$("#frmCONApplication").submit(function (event) {
    ...
    if (buttonName === "SubmitApplication") {
        ...
        if (isContinueWithExistingSD == "Yes") {
            if (Number($("#SecurityDepositeAmount").val()) < 1 || Number($("#SecurityDepositeAmount").val()) > Number($("#hdnExistingSecurityDepositeAmount").val())) {
                $("#docErrorMessage").html("Security Deposit Amount should be greater than 0 and less than or equal to the exisiting Security Deposit Amount.");
                return false;
            }
        }
        ...
        try {
            ...
            for (var j = 0; j < docname.length; j++) {
                if ($("#" + docname[j].id).val() == null || $("#" + docname[j].id).val() == "") {
					message = message + "Please enter valid document number</br>";
                    error = true;
                }
            }
...
function ValidateFile(value) {
    ...
    if (flag == false) {
        ...
        var data = "_val" + res[1];
        if (size > 2) {
            $("#message_modal .modal-body").text("Max file Size is 2 MB.");
            $("#message_modal").modal("show");
            $(value).val('');
            return false;
        }
...
function ValidatePHFile(value) {
    ...
    if (flag == false) {
        ...
        // var res = str.split("_");
        // var data = "_val" + res[1];
        $("#message_modal .modal-body").text("Only jpg, jpeg, png files are allowed.");
        $("#message_modal").modal("show");
        $(value).val('');
        return false;
    }
