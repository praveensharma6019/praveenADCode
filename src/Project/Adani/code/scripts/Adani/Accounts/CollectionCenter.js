$(document).ready(function () {
    $("#city").change(function () {
        var targetDrp = '#Area';
        if ($("#CollectionType").val() && $(this).val()) {
            $('' + targetDrp + ' option').remove();
            $('' + targetDrp + '').append('<option value="">select</option>');
            $.getJSON('/api/AdaniGas/GetAreaByCenterAndCity', { center: $("#CollectionType").val(), city: $(this).val() }, function (response) {
    ...
    $("#Area").change(function () {
        if ($(this).val()) {
            ...
            var parameters = { "center": $("#CollectionType").val(), "city": $("#city").val(), "area": $(this).val() }
            $.ajax({
                type: 'post',
                url: '/api/AdaniGas/CollectionCenter',
