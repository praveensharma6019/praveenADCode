        ...
        private void Importdata(DataTable dt)
        {
            try
            {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    {
                        Log.Info("Comma in User - " + row["LoginName"].ToString() + " - " + System.DateTime.Now.ToString() + "", this);
                    }
                    if (!string.IsNullOrEmpty(row["LoginName"].ToString()) && !row["LoginName"].ToString().Contains(",") && !Exists(row["LoginName"].ToString()))
                    {
                        if (!string.IsNullOrEmpty(row["Account No"].ToString()))
                        {
                            ...
                            //Sitecore.Data.Items.Item accountitem = CreateAccountItem(row["Account No"].ToString().Trim(), !string.IsNullOrEmpty(row["Meter No"].ToString()) ? row["Meter No"].ToString() : "0000000");
                            //string multiplerecordlist = accountitem.ID.ToString();
                            ...
                            try
                            {
                                ...
                                user.Profile.SetCustomProperty("Date of birth", !string.IsNullOrEmpty(row["Date of birth"].ToString()) ? Sitecore.DateUtil.ToIsoDate(System.Convert.ToDateTime(row["Date of birth"].ToString())) : "");
