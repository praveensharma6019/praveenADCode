...
namespace Sitecore.Electricity.Website.Services
{
    ...
    {
        public string ValidateFile(HttpPostedFileBase file, int size = 5)
        {
            ...
            try
            {
                ...
                else if (file.ContentLength / 1048576 > size)
                {
                    ErrorMessage = file.FileName + "is too big, file size Should Be UpTo " + size + " MB.";
                    return ErrorMessage;
                }
                else
                {
        ...
        public ITSR_Login GetITSRUser(Guid id)
        {
            using (TenderDataContext dbcontext = new TenderDataContext())
            {
        ...
        public bool DeleteITSRUser(Guid id)
        {
            using (TenderDataContext dbcontext = new TenderDataContext())
            {
                var user = dbcontext.ITSR_Logins.Where(u => u.Id == id).FirstOrDefault();
                dbcontext.ITSR_Logins.DeleteOnSubmit(user);
                dbcontext.SubmitChanges();
                return true;
            }
        }
        ...
        public void SendOnResendPasswordEmail(string email, string userName, string password, string redirectUrl)
        {
            //send email to owners
            try
            {
            ...
            {
                Log.Error($"SendOnResetPasswordEmail Unable to send the email to the " + email + " - Error - " + ex.Message + "", ex, this);
                //throw;
            }
        }
