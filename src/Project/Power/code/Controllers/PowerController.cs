...
using Sitecore.Power.Website.Providers;
using System;
...
namespace Sitecore.Power.Website.Controllers
{
    public class PowerController : Controller
    {
        ...
        [HttpPost]
        public ActionResult InsertContactFormdetail(PowerContactModel m)
        {
            bool validationStatus = false;
            var result = new { status = "1" };
            Log.Error("Validating Power ContactForm to stop auto script ", "Start");
            ...
            {
                Log.Error("InsertPowerContactFormRecord ", "Start");
                var getEmailTo = "";

                try
                {
                    PowerContactFormRecordDataContext rdb = new PowerContactFormRecordDataContext();
                    PowerContactFormRecord r = new PowerContactFormRecord();
                    bool isValid = true;
                    ...
                    #region Insert to DB
                    rdb.PowerContactFormRecords.InsertOnSubmit(r);
                    rdb.SubmitChanges();
                ...
                try
                {
                    var msgTpye = Sitecore.Context.Database.GetItem("{5A4D54A7-7E4E-47A7-8BC0-854012216632}");
        ...
        [HttpPost]
        public ActionResult InsertSubscribeUsFormdetail(PowerContactModel m)
        {
            bool validationStatus = false;
            var result = new { status = "1" };
            Log.Error("Validating Power ContactForm to stop auto script ", "Start");
