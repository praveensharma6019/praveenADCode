...
using Sitecore.ElectricityNew.Website;
...
namespace Sitecore.ElectricityNew.Website.Controllers
{
    ...
    public class ElectricityNewController : Controller
    {
        [HttpGet]
        // GET: Survey
            ...
            foreach (var item in parent.ToList())
            {
                ...
                foreach (var appliance in item.GetChildren().InnerChildren.ToList())
                {
                    ...
                    foreach (var ques in appliance.GetChildren().InnerChildren.ToList())
                    {
                        ...
                        {
                            Sitecore.ElectricityNew.Website.Model.Options Objopts = new Options();
        ...
        private bool CheckExtension(HttpPostedFileBase postedFile)
        {
            ...
            //if (postedFile.ContentLength > (2048 * 2048))
            {
                return false;
            }
            ...
            try
            {
                ...
                if (TenderUserSession.TenderUserSessionContext != null && !string.IsNullOrEmpty(TenderUserSession.TenderUserSessionContext.TenderId))
                {
                    ...
                    using (TenderDataContext dbcontext = new TenderDataContext())
                    {
                        ...
                        //Tender.userUpladdocument = null;
                        //Tender.DocumentStatus =   "Pending";
                    ...
                    if (!string.IsNullOrEmpty(tenderFeeandEMD))
                    {
                        try
                        {
                            if (Model.PANNumber != null)
                            {
                                ...
                                {
                                    ModelState.AddModelError(nameof(Model.PANDocument), DictionaryPhraseRepository.Current.Get("/Accounts/Tender fee/PANDocument not valid", "Please upload a valid PANDocument."));
                                    return View(GetTenderDetails());
                                }
                            }
                            if (Model.GSTNumber != null)
                            {
                                if (Model.GSTDocument == null)
                                {
                                    ModelState.AddModelError(nameof(Model.GSTDocument), DictionaryPhraseRepository.Current.Get("/Accounts/Tender fee/GSTDocument not valid", "Please upload a valid GSTDocument."));
                                    return View(GetTenderDetails());
                                }
                            }
                            if (Model.EMDBankRef != null)
                            {
                                if (Model.EMDBankRefDocument == null)
                                {
                                    ModelState.AddModelError(nameof(Model.EMDBankRefDocument), DictionaryPhraseRepository.Current.Get("/Accounts/Tender fee/EMDBankRefDocument not valid", "Please upload a valid EMDBankRefDocument."));
                                    return View(GetTenderDetails());
                                }
                            }
                            if (Model.TenderFeeBankRef != null)
                            {
                                ...
                                {
                                    ModelState.AddModelError(nameof(Model.TenderFeeBankRefDocument), DictionaryPhraseRepository.Current.Get("/Accounts/Tender fee/TenderFeeBankRefDocument not valid", "Please upload a valid TenderFeeBankRefDocument."));
                                    return View(GetTenderDetails());
                                }
                            }
                                ...
                                Model.EMDBankRefDocument != null && Model.EMDBankRefDocument.ContentLength > 0)
                            {
                                if (!ModelState.IsValid)
                                {
                                    if (Model.EMDModeOfPayment == "--Select--")
                                    {
                                        ModelState.AddModelError(nameof(Model.EMDModeOfPayment), DictionaryPhraseRepository.Current.Get("/Accounts/Tender fee/EMD Mode Of Payment", "Please select a value."));
                                    }
                                    return View(GetTenderDetails());
                                }
                                ...
                                else
                                {
                                    if (envelope3DocUploaded == true)
                                    {
                                        ...
                                        using (TenderDataContext dbcontext = new TenderDataContext())
                                        {
                                            ...
                                            var tender = dbcontext.TenderLists.Where(x => x.Id == new System.Guid(tenderId)).FirstOrDefault();
                                            ...
                                            Tn.SendTenderSubmissionMail(details.Email, tender.NITNo, tender.Description);
                                        }
        ...
        public ActionResult TenderChangePassword()
        {
            return View();
        }

        [HttpPost]
        public ActionResult TenderChangePassword(Tender Model)
        {
            ViewBag.ChangePasswordMessage = string.Empty;
            if (string.IsNullOrEmpty(Model.OldPassword))
            {
                ModelState.AddModelError(nameof(Model.OldPassword), DictionaryPhraseRepository.Current.Get("/Accounts/Tender/Required old password", "Please enter old password"));
                return View(GetTenderDetails());
            ...
            using (TenderDataContext dbcontext = new TenderDataContext())
            {
                ...
                if (user != null)
                {
                    ...
                    dbcontext.SubmitChanges();
                    //  ViewBag.Message = "Your password is updated Successfully.";
                    ViewBag.ChangePasswordMessage = "Your password is updated Successfully.";
                    return View(GetTenderDetails());
                }
                else
                {
        ...
        public ActionResult TenderCorrigendumListing()
        {
            using (TenderDataContext dbcontext = new TenderDataContext())
            {
                List<Corrigendum> Corr = dbcontext.Corrigendums.Where(x => x.Status == true).ToList();
        ...
        public ActionResult ClosedTendersListing()
        {
            using (TenderDataContext dbcontext = new TenderDataContext())
            {
                List<TenderList> tenderdata = dbcontext.TenderLists.ToList();
        ...
        public ActionResult TendersPowerPurchaseListing()
        {
            using (TenderDataContext dbcontext = new TenderDataContext())
            {
                List<TenderList> tenderdata = dbcontext.TenderLists.ToList();
        ...
        #region EV charging connection
        [HttpPost]
        public ActionResult EVChargingconnection(EVChargingconnectionModel model, string btnName)
        {
            var errordata = new
            {
                Message = ""
            ...
            try
            {
                EVChargingconnectionServices objEvService = new EVChargingconnectionServices();
                ReCaptchaResponse reCaptchaResponse = VerifyCaptcha(DictionaryPhraseRepository.Current.Get("/sitecore/content/Electricity/Global/Dictionary/Common/CaptchaKey", "6LdYFWgUAAAAAF9HgE8x42aPsoOe6Gf81WV819BC"), model.Captcha);
                if (!reCaptchaResponse.success)
                {
                    errordata = new
                    {
                        Message = "captchainvalid"
                    };
                    return Json(errordata, JsonRequestBehavior.AllowGet);
                }

                if (!string.IsNullOrEmpty(AESEncrytDecry.DecryptStringAES(model.AccountNo)))
                {
                    var consumerDetails = SapPiService.Services.RequestHandler.FetchConsumerDetailsForSolar(AESEncrytDecry.DecryptStringAES(model.AccountNo));
                    if (string.IsNullOrEmpty(consumerDetails.MeterNumber) || consumerDetails.TATA_CONSUMER == null || consumerDetails.TATA_CONSUMER != "" || !objEvService.CheckAccountNumberLength(AESEncrytDecry.DecryptStringAES(model.AccountNo)))
                    {
                        errordata = new
                        {
                            Message = DictionaryPhraseRepository.Current.Get("/Accounts/EVChargingModel/Account Number incorrect", "Enter valid contract account number.")
                        };
                        //model.Message = DictionaryPhraseRepository.Current.Get("/Accounts/EVChargingModel/Account Number incorrect", "Enter valid contract account number.");
                        return Json(errordata, JsonRequestBehavior.AllowGet);
                    }
                    else if (string.IsNullOrEmpty(consumerDetails.MOVE_OUT_DATE) || consumerDetails.MOVE_OUT_DATE != "9999-12-31" || System.Convert.ToDateTime(consumerDetails.MOVE_OUT_DATE).Date < System.DateTime.Now.Date)
                    {
                        errordata = new
                        {
                            Message = DictionaryPhraseRepository.Current.Get("/Accounts/EVChargingModel/Account Number incorrect", "Enter valid contract account number.")
                    ...
                    else
                    {
                        using (TenderDataContext dbcontext = new TenderDataContext())
                        {
                            var ctx = dbcontext.EVChargingconnectionDetails.Where(x => x.AccountNo.ToString() == AESEncrytDecry.DecryptStringAES(model.AccountNo)).FirstOrDefault();
                            if (ctx != null)
                            {
                                errordata = new
                                {
                                    Message = DictionaryPhraseRepository.Current.Get("/Accounts/EVChargingModel/Account Number Found", "Dear consumer, you have already enrolled for this initiative.")
                                };
                                //model.Message = DictionaryPhraseRepository.Current.Get("/Accounts/EVChargingModel/Account Number Found", "Dear consumer, you have already enrolled for this initiative.");
                                return Json(errordata, JsonRequestBehavior.AllowGet);
                            }
                            if (btnName == "submitEVCA")
                            {
                                string generatedOtp = objEvService.GenerateOTPEV(AESEncrytDecry.DecryptStringAES(model.AccountNo), null, "sharecharge", consumerDetails.Mobile, Request.Headers.GetValues("x-auth-key").FirstOrDefault());
                                var apiurl = string.Format(DictionaryPhraseRepository.Current.Get("/Accounts/Settings/SMSAPIforAEMLEV", "https://hapibk.connectbox.in:8181/SendSMS.aspx?UserName=Web_OTP&password=SM78ysAu&MobileNo={0} &SenderID=ADANIE&CDMAHeader=ADANIE&Message={1} is your verification OTP for authentication. Please enter the OTP to verify your details for a seamless user experience. Adani Electricity&dlt_peid=1001756674011432608&dlt_tmid=120210000043&dlt_templateid=1707166185240350000"), consumerDetails.Mobile, generatedOtp);
                                var isOTPSent = objEvService.sendEVMobileSMS(apiurl);
                                if (isOTPSent)
                                {
                                    model.Message = "validateotp";
                                    model.EmailId = AESEncrytDecry.GetEncriptedString(consumerDetails.Email);
                                    model.MobileNo = AESEncrytDecry.GetEncriptedString(consumerDetails.Mobile);
                                    model.CON_OBJ_NO = AESEncrytDecry.GetEncriptedString(consumerDetails.CON_OBJ_NO);
                                    model.SAPNAME = AESEncrytDecry.GetEncriptedString(consumerDetails.Name);
                                    model.STREET = AESEncrytDecry.GetEncriptedString(consumerDetails.Street);
                                    model.STREET2 = AESEncrytDecry.GetEncriptedString(consumerDetails.Street2);
                                    model.RATECAT = AESEncrytDecry.GetEncriptedString(consumerDetails.Rate_Category);
                                    model.DESC_CON_OBJECT = AESEncrytDecry.GetEncriptedString(consumerDetails.DESC_CON_OBJECT);
                                    model.MRU = AESEncrytDecry.GetEncriptedString(consumerDetails.MRU);
                                    model.FUNC_DESCR = AESEncrytDecry.GetEncriptedString(consumerDetails.FUNC_DESCR);
                                    model.REGSTRGROUP = AESEncrytDecry.GetEncriptedString(consumerDetails.REGSTRGROUP);
                                ...
                                else
                                {
                                    ...
                                    //model.Message = "otpfailed";
                                    return Json(errordata, JsonRequestBehavior.AllowGet);
                                }
                            }
                            else if (btnName == "validateotp")
                            {
                                if (objEvService.ValidateShareChargeRequest(AESEncrytDecry.DecryptStringAES(model.AccountNo), Request.Headers.GetValues("x-auth-key").FirstOrDefault()))
                                {
                                    string generatedOTP = string.Empty;
                                    generatedOTP = objEvService.GetShareChargeOTPByMobAndCA(AESEncrytDecry.DecryptStringAES(model.MobileNo), AESEncrytDecry.DecryptStringAES(model.AccountNo), "sharecharge", Request.Headers.GetValues("x-auth-key").FirstOrDefault());

                                    if (!string.Equals(generatedOTP, AESEncrytDecry.DecryptStringAES(model.EVOTP)))
                                    {
                                        errordata = new
                                        {
                                            Message = "otpnotmatched"
                                        };
                                        //model.Message = "otpnotmatched";
                                        return Json(errordata, JsonRequestBehavior.AllowGet);
                                    }
                                    else
                                    {
            ...
            {
                Sitecore.Diagnostics.Log.Error("Error at EVCharging connection:" + ex.Message, this);
                errordata = new
                {
                    Message = "Exception Occured"
                };
        ...
        [HttpPost]
        public ActionResult EVChargingConnectionRequest(EVChargingconnectionModel model)
        {
            var errordata = new
            {
                Message = ""
            ...
            try
            {
                if (objEvService.ValidateShareChargeRequest(AESEncrytDecry.DecryptStringAES(model.AccountNo), Request.Headers.GetValues("x-auth-key").FirstOrDefault()))
                {
                    ...
                    using (TenderDataContext dbcontext = new TenderDataContext())
                    {
                        if (consumerDetails != null && consumerDetails.Mobile != null && model.MobileNo != null)
                        {
                            if (!string.Equals(consumerDetails.Mobile, AESEncrytDecry.DecryptStringAES(model.MobileNo)))
                            {
                                errordata = new
                                {
                                    Message = "infomismatch"
                                };
                                //model.Message = "mobilenumbermismatch";
                                this.Session["UpdateRevampMessage"] = new InfoMessageRevamp(DictionaryPhraseRepository.Current.Get("/Accounts/Controller Messages/EVCharging/MobileNumberMismatchHeading", "Exception Occured"), DictionaryPhraseRepository.Current.Get("/Accounts/Controller Messages/EVCharging/MobileNumberMismatchMessage", "Internal Server Error"), InfoMessageRevamp.MessageTypeRevamp.Error, true);
                                return Json(errordata, JsonRequestBehavior.AllowGet);
                            }
                        }

                        var userExists = dbcontext.EVChargingconnectionDetails.Any(x => x.AccountNo == AESEncrytDecry.DecryptStringAES(model.AccountNo));
                        if (userExists)
                        {
                            errordata = new
                            {
                            ...
                            //model.Message = "alreadyexist";
                            this.Session["UpdateRevampMessage"] = new InfoMessageRevamp(DictionaryPhraseRepository.Current.Get("/Accounts/Controller Messages/EVCharging/AlreadyExistHeading", "Ev Charging Details Already Exists"), DictionaryPhraseRepository.Current.Get("/Accounts/Controller Messages/EVCharging/AlreadyExistMessage", "Dear consumer, you have already enrolled for this initiative."), InfoMessageRevamp.MessageTypeRevamp.Info, true);
                            return Json(errordata, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            if (string.IsNullOrEmpty(Sitecore.Context.User.Profile.UserName))
                            {
                                model.CreatedBy = AESEncrytDecry.DecryptStringAES(model.AccountNo);
                            }
                            else
                            {
                                model.CreatedBy = Sitecore.Context.User.Profile.UserName;
                            }
                            model.udf2 = "Website";
                            var isSaved = objEvService.EVChargingconnectionInsert(model);
                            if (isSaved)
                            {
                                this.Session["UpdateRevampMessage"] = new InfoMessageRevamp(DictionaryPhraseRepository.Current.Get("/Accounts/Controller Messages/EVCharging/SuccessHeading", "Ev Charging Details Saved"), DictionaryPhraseRepository.Current.Get("/Accounts/Controller Messages/EVCharging/SuccessMessage", "Your details have been saved #EMAILID#").Replace("#EMAILID#", model.EmailId), InfoMessageRevamp.MessageTypeRevamp.Success, true);
                                Session["ShareChargeEV"] = "1";
                            }
                            else
                            {
                                errordata = new
                                ...
                                this.Session["UpdateRevampMessage"] = new InfoMessageRevamp(DictionaryPhraseRepository.Current.Get("/Accounts/Controller Messages/EVCharging/FailedHeading", "Failed to save Ev Charging Details"), DictionaryPhraseRepository.Current.Get("/Accounts/Controller Messages/EVCharging/FailedMessage", "Your details are not saved please try again later"), InfoMessageRevamp.MessageTypeRevamp.Error, true);
                                Session["ShareChargeFailed"] = "2";
                                return Json(errordata, JsonRequestBehavior.AllowGet);
                            }
                            ...
                            //model.Message = "success";
                            //model.AccountNo = AESEncrytDecry.GetEncriptedString(model.AccountNo);
                            return Json(errordata, JsonRequestBehavior.AllowGet);
                        }
                    }
                }
            ...
            {
                Sitecore.Diagnostics.Log.Error("Error at EVCharging Connection Post Request:" + ex.Message, this);
                errordata = new
                {
                    Message = "exception occured"
                };
                //model.Message = "exception occured";
                return Json(errordata, JsonRequestBehavior.AllowGet);
            }
        }
        public ActionResult FetchEVChargingConnectionData()
        {
            try
            {
                using (TenderDataContext dbcontext = new TenderDataContext())
                {
                    var ctx = dbcontext.EVChargingconnectionDetails.ToList();
                    List<EVChargingconnectionModel> lstEvObj = new List<EVChargingconnectionModel>();
                    if (ctx != null && ctx.Count > 0)
                    {
                        foreach (var obj in ctx)
                        {
                            EVChargingconnectionModel evObj = new EVChargingconnectionModel();
                            evObj.AccountNo = obj.AccountNo;
                            evObj.EmailId = obj.EmailId;
                            evObj.MobileNo = obj.MobileNo;
                            evObj.EmailIdAlt = obj.EmailIdAlt;
                            evObj.MobileNoAlt = obj.MobileNoAlt;
                            evObj.FullName = obj.FullName;
                            evObj.SocietyName = obj.SocietyName;
                            evObj.PinCode = obj.PinCode;
                            evObj.SecretaryName = obj.SecretaryName;
                            evObj.SecretaryContactNo = obj.SecretaryContactNo;
                            evObj.ParkingArea = obj.ParkingArea;
                            evObj.SocietyMemberEV = obj.SocietyMemberEV;
                            evObj.CON_OBJ_NO = obj.CONOBJNO;
                            evObj.SAPNAME = obj.SAPNAME;
                            evObj.STREET = obj.STREET;
                            evObj.STREET2 = obj.STREET2;
                            evObj.REGSTRGROUP = obj.REGSTRGROUP;
                            evObj.RATECAT = obj.RATECAT;
                            evObj.FUNC_DESCR = obj.FUNC_DESCR;
                            evObj.MRU = obj.MRU;
                            evObj.DESC_CON_OBJECT = obj.DESC_CON_OBJECT;
                            evObj.CreatedBy = obj.CreatedBy;
                            evObj.Created_Date = obj.Created_Date != null ? ((DateTime)obj.Created_Date).ToString("dd-MM-yyyy HH:mm:ss") : "00-00-0000 00:00:00";
                            evObj.udf2 = obj.udf2;
                            lstEvObj.Add(evObj);
                        }
                        return Json(new { data = lstEvObj }, JsonRequestBehavior.AllowGet);
                    }
            ...
            {
                Sitecore.Diagnostics.Log.Error("Error at FetchEVChargingConnectionData Request:" + ex.Message, this);
                return Json(new { data = string.Empty }, JsonRequestBehavior.AllowGet);
            }
        }
        #endregion
        ...
        [HttpPost]
        public ActionResult AEMLVirtualCustomerCare(AEMLVirtualCustomerCare model, string btnName)
        {
            try
            {
                var serviceObj = new AEMLVirtualCustomerCareServices();
                ReCaptchaResponse reCaptchaResponse = VerifyCaptcha(DictionaryPhraseRepository.Current.Get("/sitecore/content/Electricity/Global/Dictionary/Common/CaptchaKey", "6LdYFWgUAAAAAF9HgE8x42aPsoOe6Gf81WV819BC"), model.Captcha);

                if (!reCaptchaResponse.success)
                {
                    model.Message = "captchainvalid";
                    return Json(model, JsonRequestBehavior.AllowGet);
                }
                ...
                if (btnName == "submitCA")
                {
                    ...
                    {
                        model.CANumber = AESEncrytDecry.DecryptStringAES(model.CANumber);
                        

                        var consumerDetails = SapPiService.Services.RequestHandler.FetchConsumerDetailsForSolar(model.CANumber);
                        if (string.IsNullOrEmpty(consumerDetails.MeterNumber) || consumerDetails.TATA_CONSUMER == null || consumerDetails.TATA_CONSUMER != "" || !serviceObj.CheckAccountNumberLength(model.CANumber))
                        {
                            model.Message = DictionaryPhraseRepository.Current.Get("/Accounts/AEMLVirtualCustomerCare/Account Number incorrect", "Enter valid CA account number.");
                            return Json(model, JsonRequestBehavior.AllowGet);
                        }
                        else if (string.IsNullOrEmpty(consumerDetails.MOVE_OUT_DATE) || consumerDetails.MOVE_OUT_DATE != "9999-12-31" || System.Convert.ToDateTime(consumerDetails.MOVE_OUT_DATE).Date < System.DateTime.Now.Date)
                        {
                        ...
                        else if (string.IsNullOrEmpty(consumerDetails.Mobile))
                        {
                            model.Message = DictionaryPhraseRepository.Current.Get("/Accounts/AEMLVirtualCustomerCare/MobileEmpty", "Please register your mobile number to access this service");
                            return Json(model, JsonRequestBehavior.AllowGet);
                        }
                        else if (string.IsNullOrEmpty(consumerDetails.Email))
                        {
                            model.Message = DictionaryPhraseRepository.Current.Get("/Accounts/AEMLVirtualCustomerCare/EmailEmpty", "Please register your email id to access this service");
                            return Json(model, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                          
                            model.Name = AESEncrytDecry.GetEncriptedString(consumerDetails.Name);
                            model.EmailId = AESEncrytDecry.GetEncriptedString(consumerDetails.Email);
                            model.MobileNumber = AESEncrytDecry.GetEncriptedString(consumerDetails.Mobile);
                            model.CAType = AESEncrytDecry.GetEncriptedString(consumerDetails.BP_Type);
                            //model.Message = "sendotp";
                            #region Generate New Otp for given mobile number and save to database
                            string generatedotp = serviceObj.GenerateOTPAEMLVCC(model.CANumber, null, "AEMLVCC", consumerDetails.Mobile);
                            #endregion

                            var apiurl = string.Format(DictionaryPhraseRepository.Current.Get("/Accounts/Settings/SMSAPIforAEMLVCC", "https://hapibk.connectbox.in:8181/SendSMS.aspx?UserName=Web_OTP&password=SM78ysAu&MobileNo={0} &SenderID=ADANIE&CDMAHeader=ADANIE&Message=OTP is {1} for video call verification. Adani Electricity.&dlt_peid=1001756674011432608&dlt_tmid=120210000043&dlt_templateid=1707163886760044396"), consumerDetails.Mobile, generatedotp);


                            var isOTPSent = serviceObj.sendAEMLVCCMobileSMS(apiurl);
                            if (isOTPSent)
                            {
                                model.Message = "validateotp";
                            }
                    ...
                    {
                        model.Message = DictionaryPhraseRepository.Current.Get("/Accounts/AEMLVirtualCustomerCare/Account Number incorrect", "Enter valid contract account number.");
                        return Json(model, JsonRequestBehavior.AllowGet);
                    }
                }
                else if (btnName == "validateotp")
                {
                    string generatedOTP = string.Empty;
                    generatedOTP = serviceObj.GetOTPRegistrationByMobAndCA(AESEncrytDecry.DecryptStringAES(model.MobileNumber), AESEncrytDecry.DecryptStringAES(model.CANumber), "AEMLVCC");

                    if (!string.Equals(generatedOTP, AESEncrytDecry.DecryptStringAES(model.VCCOTP)))
                    {
                        model.Message = "otpnotmatched";
                        return Json(model, JsonRequestBehavior.AllowGet);
                    }
            ...
            {
                Sitecore.Diagnostics.Log.Error("Error at AEMLVirtualCustomerCare: " + ex.Message, ex, this);
                model.Message = "Exception Occured";
                return Json(model, JsonRequestBehavior.AllowGet);
            }
        }
        ...
        [HttpPost]
        public ActionResult CreateAEMLVCCUCID(AEMLVirtualCustomerCare model, string btnName)
        {
            try
            {
                var serviceObj = new AEMLVirtualCustomerCareServices();
                ...
                Sitecore.Data.Items.Item skillItem = Sitecore.Context.Database.GetItem("{88DFCF80-070A-446B-8267-55EB2584515D}").GetChildren().FirstOrDefault(p => p["Language"].ToLower() == AESEncrytDecry.DecryptStringAES(model.Preferred_Language).ToLower() &&
              p["Priority"].ToLower() == AESEncrytDecry.DecryptStringAES(model.Priority).ToLower() && p["Consumer Segment"].ToLower().Replace("-", "").Replace(" ", "") == AESEncrytDecry.DecryptStringAES(model.CAType).ToLower().Replace("-", "").Replace(" ", "") && p["Interaction Type"].ToLower() == AESEncrytDecry.DecryptStringAES(model.Interaction_Type).ToLower());

                if (skillItem != null)
                {
                    if (btnName == "AdhocVCC")
                    {
                        var checkAdhocVccExpectedTimeURL = DictionaryPhraseRepository.Current.Get("/Accounts/AEMLVirtualCustomerCare/checkAdhocVccExpectedTimeURl", "https://ymconnector.adanielectricity.com:8443/api/ADRT/Getskillsetinfo");

                        var checkAdhocVccExpectedTimeClient = new RestClient(checkAdhocVccExpectedTimeURL);
                        var checkAdhocVccExpectedTimerequest = new RestRequest(Method.GET);
                        checkAdhocVccExpectedTimerequest.AddParameter("skillsetId", skillItem["Skillset ID"]);

                        IRestResponse checkAdhocVccExpectedTimeResponse = checkAdhocVccExpectedTimeClient.Execute(checkAdhocVccExpectedTimerequest);
                        //var checkAdhocVccExpectedTimeObjResponse = JsonConvert.DeserializeObject<AdhocVccModal>(checkAdhocVccExpectedTimeResponse.Content);
                        var checkAdhocVccExpectedTimeObjResponse = JsonConvert.DeserializeObject<List<AdhocVccModal>>(checkAdhocVccExpectedTimeResponse.Content);

                        Sitecore.Diagnostics.Log.Error("Communication detail at checkAdhocVccExpectedTimeURL from: " + checkAdhocVccExpectedTimeURL + "; Request: " + checkAdhocVccExpectedTimerequest + "; Response: " + checkAdhocVccExpectedTimeResponse.Content, this);

                        if (checkAdhocVccExpectedTimeObjResponse != null)
                        {
                            if (checkAdhocVccExpectedTimeObjResponse.Count == 0)
                            {
                                model.Message = "noagentavailable";
                                model.UCID = "";
                                return Json(model, JsonRequestBehavior.AllowGet);
                            }
                            
                            if (checkAdhocVccExpectedTimeObjResponse.FirstOrDefault().expectedtime > 180)
                            {
                                model.Message = "expectedtimeisgreater";
                                model.UCID = "";
                                return Json(model, JsonRequestBehavior.AllowGet);
                            }
                        }
                        ...
                        {
                            Sitecore.Diagnostics.Log.Error("Enable to get proper response at checkAdhocVccExpectedTimeURL from: " + checkAdhocVccExpectedTimeURL + "~"
                                                                                                                                  + checkAdhocVccExpectedTimeResponse.StatusCode + "~"
                                                                                                                                  + checkAdhocVccExpectedTimeResponse.StatusDescription + "~"
                                                                                                                                  + checkAdhocVccExpectedTimeResponse.ResponseStatus + "~"
                                                                                                                                  + checkAdhocVccExpectedTimeResponse.ErrorException + "~"
                                                                                                                                  + checkAdhocVccExpectedTimeResponse.ErrorMessage + "~"
                                                                                                                                  + checkAdhocVccExpectedTimeResponse.Content, this);
                            //model.Message = "unabletoconnectServer";
                            //model.UCID = "";
                            //return Json(model, JsonRequestBehavior.AllowGet);
                        }
                    ...
                    var url = DictionaryPhraseRepository.Current.Get("/Accounts/AEMLVirtualCustomerCare/VCCBaseUrl", "http://10.125.66.117:8080/ModuleEngine/NextAction");

                    var client = new RestClient(url);
                    var request = new RestRequest(Method.POST);
                    request.AddHeader("Content-Type", "application/json");

                    var sessionProps = new
                    {
                        CA_Type = AESEncrytDecry.DecryptStringAES(model.CAType),
                        Priority = AESEncrytDecry.DecryptStringAES(model.Priority),
                        Preferred_Language = AESEncrytDecry.DecryptStringAES(model.Preferred_Language),
                        Channel_Info = "Web Site",
                        CA = AESEncrytDecry.DecryptStringAES(model.CANumber),
                        Mobile = AESEncrytDecry.DecryptStringAES(model.MobileNumber),
                        EmailID = AESEncrytDecry.DecryptStringAES(model.EmailId),
                        Interaction_Type = AESEncrytDecry.DecryptStringAES(model.Interaction_Type),
                        CDN = skillItem["CDN"],
                        Skill = skillItem["Skillset ID"],
                        Repeat = "False"
                    };
                    var details = new
                    {
                        ...
                        actionType = "Start",
                        channel = "Chat/Voice",
                        callData = new { sessionProperties = sessionProps }
                    };

                    var requesBody = JsonConvert.SerializeObject(details);
                    request.AddJsonBody(requesBody);
                    IRestResponse response = client.Execute(request);
                    var objresponse = JsonConvert.DeserializeObject<VCCResponse>(response.Content);
                    if (objresponse != null && objresponse.callData != null)
                    {
                        model.Message = "success";
                        model.UCID = AESEncrytDecry.GetEncriptedString(objresponse.callData.sessionProperties.UCID);
                        Sitecore.Diagnostics.Log.Info("Received response at CreateAEMLVCCUCID from: " + url + "; Request: " + requesBody + "; Response: " + response.Content, this);
                    }
                    else
                    {
                        Sitecore.Diagnostics.Log.Error("Enable to get proper response at CreateAEMLVCCUCID from: " + url + "; Request: " + requesBody + "; Response: " + response.Content, this);
                        model.Message = "unabletoconnectServer";
                        model.UCID = "";
                    }
                ...
                {
                    Sitecore.Diagnostics.Log.Error("Enable to get skillset at CreateAEMLVCCUCID for language: " + model.Preferred_Language + "; Priority: " + model.Priority + "; Consumer Segment: " + model.CAType + "; Interaction Type: " + model.Interaction_Type, this);
            ...
            {
                Sitecore.Diagnostics.Log.Error("Error at CreateAEMLVCCUCID: " + ex.Message, ex, this);
                model.Message = "Exception Occured";
                return Json(model, JsonRequestBehavior.AllowGet);
            }
        }
