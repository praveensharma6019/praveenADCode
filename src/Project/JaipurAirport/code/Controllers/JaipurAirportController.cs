...
using Sitecore.JaipurAirport.Website.Models;
using Sitecore.JaipurAirport.Website.Services;
using Sitecore.JaipurAirport.Website.Utility;
using System;
...
namespace Sitecore.JaipurAirport.Website.Controllers
{
    public class JaipurAirportController : Controller
    {
        ...
        [HttpGet]
        public ActionResult JAIFlightDetails(string ArrivalTime)
        {
            ...
            JaipurAirportFlightModel JaipurAirportmodel = new JaipurAirportFlightModel();
            List<JaipurFlight> JaipurFlights = new List<JaipurFlight>();
            Table<JAI_AirportFlight> JaipurAirportFlightDetail = this.akodb.JAI_AirportFlights;
            var StatusList = Sitecore.Context.Database.GetItem("{51A94457-D140-48FE-B7B3-82DDD33B7F0E}");

            IQueryable<JAI_AirportFlight> JaipurFlightInformation = JaipurAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date).OrderBy(val => val.Scheduled);
           
                foreach (JAI_AirportFlight JaipurAirportFlightData in JaipurFlightInformation)
                {
                    DateTime? schDate = JaipurAirportFlightData.ScheduledTime;
                    ...
                    JaipurFlight Jaipurflights = new JaipurFlight()

                    {
                        Scheduled = schTimeFormat,
                        FlightNumber = JaipurAirportFlightData.FlightNumber.ToString(),
                        ArrivingFrom = JaipurAirportFlightData.ArrivingFrom.ToString(),
                        DepartureFrom = JaipurAirportFlightData.DepartureFrom.ToString(),
                        Airline = JaipurAirportFlightData.Airline,
                        terminal = JaipurAirportFlightData.Terminal,
                        Flightkind = JaipurAirportFlightData.flightkind,
                        FlightType = JaipurAirportFlightData.FlightType,
                    };
                    if (StatusList != null)
                    {
                        if (StatusList.HasChildren)
                        {
                            ...
                            {
                                if (JaipurAirportFlightData.Status == CodeName["CodeValue"])
                                {
                                    var remarkdescription = CodeName["RemarkDescription"];
                                    JaipurAirportFlightData.Status = remarkdescription;
                                    Jaipurflights.Status = JaipurAirportFlightData.Status;
                                }
                                else
                                {
                                    Jaipurflights.Status = JaipurAirportFlightData.Status;
                    ...
                    if (JaipurAirportFlightData.EstimationTime != null)
                    {
                        DateTime? estimateDate = JaipurAirportFlightData.EstimationTime;
                        var estimate = estimateDate.Value.ToShortTimeString();
                        var estimateFormat = DateTime.Parse(estimate).ToString("HH:mm");
                        Jaipurflights.EstimationTime = estimateFormat;
                    }

                    JaipurFlights.Add(Jaipurflights);
                }

              


            JaipurAirportmodel.JaipurAirportList = JaipurFlights;


            return base.View(JaipurAirportmodel.JaipurAirportList.ToList<JaipurFlight>());
        }


        [HttpGet]
        public ActionResult JaipurFlightDetailsArrivalUpdate(string ArrivalTime, string FlightType)
        {
            DateTime filterdate = DateTime.Now;
            if (ArrivalTime != null || ArrivalTime == "")
            {
            ...
            Table<JAI_AirportFlight> JaipurAirportFlightDetail = this.akodb.JAI_AirportFlights;
            var StatusList = Sitecore.Context.Database.GetItem("{51A94457-D140-48FE-B7B3-82DDD33B7F0E}");
            IQueryable<JAI_AirportFlight> JaipurFlightInformation = JaipurAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind== "Arrival" && val.FlightType == FlightType).OrderBy(val => val.Scheduled);
            if (JaipurFlightInformation.Count() >= 1)
            {
                foreach (JAI_AirportFlight JaipurAirportFlightData in JaipurFlightInformation)
                {
                    DateTime? schDate = JaipurAirportFlightData.ScheduledTime;
                    var schTime = JaipurAirportFlightData.ScheduledTime.Value.ToShortTimeString();
                    var schTimeFormat = DateTime.Parse(schTime).ToString("HH:mm");
            ...
            JaipurAirportmodel.JaipurAirportList = JaipurFlights;
            return base.View("/views/JaipurAirport/JaipurFlightDetailsArrivalUpdate.cshtml", JaipurAirportmodel.JaipurAirportList.ToList<JaipurFlight>());

        }

        [HttpGet]
        public ActionResult JaipurFlightDetailsUpdateDeparture(string DepartureFlight, string FlightType)
        {
            DateTime filterdate = DateTime.Now;
            ...
            Table<JAI_AirportFlight> JaipurAirportFlightDetail = this.akodb.JAI_AirportFlights;
            var StatusList = Sitecore.Context.Database.GetItem("{51A94457-D140-48FE-B7B3-82DDD33B7F0E}");
            IQueryable<JAI_AirportFlight> JaipurFlightInformation = JaipurAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind== "Departure" && val.FlightType == FlightType).OrderBy(val => val.Scheduled);
            if (JaipurFlightInformation.Count() >= 1)
            {
                foreach (JAI_AirportFlight JaipurAirportFlightData in JaipurFlightInformation)
                {
            ...
            return base.View("/views/JaipurAirport/JaipurFlightDetailsUpdateDeparture.cshtml", JaipurAirportmodel.JaipurAirportList.ToList<JaipurFlight>());
        }


        [HttpGet]
        public ActionResult JaipurFlighSearchDetailsArival(string Search, string ArrivalFlights, string FlightType)
        {
            DateTime filterdate = DateTime.Now;
            ...
            List<JaipurFlight> JaipurFlights = new List<JaipurFlight>();
            Table<JAI_AirportFlight> JaipurAirportFlightDetail = this.akodb.JAI_AirportFlights;
            IQueryable<JAI_AirportFlight> JaipurFlightInformation = JaipurAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind== "Arrival" && (val.Airline.StartsWith(Search) || val.FlightNumber.StartsWith(Search))).OrderBy(val => val.Scheduled);
            if (JaipurFlightInformation.Count() >= 1)
            {
                foreach (JAI_AirportFlight JaipurAirportFlightData in JaipurFlightInformation)
                {
                    if(JaipurAirportFlightData.FlightType!=FlightType)
                    {
                        continue;
                    }
        ...
        [HttpGet]
        public ActionResult JaipurFlighSearchDetailsDeparture(string Search, string DepartureFlights, string FlightType)
        {
            DateTime filterdate = DateTime.Now;
            ...
            Table<JAI_AirportFlight> JaipurAirportFlightDetail = this.akodb.JAI_AirportFlights;
            var StatusList = Sitecore.Context.Database.GetItem("{51A94457-D140-48FE-B7B3-82DDD33B7F0E}");
            IQueryable<JAI_AirportFlight> JaipurFlightInformation = JaipurAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.FlightType == FlightType && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind== "Departure" && (val.Airline.StartsWith(Search) || val.FlightNumber.StartsWith(Search))).OrderBy(val => val.Scheduled);
            if (JaipurFlightInformation.Count() >= 1)
            {
                foreach (JAI_AirportFlight JaipurAirportFlightData in JaipurFlightInformation)
            {
        ...
        public ActionResult FLIGHTS()
        {
            ...
            clsEncryptAES objcrypt = new clsEncryptAES("@JAIAdaniAirport");

            string userName_ = objcrypt.EncryptText("JAIAirport");
            string Password_ = objcrypt.EncryptText("JAI@AdaniAirport");
            ...
            if (username != "JAIAirport" || password != "JAI@AdaniAirport")
            {
                result = new { status = "3" };
                return Json(result, JsonRequestBehavior.AllowGet);
            }
                ...
                if (OBJ != null)
                {
                    ...
                    try
                    {
                        ...
                        using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                        {
                            ...
                            //if (dbcontext.JAI_AirportFlights.Where(t => t.Scheduled.Value == DateTime.Parse(Schedule.ToString()) && t.FlightNumber.ToString() == fligthNumber).Any())
                            if (dbcontext.JAI_AirportFlights.Where(t => t.OriginDate.Value == DateTime.Parse(Schedule.Value.Date.ToString()) && t.FlightNumber.ToString() == fligthNumber && t.Airline.ToString() == Airline).Any())
                            {

                                //JAI_AirportFlight flight = dbcontext.JAI_AirportFlights.Where(t => t.Scheduled.Value == DateTime.Parse(Schedule.ToString()) && t.FlightNumber.ToString() == fligthNumber).FirstOrDefault();
                                JAI_AirportFlight flight = dbcontext.JAI_AirportFlights.Where(t => t.OriginDate.Value == DateTime.Parse(Schedule.Value.Date.ToString()) && t.FlightNumber.ToString() == fligthNumber && t.Airline.ToString() == Airline).FirstOrDefault();
                            ...
                            {
                                JAI_AirportFlight r1 = new JAI_AirportFlight();
                                ...
                                #region Insert to DB
                                ////
                                dbcontext.JAI_AirportFlights.InsertOnSubmit(r1);
                                // rdb.SubmitChanges();
                                try
                                {
                                    dbcontext.SubmitChanges();
        ...
        public JsonResult GetSearchResult(string itemId)
        {
            ...
            var homeItem = Sitecore.Context.Database.GetItem("/sitecore/content/Adani/Home/Jaipur Airport");
        ...
        public ActionResult InsertContact(AdaniAirportsContactUsForm m)
        {
            ...
            if (validationStatus == true)
            {
                ...
                try
                {
                    ...
                    JAI_AdaniAirportsContactForm r1 = new JAI_AdaniAirportsContactForm();
                    ...
                    #region Insert to DB
                    rdb.JAI_AdaniAirportsContactForms.InsertOnSubmit(r1);
                    rdb.SubmitChanges();
                    //  Session["validate"] = Request.Cookies["SIDCC"].Value;
                }
                catch (Exception ex)
                ...
                try
                {
                    var msgTpye = Sitecore.Context.Database.GetItem("{25D57E4F-4BF5-4DC5-A0C2-29D9343C0398}");
           ...
           if (validationStatus == true)
            {
                ...
                /*  try
                  {
                      ...
                      {
                          if (objcontext.JAI_AirportAssistances.Where(x => x.ContactNumber.ToString() == m.ContactNumber.ToString()).Any())
                          {
                              result = new { status = "3" };
                              return Json(result, JsonRequestBehavior.AllowGet);
                          }
                ...
                if (ModelState.IsValid)
                {
                    ...
                    {
                        TenderDBDataContext rdb = new TenderDBDataContext();
                        JAI_AirportAssistance r1 = new JAI_AirportAssistance();
                        ...
                        #region Insert to DB
                        rdb.JAI_AirportAssistances.InsertOnSubmit(r1);
                        rdb.SubmitChanges();
                        //  Session["validate"] = Request.Cookies["SIDCC"].Value;
                    }
                    catch (Exception ex)
        ...
        private void SendMeetandGreetEmail_Adani(AirportAssistance m)
        {
            ...
            try
            {
                var msgTpye = Sitecore.Context.Database.GetItem("{8FDDF78C-C448-445F-8C3D-FF4A3F8DAF04}");
                getEmailTo = msgTpye.Fields["EmailTo"].Value;


                var settingsItem = Context.Database.GetItem("{C5D582DE-F593-4966-A081-EC6AE2FFD47B}");
        ...
        private void SendMeetandGreetEmail_customer(AirportAssistance m)
        {
            ...
            try
            {
                var settingsItem = Context.Database.GetItem("{EFB53B75-D19C-4508-AEB2-74EF22D33015}");
                var mailTemplateItem = settingsItem;
                var fromMail = mailTemplateItem.Fields[Templates.MailTemplate.Fields.From];
        ...
        public ActionResult TenderListing()
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                List<JAI_TenderList> tenderdata;
                if (!string.IsNullOrEmpty(TenderType))
                    tenderdata = dbcontext.JAI_TenderLists.Where(x => x.TenderType == TenderType).ToList();
                else
                    tenderdata = dbcontext.JAI_TenderLists.ToList();
                ...
                List<JAI_Corrigendum> Corr = dbcontext.JAI_Corrigendums.Where(x => x.Status == true).ToList();
            ...
            if (validationStatus)
            {
                if (ModelState.IsValid)
                {
                    ...
                    try
                    {
                        ...
                        using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                        {
                            var user = dbcontext.JAI_Registrations.Where(x => x.UniqueId == model.UniqueId && x.TenderId == model.TenderID).FirstOrDefault();
                            if (user == null)
                            {
                                JAI_Registration ObjReg = new JAI_Registration();
                                ...
                                dbcontext.JAI_Registrations.InsertOnSubmit(ObjReg);
                                dbcontext.SubmitChanges();
                            }
                            else
                            {
                        ...
                        if (TempData["Error"] == null)
                        {
                            ...
                            try
                            {
                                if (!string.IsNullOrEmpty(RedirectUrl))
                                {
                                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                                    {
                                        var tender = dbcontext.JAI_TenderLists.Where(x => x.Id == model.TenderID).FirstOrDefault();
          ...
          //  validationStatus = true;
            ...
            else
            {
                try
                {
                    ...
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        var registerUser = dbcontext.JAI_Registrations.Where(x => x.UserId == model.LoginName && x.Password == model.Password && x.status == true).FirstOrDefault();
                        if (registerUser != null && registerUser.UserType != "SuperAdmin" && registerUser.UserType != "Admin")
                        {
                            DateTime tenderClosingDate = dbcontext.JAI_TenderLists.Where(t => t.Id == registerUser.TenderId).FirstOrDefault().Closing_Date.Value;
                            if (tenderClosingDate < DateTime.Now && registerUser.UserType.ToLower() == "visitor")
                            {
                                ModelState.AddModelError(nameof(model.Password), DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Login tender Error", "This Tender is closed, you won't be able to login now."));
                                return this.View(model);
                            }
                            else if (registerUser.UserType.ToLower() == "visitor" && registerUser.AllowLogin == false)
                            {
                                ModelState.AddModelError(nameof(model.Password), DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Login tender Error", "You won't be able to login now."));
                                return this.View(model);
                            }
                        }
                        ...
                        {
                            var loginhistories = dbcontext.JAI_LoginHistories.Where(x => x.Userid == model.LoginName && x.CreatedDate.GetValueOrDefault().Date == DateTime.Now.Date).Count();
                            if (loginhistories <= 5)
                            {
                                JAI_LoginHistory entity = new JAI_LoginHistory()
                                {
                                    Userid = model.LoginName,
                                    CreatedDate = DateTime.Now.Date
                                };
                                dbcontext.JAI_LoginHistories.InsertOnSubmit(entity);
                                dbcontext.SubmitChanges();
                            }
                            else
                            {
                            ...
                            ModelState.AddModelError(nameof(model.Password), DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Login User Error", "User and password is not valid. Please enter valid credential."));
                            return this.View(model);
                        }
                ...
                catch (System.Exception ex)
                {
                    ...
                    ModelState.AddModelError(nameof(model.Password), DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Login Technical Error", "There is technical problem. Please try after sometime."));
                    return this.View(model);
                }
            }
        }
        ...
        public bool OpenTenderDocPAth(Guid tenderid, string NITPRNo)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                ...
                bool IsCorrigendumPresent = dbcontext.JAI_CorrigendumTenderMappings.Any(x => x.TenderId == tenderid && x.JAI_Corrigendum.Status == true);
        ...
        public ActionResult Logout()
        {
            ...
            if (TenderUserSession.TenderUserSessionContext != null && !string.IsNullOrEmpty(TenderUserSession.TenderUserSessionContext.userId))
            {
                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                {
                    var result = (from user in dbcontext.JAI_Registrations where user.UserId == TenderUserSession.TenderUserSessionContext.userId select user).Single();
        ...
        private string IsUploadingInSequesnce(string envelopeType, string userid)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                if (envelopeType == "2")
                {
                    if (!dbcontext.JAI_UserTenderDocuments.Any(x => x.UserId == userid && x.EnvelopeType == "1"))
                    {
                ...
                {
                    if (!dbcontext.JAI_UserTenderDocuments.Any(x => x.UserId == userid && x.EnvelopeType == "1") || !dbcontext.JAI_UserTenderDocuments.Any(x => x.UserId == userid && x.EnvelopeType == "2"))
                    {
        ...
        public FileResult DownloadUserFile(Guid id)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                var fileToDownload = dbcontext.JAI_UserTenderDocuments.Where(i => i.Id == id).FirstOrDefault();
        ...
        public FileResult DownloadFile(Guid id)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                var fileToDownload = dbcontext.JAI_TenderDocuments.Where(i => i.Id == id).FirstOrDefault();
                return File(fileToDownload.DocData.ToArray(), fileToDownload.ContentType, fileToDownload.FileName);
            }
        }
        ...
        public FileResult DownloadCorrigendumFile(Guid id)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                var fileToDownload = dbcontext.JAI_CorrigendumDocuments.Where(i => i.Id == id).FirstOrDefault();
                return File(fileToDownload.DocData.ToArray(), fileToDownload.ContentType, fileToDownload.FileName);
            }
        }
        ...
        public ActionResult TenderDetails()
        {
            ...
            try
            {
                ...
                if (TenderUserSession.TenderUserSessionContext != null && !string.IsNullOrEmpty(TenderUserSession.TenderUserSessionContext.TenderId))
                {
                    ...
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        var tenderdetails = dbcontext.JAI_TenderLists.Where(x => x.Id == new System.Guid(tenderId)).FirstOrDefault();
                        var userdocument = dbcontext.JAI_UserTenderDocuments.Where(x => x.UserId == UserId).ToList();
                        var userDetail = dbcontext.JAI_Registrations.Where(x => x.UserId == UserId).FirstOrDefault();
                        ...
                        Tender.tenderdocument = tenderdetails.JAI_TenderDocuments.Where(t => t.IsPQDoc != true).ToList();
                        Tender.tenderPQdocument = tenderdetails.JAI_TenderDocuments.Where(t => t.IsPQDoc == true).ToList();
        ...
        public ActionResult TenderDetails(Tender Model, string saveDoc = null, string uploadDoc = null, string changePassword = null)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(uploadDoc) || !string.IsNullOrEmpty(saveDoc))
                {
                    if (Model != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(uploadDoc))
                        {
                            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                            {
                                var user = dbcontext.JAI_Registrations.Where(x => x.UserId == TenderUserSession.TenderUserSessionContext.userId).FirstOrDefault();
                                if (user != null)
                                {
                                    ...
                                    // List<Registration> UserEmailList = dbcontext.JAI_Registrations.Where(x => x.UserType.ToLower() == "admin" || x.UserType.ToLower() == "superadmin" || (x.UserType.ToLower() == "envelopeadmin" && x.TenderId.ToString() == Model.TenderId.ToString())).ToList();
                                    List<JAI_Registration> UserEmailList;
                                    ...
                                    {
                                        UserEmailList = dbcontext.JAI_Registrations.Where(x => (x.UserType.ToLower() == "envelopeadmin" && x.TenderId.ToString() == Model.TenderId.ToString())).ToList();
                                    }
                                    else
                                    {
                                        UserEmailList = dbcontext.JAI_Registrations.Where(x => x.UserType.ToLower() == "admin" || x.UserType.ToLower() == "superadmin" || (x.UserType.ToLower() == "envelopeadmin" && x.TenderId.ToString() == Model.TenderId.ToString())).ToList();
                                    }
                ...
                if (!string.IsNullOrEmpty(changePassword))
                {
                    ...
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        var user = dbcontext.JAI_Registrations.Where(x => x.UserId == TenderUserSession.TenderUserSessionContext.userId && x.Password == Model.OldPassword).FirstOrDefault();
                        if (user != null)
                        {
        ...
        public string saveUserTenderFile(HttpPostedFileBase file, string envelopeType, string userid)
        {
            if (file != null && file.ContentLength > 0)
            {
                ...
                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                {
                    if (dbcontext.JAI_UserTenderDocuments.Any(x => x.UserId == userid && x.EnvelopeType == envelopeType && x.FileName == file.FileName))
                    {
                    ...
                    else
                    {
                        ...
                        JAI_UserTenderDocument dbdoc = new JAI_UserTenderDocument();
                        ...
                        dbcontext.JAI_UserTenderDocuments.InsertOnSubmit(dbdoc);
                        dbcontext.SubmitChanges();
        ...
        public Tender GetTenderDetails()
        {
            ...
            try
            {
                ...
                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                {
                    ...
                    tender.AdvDate = tenderdetails.Adv_Date;
                    tender.ClosingDate = tenderdetails.Closing_Date;
                    //tender.tenderdocument = tenderdetails.JAI_TenderDocuments.ToList();
                    tender.userUpladdocument = userdocument;
                    tender.PQDocumentStatus = userdocument != null && userdocument.Where(d => d.IsPQDoc == true || d.EnvelopeType == "1").Count() > 0 ? "Uploaded" : "Pending";
                    tender.DocumentStatus = userdocument != null && userdocument.Where(d => d.IsPQDoc == false || d.EnvelopeType == "2").Count() > 0 ? "Uploaded" : "Pending";
                    tender.IsPQVerified = userDetail.IsPQVerified == null ? false : userDetail.IsPQVerified;
        ...
        public ActionResult GetTenderDocument(Guid Id)
        {
            try
            {
                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                {
                    List<JAI_TenderDocument> TD = dbcontext.JAI_TenderDocuments.Where(x => x.TenderId == Id).ToList();
        ...
        {
            JaipurAirportRepository ElecRepo = new JaipurAirportRepository();
        ...
        public ActionResult AdminTenderListing()
        {
            ...
            try
            {
                ...
                List<JAI_TenderList> tenderdata = new List<JAI_TenderList>();
                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                {
                    JAI_TenderList objtender = new JAI_TenderList();
                    if (UserType == "SuperAdmin")
                    {
                        tenderdata = dbcontext.JAI_TenderLists.OrderBy(x => x.Closing_Date).ToList();
                    }
                    else if (UserType == "Admin")
                    {
                        tenderdata = (from tnderlist in dbcontext.JAI_TenderLists orderby tnderlist.Closing_Date ascending where tnderlist.CreatedBy == UserId select tnderlist).ToList();
                    }
                    else // Envelope Admin
                    {
                        tenderdata = (from tdruser in dbcontext.JAI_UserTenderMappings join tnderlist in dbcontext.JAI_TenderLists on tdruser.TenderId equals tnderlist.Id orderby tnderlist.Closing_Date ascending where tdruser.UserId == UserId select tnderlist).ToList();
                    }
        ...
        public ActionResult AdminTenderListing(string Status)
        {
            ...
            try
            {
                ...
                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                {
                    ...
                    if (UserType == "SuperAdmin")
                    {
                        tenderdata = dbcontext.JAI_TenderLists.OrderByDescending(x => x.Created_Date).ToList();
                    }
                    else // Envelope Admin
                    {
                        tenderdata = (from tdruser in dbcontext.JAI_UserTenderMappings join tnderlist in dbcontext.JAI_TenderLists on tdruser.TenderId equals tnderlist.Id where tdruser.UserId == UserId select tnderlist).ToList();
                    }
        ...
        public ActionResult AdminTenderUserListing()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(Request.QueryString["id"]))
                {
                    ...
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        datalist = dbcontext.JAI_Registrations.Where(x => x.TenderId == tenderId && x.status == true && x.UserType.ToLower() == "visitor").Select(s => new RegistedUserHasDocument
                        {
                            Name = s.Name,
                            ...
                            Mobile = s.Mobile,
                            UserId = s.UserId,
                            Description = s.JAI_TenderList.Description,
                            HasDocumentUploaded = dbcontext.JAI_UserTenderDocuments.Where(x => x.UserId == s.UserId && (x.IsPQDoc == false || x.EnvelopeType == "2")).Any() ? "Yes" : "No",
                            HasPQDocumentUploaded = dbcontext.JAI_UserTenderDocuments.Where(x => x.UserId == s.UserId && (x.IsPQDoc == true || x.EnvelopeType == "2")).Any() ? "Yes" : "No",
                            TenderId = s.JAI_TenderList.Id.ToString(),
        ...
        public ActionResult PQVerificationApprove(string uid, Guid tenderid)
        {
            ...
            try
            {
                ...
                if (UserType == "EnvelopeAdmin")
                {
                    ...
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        var tenderDetail = dbcontext.JAI_TenderLists.Where(x => x.Id == tenderid).FirstOrDefault();
                        if (tenderDetail.PQApprovalRequired == true)
                        {
                            var usertoapprove = dbcontext.JAI_Registrations.Where(i => i.UserId == uid).FirstOrDefault();
                            if (usertoapprove.PQEnvelopeReviewStatus == false || usertoapprove.PQEnvelopeReviewStatus == null)
                            {
                                ...
                                List<JAI_Registration> UserEmailList = dbcontext.JAI_Registrations.Where(x => x.UserType.ToLower() == "admin" || x.UserType.ToLower() == "superadmin").ToList();
                ...
                if (adminAproval)
                {
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        ...
                        try
                        {
                            if (!string.IsNullOrEmpty(RedirectUrl))
                            {
                                var tenderDetails = dbcontext.JAI_TenderLists.Where(t => t.Id == tenderid).FirstOrDefault();
                                TenderService Tn = new TenderService();
                ...
                if (!string.IsNullOrEmpty(Request.QueryString["id"]))
                {
                    ...
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        datalist = dbcontext.JAI_Registrations.Where(x => x.TenderId == tenderId && x.status == true && x.UserType.ToLower() == "visitor").Select(s => new RegistedUserHasDocument
                        {
                            ...
                            UserId = s.UserId,
                            Description = s.JAI_TenderList.Description,
                            HasDocumentUploaded = dbcontext.JAI_UserTenderDocuments.Where(x => x.UserId == s.UserId).Any() ? "Yes" : "No",
                            TenderId = s.JAI_TenderList.Id.ToString()
                        }).ToList();
                        var TenderNITNo = dbcontext.JAI_TenderLists.Where(x => x.Id == tenderId).FirstOrDefault().NITNo;
        ...
        public ActionResult AdminTenderDetails()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(Request.QueryString["id"]) && !string.IsNullOrEmpty(Request.QueryString["uid"]))
                {
                    ...
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        var tenderdetails = dbcontext.JAI_TenderLists.Where(x => x.Id == tenderId).FirstOrDefault();
                        var userdocument = dbcontext.JAI_UserTenderDocuments.Where(x => x.UserId == tenderuserid).ToList();

                        var userdetails = dbcontext.JAI_Registrations.Where(x => x.TenderId == tenderId && x.UserId == tenderuserid).FirstOrDefault();
                        ...
                        tender.userUpladdocument = userdocument;
                        tender.PQApprovalRequired = tenderdetails.PQApprovalRequired == null ? false : tenderdetails.PQApprovalRequired;
                        ViewBag.Envelopetype = UserType == "SuperAdmin" ? "0" : dbcontext.JAI_UserTenderMappings.Where(x => x.UserId == UserId && x.TenderId == tenderId).FirstOrDefault().Envelope;
            ...
            else
            {
                try
                {
                    ...
                    if (ModelState.IsValid)
                    {
                        ...
                        {
                            ModelState.AddModelError("Adv_Date", DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Date Invalid formate", "Please Input Date in dd-MM-yyyy HH:mm:ss format"));
                        ...
                        {
                            ModelState.AddModelError("Closing_Date", DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Date Invalid formate", "Please Input Date in dd-MM-yyyy HH:mm:ss format"));
                            return View(ReturnObj(obj));
                        }
                        if (obj.Files.Count() > 0 && obj.PQFiles.Count() > 0)
                        {
                            ...
                            if (obj.Files[0] == null)
                            {
                                ModelState.AddModelError("Files", DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Date Invalid formate", "Please upload tender document"));
                                err = true;
                            }
                            if (obj.PQFiles[0] == null && obj.PQApprovalRequired == true)
                            {
                                ModelState.AddModelError("PQFiles", DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Date Invalid formate", "Please upload PQ document"));
                                err = true;
                            }
                            if (err)
                            {
                        ...
                        foreach (HttpPostedFileBase file in obj.Files)
                        {
                            if (file != null)
                            {
                                ...
                                if (size > 10240)
                                {
                                    ModelState.AddModelError("Files", DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Date Invalid formate", "Tender document size can not exceed then 10 MB"));
                                    return View(ReturnObj(obj));
                                }
                            }
                        }
                        ...
                        foreach (HttpPostedFileBase file in obj.PQFiles)
                        {
                            if (file != null)
                            {
                                ...
                                if (size > 10000)
                                {
                                    ModelState.AddModelError("PQFiles", DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Date Invalid formate", "PQ document size can not exceed then 10 MB"));
                                    return View(ReturnObj(obj));
                                }
                            }
                        }
                        ...
                        if (string.IsNullOrEmpty(obj.EnvelopUser1email) == false || string.IsNullOrEmpty(obj.EnvelopUser1Mobile) == false || string.IsNullOrEmpty(obj.EnvelopUser1Name) == false || obj.EnvelopUser1EnvelopNameCheckboxs.Where(x => x.IsChecked == true).Count() > 0)
                        {
                            ...
                            {
                                ModelState.AddModelError("EnvelopUser1Mobile", DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Date Invalid formate", "All Fields are mandatory for Envelop user."));
                                return View(ReturnObj(obj));
                            }
                        }
                        if (string.IsNullOrEmpty(obj.EnvelopUser2email) == false || string.IsNullOrEmpty(obj.EnvelopUser2Mobile) == false || string.IsNullOrEmpty(obj.EnvelopUser2Name) == false || obj.EnvelopUser2EnvelopNameCheckboxs.Where(x => x.IsChecked == true).Count() > 0)
                        {
                            ...
                            {
                                ModelState.AddModelError("EnvelopUser2Mobile", DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Date Invalid formate", "All Fields are mandatory for Envelop user."));
                                return View(ReturnObj(obj));
                            }
                        }
                        if (string.IsNullOrEmpty(obj.EnvelopUser3email) == false || string.IsNullOrEmpty(obj.EnvelopUser3Mobile) == false || string.IsNullOrEmpty(obj.EnvelopUser3Name) == false)
                        {
                            ...
                            {
                                ModelState.AddModelError("EnvelopUser3Mobile", DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Date Invalid formate", "All Fields are mandatory for Envelop user."));
                                return View(ReturnObj(obj));
                            }
                        }
                        if (string.IsNullOrEmpty(obj.EnvelopUser4email) == false || string.IsNullOrEmpty(obj.EnvelopUser4Mobile) == false || string.IsNullOrEmpty(obj.EnvelopUser4Name) == false)
                        {
                            ...
                            {
                                ModelState.AddModelError("EnvelopUser4Mobile", DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Date Invalid formate", "All Fields are mandatory for Envelop user."));
                                return View(ReturnObj(obj));
                            }
                        }
                        if (string.IsNullOrEmpty(obj.EnvelopUser5email) == false || string.IsNullOrEmpty(obj.EnvelopUser5Mobile) == false || string.IsNullOrEmpty(obj.EnvelopUser5Name) == false)
                        {
                            ...
                            {
                                ModelState.AddModelError("EnvelopUser5Mobile", DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Date Invalid formate", "All Fields are mandatory for Envelop user."));
                                return View(ReturnObj(obj));
                            }
                        }
                        ...
                        if (Envelop1User)
                        {
                            ...
                            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                            {
                                var tender = dbcontext.JAI_TenderLists.Where(x => x.Id == new Guid(EnpObj.SelectTenderId)).FirstOrDefault();
                                if (!string.IsNullOrEmpty(RedirectUrl))
                                {
                                    //Note : Send Mail to User 
                                    TenderService Tn = new TenderService();
                        ...
                        ViewBag.SuccessMsg = DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Tender Successful Message", "Tender Uploaded Successfully");
                        ModelState.Clear();
                        return View(ReturnObj(obj));
                ...
                catch (Exception ex)
                {
                    ...
                    ViewBag.ErrorMsg = DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/AdminTender Exception", "Error in uploading Please try again");
                    return View(ReturnObj(obj));
                }
            }
        ...
        public ActionResult AdminTenderUpdate(TenderEditModel obj, string Inactivate_tender = null, string Activate_tender = null)
        {
            ...
            try
            {
                ...
                else if (ModelState.IsValid)
                {
                    ...
                    else
                    {
                        ...
                        ViewBag.Message = DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Tender Successful Message", "Tender Uploaded Successfully");
                        return Redirect(listingitem.Url());
                    }
                }
                return View(obj);
            ...
            catch (Exception ex)
            {
                ...
                ViewBag.EditError = DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/TenderUpdate Exception", "Error in uploading Please try again");
                return View(obj);
            }
        }
        ...
        public ActionResult TenderCorrigendumList()
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                //var CorrigendumIdList = dbcontext.JAI_CorrigendumTenderMappings.Where(x => x.TenderId == TenderId).Select(x => x.Corrigendum).ToList();
                var TenderDetail = dbcontext.JAI_TenderLists.Where(x => x.Id == TenderId).FirstOrDefault();
                ViewBag.TenderDetail = TenderDetail.NITNo + " - " + TenderDetail.Business;

                List<JAI_Corrigendum> Corr = dbcontext.JAI_CorrigendumTenderMappings.Where(x => x.TenderId == TenderId && x.JAI_Corrigendum.Status == true).Select(x => x.JAI_Corrigendum).ToList();
        ...
        public List<TenderDetails> NITRNoFunction(Guid corrigenudmID)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                ...
                var data = dbcontext.JAI_CorrigendumTenderMappings.Where(x => x.CorrigendumId == corrigenudmID).Select(x => x.TenderId).ToList();
                foreach (var item in data)
                {
                    var tendersNITList = dbcontext.JAI_TenderLists.Where(x => x.Id == item).Select(s => new TenderDetails() { NITPRNo = s.NITNo }).ToList();
        ...
        public List<TenderDetails> TenderDocFunction(Guid corrigenudmID)
        {
            ...
            {
                List<TenderDetails> list = new List<TenderDetails>();
                var TenderDocList = dbcontext.JAI_CorrigendumDocuments.Where(x => x.CorrigendumId == corrigenudmID).Select(x => new TenderDetails() { Id = x.Id, FileName = x.FileName }).ToList();
        ...
        public ActionResult CorrigendumCreate(CorrigendumModel obj)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    {
                        ViewBag.selectedRecordNull = DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Tender Selection Validation", "Please Select Tender for Corrigendum List");
                        return View(obj);
                    }
                    ...
                    {
                        ModelState.AddModelError("Date", DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Date Invalid formate", "Please Input Date in dd-MM-yyyy HH:mm:ss format"));
                        return View(obj);
                    }
                    else
                    {
                        ...
                        foreach (var ls in selectedRecords)
                        {
                            ...
                            try
                            {
                                ...
                                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                                {
                                    var tender = dbcontext.JAI_TenderLists.Where(x => x.Id == TenderId).FirstOrDefault();
                                    var tenderBidders = dbcontext.JAI_Registrations.Where(x => x.TenderId == TenderId && x.UserType.ToLower() == "visitor").Select(s => s.Email).Distinct().ToList();
                    ...
                    ViewBag.SuccessMsg = DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Corrigendum Success Msg", "Corrigendum Uploaded Successfully");
                    ModelState.Clear();
                    CorrigendumModel model = new CorrigendumModel();
            ...
            catch (Exception ex)
            {
                ...
                ViewBag.ErrorMsg = DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Corrigendum exception msg", "Error in Insert Please try again");
                return View(obj);
            }
        }
        ...
        public ActionResult CorrigenudumUpdate(CorrigedumEditModel obj, string Inactivate_Corrigendum = null, string Activate_Corrigendum = null)
        {
            ...
            try
            {
                ...
                else if (ModelState.IsValid)
                {
                    ...
                    {
                        ViewBag.selectedRecordNull = DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Tender Selection Validation", "Please Select Tender List");
                        return View(obj);
                    }
                    else
                    {
            ...
            catch (Exception ex)
            {
                ...
                ViewBag.EditError = DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Corrigendum exception msg", "Error in Insert Please try again");
                return View(obj);
            }
        }
        ...
        public ActionResult DeleteCorrigendumFile(Guid id, string DocumentPath)
        {
            ...
            catch (Exception ex)
            {
                ...
                //return RedirectToAction("EditCorrigendum", "MangaloreAirport", new { @id = Edit_Id });
            }
            //  Guid EditId = (Guid)TempData["id"];
            //return RedirectToAction("EditCorrigendum", "MangaloreAirport", new { @id = EditId });
            return Redirect(updateitem.Url() + "?id=" + TempData["id"]);
        }
        #endregion
            ...
            if (validationStatus)
            {
                ...
                try
                {
                    if (ModelState.IsValid)
                    {
                        ...
                        {
                            ViewBag.SelectTenderId = DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Selection Validation msg", "Please Select Tender From List");
                            return View(ReturnObj);
                        }
                        if (SelectedEnvelope.Count <= 0)
                        {
                            ViewBag.SelectedEnvelope = DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/Envelop selection msg", "Please Select Minimum 1 Envelope");
                            return View(ReturnObj);
                        }
                        ...
                        using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                        {
                            var tender = dbcontext.JAI_TenderLists.Where(x => x.Id == new Guid(obj.SelectTenderId)).FirstOrDefault();
                            if (!string.IsNullOrEmpty(RedirectUrl))
                            {
                                //Note : Send Mail to User 
                                TenderService Tn = new TenderService();
                        ...
                        ViewBag.SuccessMsg = DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/EnvelopUser Success Msg", "User created successfully and an email has been sent with details.");
                        ModelState.Clear();
                        return View(ReturnObj);
                    }
                    else
                ...
                catch (Exception ex)
                {
                    ...
                    ViewBag.ErrorMsg = DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/EnvelopUser Exception Msg", "Error in Insert Please try again");
                    return View(ReturnObj);
                }
            }
            else {
        ...
        public ActionResult DisableEnvelopeUser(string id)
        {
            ...
            catch (Exception ex)
            {
                ...
                ViewBag.DisableUserError = DictionaryPhraseRepository.Current.Get("/JaipurAirport/Tender/DisableEnvelopeUser Exception Msg", "There is Error in Disable Users");
                return View();
            }
        ...
        public ActionResult AirportSurveyFeedback(AirportSurveyFeedback airportSurvey)
        {
            ...
            if (validationStatus == true)
            {
                ...
                if (isValid == false)
                {
                    ...
                    {
                        TenderDBDataContext rdb = new TenderDBDataContext();
                        JAI_AirportSurveyFeedback ASF = new JAI_AirportSurveyFeedback();
                        ASF.Id = (rdb.JAI_AirportSurveyFeedbacks.Select(x => (long?)x.Id).Max()??0) + 1;
                        ...
                        #region Insert to DB
                        rdb.JAI_AirportSurveyFeedbacks.InsertOnSubmit(ASF);
                        rdb.SubmitChanges();
        ...
        //Contact Us Form
        [HttpPost]
        public ActionResult AirportContactUs(JAIAirportContactUs airportContactUs)
        {
            bool isValid = true;
            bool validationStatus = true;
            var result = new { status = "1" };
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    string securitytoken = DictionaryPhraseRepository.Current.Get("/JAIContactFormCRM/securitytoken", "");
                    string CRMAPI = DictionaryPhraseRepository.Current.Get("/JAIContactFormCRM/API", "");
                    Log.Info("CRM Start  JAI Contact Us form", this);
                    ...
                    Log.Info("CRM END  JAI Contact Us form", this);


                    Log.Info("Insert data into JAI Contact Us DB ", this);
                    TenderDBDataContext rdb = new TenderDBDataContext();
                    JAI_AirportContactUsForm contactUsForm = new JAI_AirportContactUsForm();
                    contactUsForm.Id = (rdb.JAI_AirportContactUsForms.Select(i => (long?)i.Id).Max() ?? 0) + 1;
                    ...
                    #region Insert to DB
                    rdb.JAI_AirportContactUsForms.InsertOnSubmit(contactUsForm);
                    rdb.SubmitChanges();
                    Log.Info("data inserted in  DB  sucessfully  JAI Contact Us form", this);
