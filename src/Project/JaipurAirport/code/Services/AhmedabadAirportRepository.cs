...
{
    public class JaipurAirportRepository
    {
        public JaipurAirportRepository()
        {
        }
        ...
        public void ActivateTender(Guid id)
        {
            ...
            {
                JAI_TenderList tenderlist = (
                    from x in dbcontext.JAI_TenderLists
                    where x.Id == id
                    select x).Single<JAI_TenderList>();
        ...
        public void DeleteTenderDocument(Guid id, string DocumentPath)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                JAI_TenderDocument docFile = (
                    from x in dbcontext.JAI_TenderDocuments
                    where x.Id == id
                    select x).Single<JAI_TenderDocument>();
                dbcontext.JAI_TenderDocuments.DeleteOnSubmit(docFile);
                try
                {
                    dbcontext.SubmitChanges();
                }
        ...
        public int DeleteUserTenderDocument(Guid id, string userId, string tenderId)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                JAI_UserTenderDocument docFile = (
                    from x in dbcontext.JAI_UserTenderDocuments
                    where x.Id == id
                    select x).Single<JAI_UserTenderDocument>();
                ...
                {
                    dbcontext.JAI_UserTenderDocuments.DeleteOnSubmit(docFile);
                    try
                    {
                        dbcontext.SubmitChanges();
                        num = 0;
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                IQueryable<TenderEditModel> list =
                    ...
                    select new TenderEditModel()
                    {
                        ...
                        Location = r.Location,
                        Status = r.Staus,
                        TenderDocuments = r.JAI_TenderDocuments.Where<JAI_TenderDocument>((JAI_TenderDocument x) => (x.TenderId == (Guid?)r.Id) && x.IsPQDoc == (bool?)false).ToList<JAI_TenderDocument>(),
                        PQDocuments = r.JAI_TenderDocuments.Where<JAI_TenderDocument>((JAI_TenderDocument x) => (x.TenderId == (Guid?)r.Id) && x.IsPQDoc == (bool?)true).ToList<JAI_TenderDocument>()
                    };
        ...
        public void InsertDocumentList(TenderCreateModel obj)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                ...
                {
                    JAI_TenderDocument aMDTenderDocument = new JAI_TenderDocument()
                    {
                        Id = Guid.NewGuid(),
                    ...
                    dbcontext.JAI_TenderDocuments.InsertOnSubmit(aMDTenderDocument);
                }
                dbcontext.SubmitChanges();
            }
        }
        ...
        public Guid InsertTenderList(TenderCreateModel obj)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                JAI_TenderList Tlist = new JAI_TenderList()
                {
                    Id = Guid.NewGuid(),
                    NITNo = obj.NITNo,
                ...
                dbcontext.JAI_TenderLists.InsertOnSubmit(Tlist);
                dbcontext.SubmitChanges();
                id = Tlist.Id;
            }
            return id;
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                ...
                for (int j = 0; j < (int)pQFiles.Length; j++)
                {
                    ...
                    if (file != null && this.CheckExtension(file.FileName))
                    {
                        ...
                        Path.GetFileNameWithoutExtension(file.FileName);
                        Path.GetExtension(file.FileName);
                        JAI_TenderDocument aMDTenderDocument1 = new JAI_TenderDocument()
                        {
                            Id = Guid.NewGuid(),
                            TenderId = new Guid?(tenderlist.Id),
                            FileName = file.FileName,
                        ...
                        dbcontext.JAI_TenderDocuments.InsertOnSubmit(aMDTenderDocument1);
                        dbcontext.SubmitChanges();
                    }
                }
            }
