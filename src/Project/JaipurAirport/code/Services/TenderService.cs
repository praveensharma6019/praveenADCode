...
namespace Sitecore.JaipurAirport.Website.Services
{
    public class TenderService
    {
        ...
        public void SendEmailOnTenderClose_Scheduler()
        {
            try
            {
                ...
                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                {
                    ...
                    if (dbcontext.JAI_TenderLists.Where(t => t.Closing_Date.Value.Date == DateTime.Now.Date && t.Closing_Date.Value.Hour <= DateTime.Now.Hour && t.IsCloseEmailSent!=true).Any())
                    {
                        var tenders = dbcontext.JAI_TenderLists.Where(t => t.Closing_Date.Value.Date == DateTime.Now.Date && t.Closing_Date.Value.Hour <= DateTime.Now.Hour && t.IsCloseEmailSent != true).ToList();
                        ...
                        foreach (var tender in tenders)
                        {
                            ...
                            var tenderUsers = dbcontext.JAI_UserTenderMappings.Where(u => u.TenderId == tender.Id).Select(m => m.UserId).ToList();
                            var users = dbcontext.JAI_Registrations.Where(u => tenderUsers.Contains(u.UserId)).ToList();
                            ...
                            JAI_TenderList tenderToUpdate = dbcontext.JAI_TenderLists.Where(t => t.Id == tender.Id).FirstOrDefault();
        ...
        public void SendReminderForTenderClose_Scheduler()
        {
            try
            {
                ...
                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                {
                    Log.Info("inside dbcontext at " + DateTime.Now.ToString() + "", this);
                    List<TenderUserDetailsForMail> listTenderUserDetailsForMail = new List<TenderUserDetailsForMail>();
                    if (dbcontext.JAI_TenderLists.Where(t => t.Closing_Date.Value.Date == DateTime.Now.AddDays(1).Date && t.Closing_Date.Value.Hour <= DateTime.Now.Hour && t.IsReminderMailSent != true).Any())
                    {
                        var tenders = dbcontext.JAI_TenderLists.Where(t => t.Closing_Date.Value.Date == DateTime.Now.AddDays(1).Date && t.Closing_Date.Value.Hour <= DateTime.Now.Hour && t.IsReminderMailSent != true).ToList();
                        Log.Info("inside tenders at " + DateTime.Now.ToString() + " Tenders count:" + tenders.Count, this);
                        foreach (var tender in tenders)
                        {
                            Log.Info("inside tender at " + DateTime.Now.ToString() + " Tender name:" + tender.NITNo + "-" + tender.Description, this);
        ...
        public void SendForPQApprovalEmail(List<JAI_Registration> userEmailList, bool PQVerifyed, string UserName, string userMobile, string userId, string TenderName, string NitNo)
        {
            try
            {
                if (PQVerifyed)
                {
                    ...
                    foreach (JAI_Registration email in userEmailList)
                    {
