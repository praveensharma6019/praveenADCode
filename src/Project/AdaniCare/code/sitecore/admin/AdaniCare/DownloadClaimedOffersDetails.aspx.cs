...
{
    public partial class DownloadClaimedOffersDetails : System.Web.UI.Page
    {
        ...
        protected void btnDownload_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var predicateQuery = dc.ClaimedOffersDetails.AsQueryable();
                ...
                if (!string.IsNullOrEmpty(txtFromDate.Text) && !string.IsNullOrEmpty(txtToDate.Text))
                {
                    ...
                    predicateQuery = predicateQuery.Where(p => p.ClaimedDate >= System.Convert.ToDateTime(txtFromDate.Text) && p.ClaimedDate <= System.Convert.ToDateTime(txtToDate.Text));
                }
                ...
                if (predicateQuery != null && predicateQuery.Any())
                {
                    ...
                    var claimedOffer = predicateQuery.ToList().Select(x => new ClaimedOffersDetail()
                    {
                        ID=x.ID,
                        ConsumerName = x.ConsumerName,
                        ConsumerAccountNumber = x.ConsumerAccountNumber,
                        ConsumerMobileNumber = x.ConsumerMobileNumber,
                        ClaimedEmailAddress = x.ClaimedEmailAddress,
                        ClaimedMobileNumber = x.ClaimedMobileNumber,
                        OfferId = x.OfferId,
                        OfferCompany = x.OfferCompany,
                        OfferName = x.OfferName,
                        ClaimedDate = x.ClaimedDate,
                    }).ToList();

                    DataTable dt = ToDataTable(claimedOffer);
                    string savefilepath = Server.MapPath("~/sitecore/admin/AdaniCare/SampleFile/ClaimedOfferDetails.csv");
        ...
        public void DownLoad(string FName)
        {
            ...
            if (file.Exists)
            {
                ...
                Response.Clear();
                Response.AddHeader("Content-Disposition", "attachment; filename=Claimed_offers.csv");
