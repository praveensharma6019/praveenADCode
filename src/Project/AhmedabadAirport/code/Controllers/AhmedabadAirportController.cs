...
using Sitecore.AhmedabadAirport.Website.Models;
using Sitecore.AhmedabadAirport.Website.Services;
using Sitecore.AhmedabadAirport.Website.Utility;
...
namespace Sitecore.AhmedabadAirport.Website.Controllers
{
    public class AhmedabadAirportController : Controller
    {
        ...
        [HttpGet]
        public ActionResult AmdFlightDetails(string ArrivalTime)
        {
            ...
            AhmedabadAirportFlightModel ahmedabadairportmodel = new AhmedabadAirportFlightModel();
            List<AhmedabadFlight> AhmedabadFlights = new List<AhmedabadFlight>();
            Table<AMD_AirportFlight> AhmedabadAirportFlightDetail = this.akodb.AMD_AirportFlights;
            var StatusList = Sitecore.Context.Database.GetItem("{978F4AC0-5FCC-45F4-B8E1-2CF5DF7D6982}");

            IQueryable<AMD_AirportFlight> AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date).OrderBy(val => val.Scheduled);

            foreach (AMD_AirportFlight AhmedabadAirportFlightData in AhmedabadFlightInformation)
            {
                DateTime? schDate = AhmedabadAirportFlightData.ScheduledTime;
                ...
                AhmedabadFlight ahmedabadflights = new AhmedabadFlight()

                {
                    Scheduled = schTimeFormat,
                    FlightNumber = AhmedabadAirportFlightData.FlightNumber.ToString(),
                    ArrivingFrom = AhmedabadAirportFlightData.ArrivingFrom.ToString(),
                    DepartureFrom = AhmedabadAirportFlightData.DepartureFrom.ToString(),
                    Airline = AhmedabadAirportFlightData.Airline,
                    terminal = AhmedabadAirportFlightData.Terminal,
                    Flightkind = AhmedabadAirportFlightData.flightkind,
                    OriginDate = (DateTime)AhmedabadAirportFlightData.OriginDate,
                    ScheduledTime = (DateTime)AhmedabadAirportFlightData.ScheduledTime,
                    //EstimationTime = AhmedabadAirportFlightData.ToString(),
                    //ActualTime = (DateTime)AhmedabadAirportFlightData.ActualTime
                };
                if (StatusList != null)
                {
                    if (StatusList.HasChildren)
                    {
                        ...
                        {
                            if (AhmedabadAirportFlightData.Status == CodeName["CodeValue"])
                            {
                                var remarkdescription = CodeName["RemarkDescription"];
                                AhmedabadAirportFlightData.Status = remarkdescription;
                                ahmedabadflights.Status = AhmedabadAirportFlightData.Status;
                            }
                            else
                            {
                                ahmedabadflights.Status = AhmedabadAirportFlightData.Status;
                ...
                if (AhmedabadAirportFlightData.EstimationTime != null)
                {
                    DateTime? estimateDate = AhmedabadAirportFlightData.EstimationTime;
                    var estimate = estimateDate.Value.ToShortTimeString();
                    var estimateFormat = DateTime.Parse(estimate).ToString("HH:mm");
                    ahmedabadflights.EstimationTime = estimateFormat;
                }

                AhmedabadFlights.Add(ahmedabadflights);
            }




            ahmedabadairportmodel.AhmedabadAirportList = AhmedabadFlights;
            ViewBag.OriginArrival = AhmedabadAirportFlightDetail.Where(a => a.flightkind == "Arrival").Select(e => e.ArrivingFrom).Distinct().ToList();
            ViewBag.OriginDeparture = AhmedabadAirportFlightDetail.Where(a => a.flightkind == "Departure").Select(e => e.ArrivingFrom).Distinct().ToList();
            ViewBag.AirlineArrival = AhmedabadAirportFlightDetail.Where(a => a.flightkind == "Arrival").Select(e => e.Airline).Distinct().ToList();
            ViewBag.AirlineDeparture = AhmedabadAirportFlightDetail.Where(a => a.flightkind == "Departure").Select(e => e.Airline).Distinct().ToList();

            return base.View(ahmedabadairportmodel.AhmedabadAirportList.ToList<AhmedabadFlight>());
        }


        [HttpGet]
        public ActionResult AhmedabadFlightDetailsArrivalUpdate(string ArrivalTime)
        {
            DateTime filterdate = DateTime.Now;
            if (ArrivalTime != null || ArrivalTime == "")
            {
            ...
            Table<AMD_AirportFlight> AhmedabadAirportFlightDetail = this.akodb.AMD_AirportFlights;
            var StatusList = Sitecore.Context.Database.GetItem("{978F4AC0-5FCC-45F4-B8E1-2CF5DF7D6982}");
            IQueryable<AMD_AirportFlight> AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind == "Arrival").OrderBy(val => val.Scheduled);
            if (AhmedabadFlightInformation.Count() >= 1)
            {
                foreach (AMD_AirportFlight AhmedabadAirportFlightData in AhmedabadFlightInformation)
                {
                    DateTime? schDate = AhmedabadAirportFlightData.ScheduledTime;
                    var schTime = AhmedabadAirportFlightData.ScheduledTime.Value.ToShortTimeString();
                    var schTimeFormat = DateTime.Parse(schTime).ToString("HH:mm");
            ...
            ahmedabadairportmodel.AhmedabadAirportList = AhmedabadFlights;
            return base.View("/views/AhmedabadAirport/AhmedabadFlightDetailsArrivalUpdate.cshtml", ahmedabadairportmodel.AhmedabadAirportList.ToList<AhmedabadFlight>());

        }

        [HttpGet]
        public ActionResult AhmedabadFlightDetailsUpdateDeparture(string DepartureFlight)
        {
            DateTime filterdate = DateTime.Now;
            ...
            Table<AMD_AirportFlight> AhmedabadAirportFlightDetail = this.akodb.AMD_AirportFlights;
            var StatusList = Sitecore.Context.Database.GetItem("{978F4AC0-5FCC-45F4-B8E1-2CF5DF7D6982}");
            IQueryable<AMD_AirportFlight> AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind == "Departure").OrderBy(val => val.Scheduled);
            if (AhmedabadFlightInformation.Count() >= 1)
            {
                foreach (AMD_AirportFlight AhmedabadAirportFlightData in AhmedabadFlightInformation)
                {
            ...
            return base.View("/views/AhmedabadAirport/AhmedabadFlightDetailsUpdateDeparture.cshtml", ahmedabadairportmodel.AhmedabadAirportList.ToList<AhmedabadFlight>());
        }


        [HttpGet]
        public ActionResult AhmedabadFlighSearchDetailsArival(string Search, string ArrivalFlights)
        {
            DateTime filterdate = DateTime.Now;
            ...
            List<AhmedabadFlight> AhmedabadFlights = new List<AhmedabadFlight>();
            Table<AMD_AirportFlight> AhmedabadAirportFlightDetail = this.akodb.AMD_AirportFlights;
            IQueryable<AMD_AirportFlight> AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind == "Arrival" && (val.Airline.StartsWith(Search) || val.FlightNumber.StartsWith(Search))).OrderBy(val => val.Scheduled);
            if (AhmedabadFlightInformation.Count() >= 1)
            {
                foreach (AMD_AirportFlight AhmedabadAirportFlightData in AhmedabadFlightInformation)
                {
        ...
        //[HttpGet]
        //public ActionResult AhmedabadFlighSearchDetailsArival(string Search, string Airline, string FlightType, string Origin, string ArrivalFlights)
        //{
        //    DateTime filterdate = DateTime.Now;
        //    if (!String.IsNullOrEmpty(ArrivalFlights) && ArrivalFlights != "All")
        //    {
        //        filterdate = DateTime.Parse(ArrivalFlights);
        //    }
        //    var StatusList = Sitecore.Context.Database.GetItem("{978F4AC0-5FCC-45F4-B8E1-2CF5DF7D6982}");
        ...
        //    List<AhmedabadFlight> AhmedabadFlights = new List<AhmedabadFlight>();
        //    Table<AMD_AirportFlight> AhmedabadAirportFlightDetail = this.akodb.AMD_AirportFlights;
        //    IQueryable<AMD_AirportFlight> AhmedabadFlightInformation;
        //    if (!String.IsNullOrEmpty(Search))
        //    {
        //        AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind == "Arrival" && (val.Airline.Contains(Search) || val.FlightNumber.StartsWith(Search) || val.OperationalStatus.Contains(Search) || val.Status.Contains(Search) || val.ArrivingFrom.Contains(Search))).OrderBy(val => val.Scheduled);
        //    }
        //    else
        //    {
        //        if (Origin == "All" && Airline == "All" && ArrivalFlights == "All" && FlightType == "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date >= filterdate.Date && val.flightkind == "Arrival").OrderBy(val => val.Scheduled);
        //        }
        //        else if (Origin == "All" && Airline == "All" && ArrivalFlights != "All" && FlightType == "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind == "Arrival").OrderBy(val => val.Scheduled);
        //        }
        //        else if (Origin == "All" && Airline != "All" && ArrivalFlights == "All" && FlightType == "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date >= filterdate.Date && val.flightkind == "Arrival" && val.Airline.Contains(Airline)).OrderBy(val => val.Scheduled);
        //        }
        //        else if (Origin == "All" && Airline != "All" && ArrivalFlights != "All" && FlightType == "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind == "Arrival" && val.Airline.Contains(Airline)).OrderBy(val => val.Scheduled);
        ...
        //        else if (Origin != "All" && Airline == "All" && ArrivalFlights == "All" && FlightType == "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date >= filterdate.Date && val.flightkind == "Arrival" && val.ArrivingFrom.Contains(Origin)).OrderBy(val => val.Scheduled);
        //        }
        //        else if (Origin != "All" && Airline == "All" && ArrivalFlights != "All" && FlightType == "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind == "Arrival" && val.ArrivingFrom.Contains(Origin)).OrderBy(val => val.Scheduled);
        ...
        //        else if (Origin != "All" && Airline != "All" && ArrivalFlights == "All" && FlightType == "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date >= filterdate.Date && val.flightkind == "Arrival" && val.ArrivingFrom.Contains(Origin) && val.Airline.Contains(Airline)).OrderBy(val => val.Scheduled);
        //        }
        //        else if (Origin != "All" && Airline != "All" && ArrivalFlights != "All" && FlightType == "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind == "Arrival" && val.ArrivingFrom.Contains(Origin) && val.Airline.Contains(Airline)).OrderBy(val => val.Scheduled);
        ...
        //        else if (Origin == "All" && Airline == "All" && ArrivalFlights == "All" && FlightType != "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date >= filterdate.Date && val.flightkind == "Arrival" && val.FlightType == FlightType).OrderBy(val => val.Scheduled);
        //        }
        //        else if (Origin == "All" && Airline == "All" && ArrivalFlights != "All" && FlightType != "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind == "Arrival" && val.FlightType == FlightType).OrderBy(val => val.Scheduled);
        ...
        //        else if (Origin == "All" && Airline != "All" && ArrivalFlights == "All" && FlightType != "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date >= filterdate.Date && val.flightkind == "Arrival" && val.Airline.Contains(Airline) && val.FlightType == FlightType).OrderBy(val => val.Scheduled);
        //        }
        //        else if (Origin == "All" && Airline != "All" && ArrivalFlights != "All" && FlightType != "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind == "Arrival" && val.Airline.Contains(Airline) && val.FlightType == FlightType).OrderBy(val => val.Scheduled);
        ...
        //        else if (Origin != "All" && Airline == "All" && ArrivalFlights == "All" && FlightType != "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date >= filterdate.Date && val.flightkind == "Arrival" && val.ArrivingFrom.Contains(Origin) && val.FlightType == FlightType).OrderBy(val => val.Scheduled);
        //        }
        //        else if (Origin != "All" && Airline == "All" && ArrivalFlights != "All" && FlightType != "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind == "Arrival" && val.ArrivingFrom.Contains(Origin) && val.FlightType == FlightType).OrderBy(val => val.Scheduled);
        ...
        //        else if (Origin != "All" && Airline != "All" && ArrivalFlights == "All" && FlightType != "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date >= filterdate.Date && val.flightkind == "Arrival" && val.ArrivingFrom.Contains(Origin) && val.Airline.Contains(Airline) && val.FlightType == FlightType).OrderBy(val => val.Scheduled);
        //        }
        //        else
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind == "Arrival" && val.ArrivingFrom.Contains(Origin) && val.Airline.Contains(Airline)).OrderBy(val => val.Scheduled);
        ...
        [HttpGet]
        public ActionResult AhmedabadFlighSearchDetailsDeparture(string Search, string DepartureFlights)
        {
            DateTime filterdate = DateTime.Now;
        ...
        //[HttpGet]
        //public ActionResult AhmedabadFlighSearchDetailsDeparture(string Search, string Airline, string Origin,string FlightType, string DepartureFlights)
        //{
        //    DateTime filterdate = DateTime.Now;
        //    if (!String.IsNullOrEmpty(DepartureFlights) && DepartureFlights !="All")
        //    {
        //        filterdate = DateTime.Parse(DepartureFlights);
        //    }
        ...
        //    if (!String.IsNullOrEmpty(Search))
        //    {
        //        AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind == "Departure" && (val.Airline.Contains(Search) || val.FlightNumber.StartsWith(Search) || val.OperationalStatus.Contains(Search) || val.Status.Contains(Search) || val.ArrivingFrom.Contains(Search))).OrderBy(val => val.Scheduled);
        //    }
        //    else
        //    {
        //        if (Origin == "All" && Airline == "All" && DepartureFlights == "All" && FlightType == "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date >= filterdate.Date && val.flightkind == "Departure").OrderBy(val => val.Scheduled);
        //        }
        //        else if (Origin == "All" && Airline == "All" && DepartureFlights != "All" && FlightType == "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind == "Departure").OrderBy(val => val.Scheduled);
        //        }
        //        else if (Origin == "All" && Airline != "All" && DepartureFlights == "All" && FlightType == "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date >= filterdate.Date && val.flightkind == "Departure" && val.Airline.Contains(Airline)).OrderBy(val => val.Scheduled);
        //        }
        //        else if (Origin == "All" && Airline != "All" && DepartureFlights != "All" && FlightType == "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind == "Departure" && val.Airline.Contains(Airline)).OrderBy(val => val.Scheduled);
        ...
        //        else if (Origin != "All" && Airline == "All" && DepartureFlights == "All" && FlightType == "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date >= filterdate.Date && val.flightkind == "Departure" && val.ArrivingFrom.Contains(Origin)).OrderBy(val => val.Scheduled);
        //        }
        //        else if (Origin != "All" && Airline == "All" && DepartureFlights != "All" && FlightType == "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind == "Departure" && val.ArrivingFrom.Contains(Origin)).OrderBy(val => val.Scheduled);
        ...
        //        else if (Origin != "All" && Airline != "All" && DepartureFlights == "All" && FlightType == "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date >= filterdate.Date && val.flightkind == "Departure" && val.ArrivingFrom.Contains(Origin) && val.Airline.Contains(Airline)).OrderBy(val => val.Scheduled);
        //        }
        //        else if (Origin != "All" && Airline != "All" && DepartureFlights != "All" && FlightType == "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind == "Departure" && val.ArrivingFrom.Contains(Origin) && val.Airline.Contains(Airline)).OrderBy(val => val.Scheduled);
        ...
        //        else if (Origin == "All" && Airline == "All" && DepartureFlights == "All" && FlightType != "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date >= filterdate.Date && val.flightkind == "Departure" && val.FlightType == FlightType).OrderBy(val => val.Scheduled);
        //        }
        //        else if (Origin == "All" && Airline == "All" && DepartureFlights != "All" && FlightType != "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind == "Departure" && val.FlightType == FlightType).OrderBy(val => val.Scheduled);
        ...
        //        else if (Origin == "All" && Airline != "All" && DepartureFlights == "All" && FlightType != "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date >= filterdate.Date && val.flightkind == "Departure" && val.Airline.Contains(Airline) && val.FlightType == FlightType).OrderBy(val => val.Scheduled);
        //        }
        //        else if (Origin == "All" && Airline != "All" && DepartureFlights != "All" && FlightType != "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind == "Departure" && val.Airline.Contains(Airline) && val.FlightType == FlightType).OrderBy(val => val.Scheduled);
        ...
        //        else if (Origin != "All" && Airline == "All" && DepartureFlights == "All" && FlightType != "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date >= filterdate.Date && val.flightkind == "Departure" && val.ArrivingFrom.Contains(Origin) && val.FlightType == FlightType).OrderBy(val => val.Scheduled);
        //        }
        //        else if (Origin != "All" && Airline == "All" && DepartureFlights != "All" && FlightType != "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind == "Departure" && val.ArrivingFrom.Contains(Origin) && val.FlightType == FlightType).OrderBy(val => val.Scheduled);
        ...
        //        else if (Origin != "All" && Airline != "All" && DepartureFlights == "All" && FlightType != "All")
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date >= filterdate.Date && val.flightkind == "Departure" && val.ArrivingFrom.Contains(Origin) && val.Airline.Contains(Airline) && val.FlightType == FlightType).OrderBy(val => val.Scheduled);
        //        }
        //        else 
        //        {
        //            AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind == "Departure" && val.ArrivingFrom.Contains(Origin) && val.Airline.Contains(Airline)).OrderBy(val => val.Scheduled);
        ...
        public ActionResult FLIGHTS()
        {
            ...
            clsEncryptAES objcrypt = new clsEncryptAES("@AmdAdaniAirport");

            string userName_ = objcrypt.EncryptText("AMDAirport");
            string Password_ = objcrypt.EncryptText("AMD@AdaniAirport");
            ...
            if (username != "AMDAirport" || password != "AMD@AdaniAirport")
            {
                result = new { status = "3" };
                return Json(result, JsonRequestBehavior.AllowGet);
            }
                ...
                if (OBJ != null)
                {
                    ...
                    try
                    {
                        ...
                        using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                        {
                            ...
                            //if (dbcontext.AMD_AirportFlights.Where(t => t.Scheduled.Value == DateTime.Parse(Schedule.ToString()) && t.FlightNumber.ToString() == fligthNumber).Any())
                            if (dbcontext.AMD_AirportFlights.Where(t => t.OriginDate.Value == DateTime.Parse(Schedule.Value.Date.ToString()) && t.FlightNumber.ToString() == fligthNumber && t.Airline.ToString() == Airline).Any())
                            {

                                //AMD_AirportFlight flight = dbcontext.AMD_AirportFlights.Where(t => t.Scheduled.Value == DateTime.Parse(Schedule.ToString()) && t.FlightNumber.ToString() == fligthNumber).FirstOrDefault();
                                AMD_AirportFlight flight = dbcontext.AMD_AirportFlights.Where(t => t.OriginDate.Value == DateTime.Parse(Schedule.Value.Date.ToString()) && t.FlightNumber.ToString() == fligthNumber && t.Airline.ToString() == Airline).FirstOrDefault();
                            ...
                            {
                                AMD_AirportFlight r1 = new AMD_AirportFlight();
                                ...
                                #region Insert to DB
                                ////
                                dbcontext.AMD_AirportFlights.InsertOnSubmit(r1);
                                // rdb.SubmitChanges();
                                try
                                {
                                    dbcontext.SubmitChanges();
        ...
        [HttpPost]
        public JsonResult GetSearchResult(string itemId, string path)
        {

            var homeItem = Sitecore.Context.Database.GetItem(path);
        ...
        public ActionResult InsertContact(AdaniAirportsContactUsForm m)
        {
            ...
            if (validationStatus == true)
            {
                ...
                try
                {
                    ...
                    AMD_AdaniAirportsContactForm r1 = new AMD_AdaniAirportsContactForm();
                    ...
                    #region Insert to DB
                    rdb.AMD_AdaniAirportsContactForms.InsertOnSubmit(r1);
                    rdb.SubmitChanges();
                    //  Session["validate"] = Request.Cookies["SIDCC"].Value;
                }
                catch (Exception ex)
                ...
                try
                {
                    var msgTpye = Sitecore.Context.Database.GetItem("{8BD2508F-F90E-4B8C-A95C-4DA3472EF581}");
        ...
        public ActionResult BookAirportAssistance(AirportAssistance m)
        {
            ...
            if (validationStatus == true)
            {
                ...
                /* try
                 {
                     ...
                     {
                         if (objcontext.AMD_AirportAssistances.Where(x => x.ContactNumber.ToString() == m.ContactNumber.ToString()).Any())
                         {
                             result = new { status = "3" };
                             return Json(result, JsonRequestBehavior.AllowGet);
                         }
                ...
                if (ModelState.IsValid)
                {
                    ...
                    {
                        TenderDBDataContext rdb = new TenderDBDataContext();
                        AMD_AirportAssistance r1 = new AMD_AirportAssistance();
                        ...
                        #region Insert to DB
                        rdb.AMD_AirportAssistances.InsertOnSubmit(r1);
                        rdb.SubmitChanges();
                        //  Session["validate"] = Request.Cookies["SIDCC"].Value;
                    }
                    catch (Exception ex)
        ...
        private void SendMeetandGreetEmail_Adani(AirportAssistance m)
        {
            ...
            try
            {
                var msgTpye = Sitecore.Context.Database.GetItem("{47CFF0C1-AEB9-4133-98B8-9A3960594AA5}");
                getEmailTo = msgTpye.Fields["EmailTo"].Value;


                var settingsItem = Context.Database.GetItem("{2F5DE391-46B4-4F6C-B324-944143DDDBB9}");
        ...
        private void SendMeetandGreetEmail_customer(AirportAssistance m)
        {
            ...
            try
            {
                var settingsItem = Context.Database.GetItem("{876602BD-A2ED-4E56-8C03-0DA53C0D9BF8}");
                var mailTemplateItem = settingsItem;
                var fromMail = mailTemplateItem.Fields[Templates.MailTemplate.Fields.From];
        ...
        public ActionResult TenderListing()
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                List<AMD_TenderList> tenderdata;
                if (!string.IsNullOrEmpty(TenderType))
                    tenderdata = dbcontext.AMD_TenderLists.Where(x => x.TenderType == TenderType).ToList();
                else
                    tenderdata = dbcontext.AMD_TenderLists.ToList();
                ...
                List<AMD_Corrigendum> Corr = dbcontext.AMD_Corrigendums.Where(x => x.Status == true).ToList();
        ...
        public ActionResult UserRegistration(UserDetails model)
        {
            ...
            if (!currentUrl.Contains("adaniairports.com"))
            {
                listPage = Sitecore.Context.Database.GetItem("{D097315C-7602-4E66-9BF7-0AB2AE5B80C6}");
            }
            ...
            if (validationStatus)
            {
                if (ModelState.IsValid)
                {
                    ...
                    try
                    {
                        ...
                        using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                        {
                            var user = dbcontext.AMD_Registrations.Where(x => x.UniqueId == model.UniqueId && x.TenderId == model.TenderID).FirstOrDefault();
                            if (user == null)
                            {
                                AMD_Registration ObjReg = new AMD_Registration();
                                ...
                                dbcontext.AMD_Registrations.InsertOnSubmit(ObjReg);
                                dbcontext.SubmitChanges();
                            }
                            else
                            {
                        ...
                        if (TempData["Error"] == null)
                        {
                            ...
                            var tenderLoginitem = Sitecore.Context.Database.GetItem(Templates.Tender.TenderLoginOld);
                            if (tenderLoginitem != null)
                            {
                                ...
                                {
                                    var baseU = Sitecore.Context.Database.GetItem("{2968DD51-B06A-4565-9049-0EA22ACC0C7A}").Name;
                                    var baseur = Sitecore.Context.Database.GetItem("{8BA1215D-20A1-4F64-8F6D-B2C638F636AF}").Name;
                                    RedirectUrl = HttpContext.Request.Url.Scheme + "://" + HttpContext.Request.Url.Host + "/" + baseU.Replace(' ', '-') + "/" + baseur.Replace(' ', '-');
                            ...
                            try
                            {
                                if (!string.IsNullOrEmpty(RedirectUrl))
                                {
                                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                                    {
                                        var tender = dbcontext.AMD_TenderLists.Where(x => x.Id == model.TenderID).FirstOrDefault();
        ...
        public ActionResult TenderLogin(Login model)
        {
            ...
            else
            {
                try
                {
                    ...
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        var registerUser = dbcontext.AMD_Registrations.Where(x => x.UserId == model.LoginName && x.Password == model.Password && x.status == true).FirstOrDefault();
                        if (registerUser != null && registerUser.UserType != "SuperAdmin" && registerUser.UserType != "Admin")
                        {
                            DateTime tenderClosingDate = dbcontext.AMD_TenderLists.Where(t => t.Id == registerUser.TenderId).FirstOrDefault().Closing_Date.Value;
                            if (tenderClosingDate < DateTime.Now && registerUser.UserType.ToLower() == "visitor")
                            {
                                ModelState.AddModelError(nameof(model.Password), DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Login tender Error", "This Tender is closed, you won't be able to login now."));
                                return this.View(model);
                            }
                            else if (registerUser.UserType.ToLower() == "visitor" && registerUser.AllowLogin == false)
                            {
                                ModelState.AddModelError(nameof(model.Password), DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Login tender Error", "You won't be able to login now."));
                                return this.View(model);
                            }
                        }
                        ...
                        {
                            var loginhistories = dbcontext.AMD_LoginHistories.Where(x => x.Userid == model.LoginName && x.CreatedDate.GetValueOrDefault().Date == DateTime.Now.Date).Count();
                            if (loginhistories <= 5)
                            {
                                AMD_LoginHistory entity = new AMD_LoginHistory()
                                {
                                    Userid = model.LoginName,
                                    CreatedDate = DateTime.Now.Date
                                };
                                dbcontext.AMD_LoginHistories.InsertOnSubmit(entity);
                                dbcontext.SubmitChanges();
                            }
                            else
                            {
                            ...
                            ModelState.AddModelError(nameof(model.Password), DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Login User Error", "User and password is not valid. Please enter valid credential."));
                            return this.View(model);
                        }
                ...
                catch (System.Exception ex)
                {
                    ...
                    ModelState.AddModelError(nameof(model.Password), DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Login Technical Error", "There is technical problem. Please try after sometime."));
                    return this.View(model);
                }
            }
        }
        ...
        public bool OpenTenderDocPAth(Guid tenderid, string NITPRNo)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                ...
                bool IsCorrigendumPresent = dbcontext.AMD_CorrigendumTenderMappings.Any(x => x.TenderId == tenderid && x.AMD_Corrigendum.Status == true);
        ...
        public ActionResult Logout()
        {
            ...
            if (TenderUserSession.TenderUserSessionContext != null && !string.IsNullOrEmpty(TenderUserSession.TenderUserSessionContext.userId))
            {
                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                {
                    var result = (from user in dbcontext.AMD_Registrations where user.UserId == TenderUserSession.TenderUserSessionContext.userId select user).Single();
        ...
        private string IsUploadingInSequesnce(string envelopeType, string userid)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                if (envelopeType == "2")
                {
                    if (!dbcontext.AMD_UserTenderDocuments.Any(x => x.UserId == userid && x.EnvelopeType == "1"))
                    {
                ...
                {
                    if (!dbcontext.AMD_UserTenderDocuments.Any(x => x.UserId == userid && x.EnvelopeType == "1") || !dbcontext.AMD_UserTenderDocuments.Any(x => x.UserId == userid && x.EnvelopeType == "2"))
                    {
        ...
        public FileResult DownloadUserFile(Guid id)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                var fileToDownload = dbcontext.AMD_UserTenderDocuments.Where(i => i.Id == id).FirstOrDefault();
        ...
        public FileResult DownloadFile(Guid id)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                var fileToDownload = dbcontext.AMD_TenderDocuments.Where(i => i.Id == id).FirstOrDefault();
                return File(fileToDownload.DocData.ToArray(), fileToDownload.ContentType, fileToDownload.FileName);
            }
        }
        ...
        public FileResult DownloadCorrigendumFile(Guid id)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                var fileToDownload = dbcontext.AMD_CorrigendumDocuments.Where(i => i.Id == id).FirstOrDefault();
                return File(fileToDownload.DocData.ToArray(), fileToDownload.ContentType, fileToDownload.FileName);
            }
        }
        ...
        public ActionResult TenderDetails()
        {
            ...
            try
            {
                ...
                if (TenderUserSession.TenderUserSessionContext != null && !string.IsNullOrEmpty(TenderUserSession.TenderUserSessionContext.TenderId))
                {
                    ...
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        var tenderdetails = dbcontext.AMD_TenderLists.Where(x => x.Id == new System.Guid(tenderId)).FirstOrDefault();
                        var userdocument = dbcontext.AMD_UserTenderDocuments.Where(x => x.UserId == UserId).ToList();
                        var userDetail = dbcontext.AMD_Registrations.Where(x => x.UserId == UserId).FirstOrDefault();
                        ...
                        Tender.tenderdocument = tenderdetails.AMD_TenderDocuments.Where(t => t.IsPQDoc != true).ToList();
                        Tender.tenderPQdocument = tenderdetails.AMD_TenderDocuments.Where(t => t.IsPQDoc == true).ToList();
        ...
        public ActionResult TenderDetails(Tender Model, string saveDoc = null, string uploadDoc = null, string changePassword = null)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(uploadDoc) || !string.IsNullOrEmpty(saveDoc))
                {
                    if (Model != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(uploadDoc))
                        {
                            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                            {
                                var user = dbcontext.AMD_Registrations.Where(x => x.UserId == TenderUserSession.TenderUserSessionContext.userId).FirstOrDefault();
                                if (user != null)
                                {
                                    ...
                                    // List<Registration> UserEmailList = dbcontext.AMD_Registrations.Where(x => x.UserType.ToLower() == "admin" || x.UserType.ToLower() == "superadmin" || (x.UserType.ToLower() == "envelopeadmin" && x.TenderId.ToString() == Model.TenderId.ToString())).ToList();
                                    List<AMD_Registration> UserEmailList;
                                    ...
                                    {
                                        UserEmailList = dbcontext.AMD_Registrations.Where(x => (x.UserType.ToLower() == "envelopeadmin" && x.TenderId.ToString() == Model.TenderId.ToString())).ToList();
                                    }
                                    else
                                    {
                                        UserEmailList = dbcontext.AMD_Registrations.Where(x => x.UserType.ToLower() == "admin" || x.UserType.ToLower() == "superadmin" || (x.UserType.ToLower() == "envelopeadmin" && x.TenderId.ToString() == Model.TenderId.ToString())).ToList();
                                    }
                ...
                if (!string.IsNullOrEmpty(changePassword))
                {
                    ...
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        var user = dbcontext.AMD_Registrations.Where(x => x.UserId == TenderUserSession.TenderUserSessionContext.userId && x.Password == Model.OldPassword).FirstOrDefault();
                        if (user != null)
                        {
        ...
        public string saveUserTenderFile(HttpPostedFileBase file, string envelopeType, string userid)
        {
            if (file != null && file.ContentLength > 0)
            {
                ...
                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                {
                    if (dbcontext.AMD_UserTenderDocuments.Any(x => x.UserId == userid && x.EnvelopeType == envelopeType && x.FileName == file.FileName))
                    {
                    ...
                    else
                    {
                        ...
                        AMD_UserTenderDocument dbdoc = new AMD_UserTenderDocument();
                        ...
                        dbcontext.AMD_UserTenderDocuments.InsertOnSubmit(dbdoc);
                        dbcontext.SubmitChanges();
        ...
        public Tender GetTenderDetails()
        {
            ...
            try
            {
                ...
                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                {
                    ...
                    tender.AdvDate = tenderdetails.Adv_Date;
                    tender.ClosingDate = tenderdetails.Closing_Date;
                    //tender.tenderdocument = tenderdetails.AMD_TenderDocuments.ToList();
                    tender.userUpladdocument = userdocument;
                    tender.PQDocumentStatus = userdocument != null && userdocument.Where(d => d.IsPQDoc == true || d.EnvelopeType == "1").Count() > 0 ? "Uploaded" : "Pending";
                    tender.DocumentStatus = userdocument != null && userdocument.Where(d => d.IsPQDoc == false || d.EnvelopeType == "2").Count() > 0 ? "Uploaded" : "Pending";
                    tender.IsPQVerified = userDetail.IsPQVerified == null ? false : userDetail.IsPQVerified;
        ...
        public ActionResult GetTenderDocument(Guid Id)
        {
            try
            {
                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                {
                    List<AMD_TenderDocument> TD = dbcontext.AMD_TenderDocuments.Where(x => x.TenderId == Id).ToList();
        ...
        {
            AhmedabadAirportRepository ElecRepo = new AhmedabadAirportRepository();
        ...
        public ActionResult AdminTenderListing()
        {
            ...
            try
            {
                ...
                List<AMD_TenderList> tenderdata = new List<AMD_TenderList>();
                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                {
                    AMD_TenderList objtender = new AMD_TenderList();
                    if (UserType == "SuperAdmin")
                    {
                        tenderdata = dbcontext.AMD_TenderLists.OrderBy(x => x.Closing_Date).ToList();
                    }
                    else if (UserType == "Admin")
                    {
                        tenderdata = (from tnderlist in dbcontext.AMD_TenderLists orderby tnderlist.Closing_Date ascending where tnderlist.CreatedBy == UserId select tnderlist).ToList();
                    }
                    else // Envelope Admin
                    {
                        tenderdata = (from tdruser in dbcontext.AMD_UserTenderMappings join tnderlist in dbcontext.AMD_TenderLists on tdruser.TenderId equals tnderlist.Id orderby tnderlist.Closing_Date ascending where tdruser.UserId == UserId select tnderlist).ToList();
                    }
        ...
        public ActionResult AdminTenderListing(string Status)
        {
            ...
            try
            {
                ...
                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                {
                    ...
                    if (UserType == "SuperAdmin")
                    {
                        tenderdata = dbcontext.AMD_TenderLists.OrderByDescending(x => x.Created_Date).ToList();
                    }
                    else // Envelope Admin
                    {
                        tenderdata = (from tdruser in dbcontext.AMD_UserTenderMappings join tnderlist in dbcontext.AMD_TenderLists on tdruser.TenderId equals tnderlist.Id where tdruser.UserId == UserId select tnderlist).ToList();
                    }
        ...
        public ActionResult AdminTenderUserListing()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(Request.QueryString["id"]))
                {
                    ...
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        datalist = dbcontext.AMD_Registrations.Where(x => x.TenderId == tenderId && x.status == true && x.UserType.ToLower() == "visitor").Select(s => new RegistedUserHasDocument
                        {
                            Name = s.Name,
                            ...
                            Mobile = s.Mobile,
                            UserId = s.UserId,
                            Description = s.AMD_TenderList.Description,
                            HasDocumentUploaded = dbcontext.AMD_UserTenderDocuments.Where(x => x.UserId == s.UserId && (x.IsPQDoc == false || x.EnvelopeType == "2")).Any() ? "Yes" : "No",
                            HasPQDocumentUploaded = dbcontext.AMD_UserTenderDocuments.Where(x => x.UserId == s.UserId && (x.IsPQDoc == true || x.EnvelopeType == "2")).Any() ? "Yes" : "No",
                            TenderId = s.AMD_TenderList.Id.ToString(),
        ...
        public ActionResult PQVerificationApprove(string uid, Guid tenderid)
        {
            ...
            try
            {
                ...
                if (UserType == "EnvelopeAdmin")
                {
                    ...
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        var tenderDetail = dbcontext.AMD_TenderLists.Where(x => x.Id == tenderid).FirstOrDefault();
                        if (tenderDetail.PQApprovalRequired == true)
                        {
                            var usertoapprove = dbcontext.AMD_Registrations.Where(i => i.UserId == uid).FirstOrDefault();
                            if (usertoapprove.PQEnvelopeReviewStatus == false || usertoapprove.PQEnvelopeReviewStatus == null)
                            {
                                ...
                                List<AMD_Registration> UserEmailList = dbcontext.AMD_Registrations.Where(x => x.UserType.ToLower() == "admin" || x.UserType.ToLower() == "superadmin").ToList();
                ...
                if (adminAproval)
                {
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        ...
                        if (tenderLoginitem != null)
                        {
                            ...
                            var subsiteURL = Sitecore.Context.Database.GetItem("{2968DD51-B06A-4565-9049-0EA22ACC0C7A}").Name;
                            RedirectUrl = HttpContext.Request.Url.Scheme + "://" + HttpContext.Request.Url.Host + "/" + subsiteURL.Replace(' ', '-') + "/" + baseurl.Replace(' ', '-');
                        ...
                        try
                        {
                            if (!string.IsNullOrEmpty(RedirectUrl))
                            {
                                var tenderDetails = dbcontext.AMD_TenderLists.Where(t => t.Id == tenderid).FirstOrDefault();
                                TenderService Tn = new TenderService();
                ...
                if (!string.IsNullOrEmpty(Request.QueryString["id"]))
                {
                    ...
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        datalist = dbcontext.AMD_Registrations.Where(x => x.TenderId == tenderId && x.status == true && x.UserType.ToLower() == "visitor").Select(s => new RegistedUserHasDocument
                        {
                            ...
                            UserId = s.UserId,
                            Description = s.AMD_TenderList.Description,
                            HasDocumentUploaded = dbcontext.AMD_UserTenderDocuments.Where(x => x.UserId == s.UserId).Any() ? "Yes" : "No",
                            TenderId = s.AMD_TenderList.Id.ToString()
                        }).ToList();
                        var TenderNITNo = dbcontext.AMD_TenderLists.Where(x => x.Id == tenderId).FirstOrDefault().NITNo;
        ...
        public ActionResult AdminTenderDetails()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(Request.QueryString["id"]) && !string.IsNullOrEmpty(Request.QueryString["uid"]))
                {
                    ...
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        var tenderdetails = dbcontext.AMD_TenderLists.Where(x => x.Id == tenderId).FirstOrDefault();
                        var userdocument = dbcontext.AMD_UserTenderDocuments.Where(x => x.UserId == tenderuserid).ToList();

                        var userdetails = dbcontext.AMD_Registrations.Where(x => x.TenderId == tenderId && x.UserId == tenderuserid).FirstOrDefault();
                        ...
                        tender.userUpladdocument = userdocument;
                        tender.PQApprovalRequired = tenderdetails.PQApprovalRequired == null ? false : tenderdetails.PQApprovalRequired;
                        ViewBag.Envelopetype = UserType == "SuperAdmin" ? "0" : dbcontext.AMD_UserTenderMappings.Where(x => x.UserId == UserId && x.TenderId == tenderId).FirstOrDefault().Envelope;
        ...
        public ActionResult AdminTenderCreate(TenderCreateModel obj)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    {
                        ModelState.AddModelError("Adv_Date", DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Date Invalid formate", "Please Input Date in dd-MM-yyyy HH:mm:ss format"));
                    ...
                    {
                        ModelState.AddModelError("Closing_Date", DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Date Invalid formate", "Please Input Date in dd-MM-yyyy HH:mm:ss format"));
                        return View(ReturnObj(obj));
                    }
                    if (obj.Files.Count() > 0 && obj.PQFiles.Count() > 0)
                    {
                        ...
                        if (obj.Files[0] == null)
                        {
                            ModelState.AddModelError("Files", DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Date Invalid formate", "Please upload tender document"));
                            err = true;
                        }
                        if (obj.PQFiles[0] == null && obj.PQApprovalRequired == true)
                        {
                            ModelState.AddModelError("PQFiles", DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Date Invalid formate", "Please upload PQ document"));
                            err = true;
                        }
                        if (err)
                        {
                    ...
                    foreach (HttpPostedFileBase file in obj.Files)
                    {
                        if (file != null)
                        {
                            ...
                            if (size > 10240)
                            {
                                ModelState.AddModelError("Files", DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Date Invalid formate", "Tender document size can not exceed then 10 MB"));
                                return View(ReturnObj(obj));
                            }
                        }
                    }
                    ...
                    foreach (HttpPostedFileBase file in obj.PQFiles)
                    {
                        if (file != null)
                        {
                            ...
                            if (size > 10000)
                            {
                                ModelState.AddModelError("PQFiles", DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Date Invalid formate", "PQ document size can not exceed then 10 MB"));
                                return View(ReturnObj(obj));
                            }
                        }
                    }
                    ...
                    if (string.IsNullOrEmpty(obj.EnvelopUser1email) == false || string.IsNullOrEmpty(obj.EnvelopUser1Mobile) == false || string.IsNullOrEmpty(obj.EnvelopUser1Name) == false || obj.EnvelopUser1EnvelopNameCheckboxs.Where(x => x.IsChecked == true).Count() > 0)
                    {
                        ...
                        {
                            ModelState.AddModelError("EnvelopUser1Mobile", DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Date Invalid formate", "All Fields are mandatory for Envelop user."));
                            return View(ReturnObj(obj));
                        }
                    }
                    if (string.IsNullOrEmpty(obj.EnvelopUser2email) == false || string.IsNullOrEmpty(obj.EnvelopUser2Mobile) == false || string.IsNullOrEmpty(obj.EnvelopUser2Name) == false || obj.EnvelopUser2EnvelopNameCheckboxs.Where(x => x.IsChecked == true).Count() > 0)
                    {
                        ...
                        {
                            ModelState.AddModelError("EnvelopUser2Mobile", DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Date Invalid formate", "All Fields are mandatory for Envelop user."));
                            return View(ReturnObj(obj));
                        }
                    }
                    if (string.IsNullOrEmpty(obj.EnvelopUser3email) == false || string.IsNullOrEmpty(obj.EnvelopUser3Mobile) == false || string.IsNullOrEmpty(obj.EnvelopUser3Name) == false)
                    {
                        ...
                        {
                            ModelState.AddModelError("EnvelopUser3Mobile", DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Date Invalid formate", "All Fields are mandatory for Envelop user."));
                            return View(ReturnObj(obj));
                        }
                    }
                    if (string.IsNullOrEmpty(obj.EnvelopUser4email) == false || string.IsNullOrEmpty(obj.EnvelopUser4Mobile) == false || string.IsNullOrEmpty(obj.EnvelopUser4Name) == false)
                    {
                        ...
                        {
                            ModelState.AddModelError("EnvelopUser4Mobile", DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Date Invalid formate", "All Fields are mandatory for Envelop user."));
                            return View(ReturnObj(obj));
                        }
                    }
                    if (string.IsNullOrEmpty(obj.EnvelopUser5email) == false || string.IsNullOrEmpty(obj.EnvelopUser5Mobile) == false || string.IsNullOrEmpty(obj.EnvelopUser5Name) == false)
                    {
                        ...
                        {
                            ModelState.AddModelError("EnvelopUser5Mobile", DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Date Invalid formate", "All Fields are mandatory for Envelop user."));
                            return View(ReturnObj(obj));
                        }
                    }
                    ...
                    if (Envelop1User)
                    {
                        ...
                        using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                        {
                            var tender = dbcontext.AMD_TenderLists.Where(x => x.Id == new Guid(EnpObj.SelectTenderId)).FirstOrDefault();
                            if (!string.IsNullOrEmpty(RedirectUrl))
                            {
                                //Note : Send Mail to User 
                                TenderService Tn = new TenderService();
                    ...
                    ViewBag.SuccessMsg = DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Tender Successful Message", "Tender Uploaded Successfully");
                    ModelState.Clear();
                    return View(ReturnObj(obj));
            ...
            catch (Exception ex)
            {
                ...
                ViewBag.ErrorMsg = DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/AdminTender Exception", "Error in uploading Please try again");
                return View(ReturnObj(obj));
            }
        ...
        public ActionResult AdminTenderUpdate(TenderEditModel obj, string Inactivate_tender = null, string Activate_tender = null)
        {
            ...
            try
            {
                ...
                else if (ModelState.IsValid)
                {
                    ...
                    else
                    {
                        ...
                        ViewBag.Message = DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Tender Successful Message", "Tender Uploaded Successfully");
                        return Redirect(listingitem.Url());
                    }
                }
                return View(obj);
            ...
            catch (Exception ex)
            {
                ...
                ViewBag.EditError = DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/TenderUpdate Exception", "Error in uploading Please try again");
                return View(obj);
            }
        }
        ...
        public ActionResult TenderCorrigendumList()
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                //var CorrigendumIdList = dbcontext.AMD_CorrigendumTenderMappings.Where(x => x.TenderId == TenderId).Select(x => x.Corrigendum).ToList();
                var TenderDetail = dbcontext.AMD_TenderLists.Where(x => x.Id == TenderId).FirstOrDefault();
                ViewBag.TenderDetail = TenderDetail.NITNo + " - " + TenderDetail.Business;

                List<AMD_Corrigendum> Corr = dbcontext.AMD_CorrigendumTenderMappings.Where(x => x.TenderId == TenderId && x.AMD_Corrigendum.Status == true).Select(x => x.AMD_Corrigendum).ToList();
        ...
        public List<TenderDetails> NITRNoFunction(Guid corrigenudmID)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                ...
                var data = dbcontext.AMD_CorrigendumTenderMappings.Where(x => x.CorrigendumId == corrigenudmID).Select(x => x.TenderId).ToList();
                foreach (var item in data)
                {
                    var tendersNITList = dbcontext.AMD_TenderLists.Where(x => x.Id == item).Select(s => new TenderDetails() { NITPRNo = s.NITNo }).ToList();
        ...
        public List<TenderDetails> TenderDocFunction(Guid corrigenudmID)
        {
            ...
            {
                List<TenderDetails> list = new List<TenderDetails>();
                var TenderDocList = dbcontext.AMD_CorrigendumDocuments.Where(x => x.CorrigendumId == corrigenudmID).Select(x => new TenderDetails() { Id = x.Id, FileName = x.FileName }).ToList();
        ...
        public ActionResult CorrigendumCreate(CorrigendumModel obj)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    {
                        ViewBag.selectedRecordNull = DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Tender Selection Validation", "Please Select Tender for Corrigendum List");
                        return View(obj);
                    }
                    ...
                    {
                        ModelState.AddModelError("Date", DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Date Invalid formate", "Please Input Date in dd-MM-yyyy HH:mm:ss format"));
                        return View(obj);
                    }
                    else
                    {
                        ...
                        if (tenderLoginitem != null)
                        {
                            ...
                            var subsiteURL = Sitecore.Context.Database.GetItem("{2968DD51-B06A-4565-9049-0EA22ACC0C7A}").Name;
                            baseurl = HttpContext.Request.Url.Scheme + "://" + HttpContext.Request.Url.Host + "/" + subsiteURL.Replace(' ', '-') + "/" + tenderLoginitem.Url().Replace(' ', '-');
                        }
                        foreach (var ls in selectedRecords)
                        {
                            ...
                            try
                            {
                                ...
                                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                                {
                                    var tender = dbcontext.AMD_TenderLists.Where(x => x.Id == TenderId).FirstOrDefault();
                                    var tenderBidders = dbcontext.AMD_Registrations.Where(x => x.TenderId == TenderId && x.UserType.ToLower() == "visitor").Select(s => s.Email).Distinct().ToList();
                    ...
                    ViewBag.SuccessMsg = DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Corrigendum Success Msg", "Corrigendum Uploaded Successfully");
                    ModelState.Clear();
                    CorrigendumModel model = new CorrigendumModel();
            ...
            catch (Exception ex)
            {
                ...
                ViewBag.ErrorMsg = DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Corrigendum exception msg", "Error in Insert Please try again");
                return View(obj);
            }
        }
        ...
        public ActionResult CorrigenudumUpdate(CorrigedumEditModel obj, string Inactivate_Corrigendum = null, string Activate_Corrigendum = null)
        {
            ...
            try
            {
                ...
                else if (ModelState.IsValid)
                {
                    ...
                    {
                        ViewBag.selectedRecordNull = DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Tender Selection Validation", "Please Select Tender List");
                        return View(obj);
                    }
                    else
                    {
            ...
            catch (Exception ex)
            {
                ...
                ViewBag.EditError = DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Corrigendum exception msg", "Error in Insert Please try again");
                return View(obj);
            }
        }
        ...
        public ActionResult DeleteCorrigendumFile(Guid id, string DocumentPath)
        {
            ...
            catch (Exception ex)
            {
                ...
                //return RedirectToAction("EditCorrigendum", "AhmedabadAirport", new { @id = Edit_Id });
            }
            //  Guid EditId = (Guid)TempData["id"];
            //return RedirectToAction("EditCorrigendum", "AhmedabadAirport", new { @id = EditId });
            return Redirect(updateitem.Url() + "?id=" + TempData["id"]);
        }
        #endregion
            ...
            if (validationStatus)
            {
                ...
                try
                {
                    if (ModelState.IsValid)
                    {
                        ...
                        {
                            ViewBag.SelectTenderId = DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Selection Validation msg", "Please Select Tender From List");
                            return View(ReturnObj);
                        }
                        if (SelectedEnvelope.Count <= 0)
                        {
                            ViewBag.SelectedEnvelope = DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/Envelop selection msg", "Please Select Minimum 1 Envelope");
                            return View(ReturnObj);
                        }
                        ...
                        using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                        {
                            var tender = dbcontext.AMD_TenderLists.Where(x => x.Id == new Guid(obj.SelectTenderId)).FirstOrDefault();
                            if (!string.IsNullOrEmpty(RedirectUrl))
                            {
                                //Note : Send Mail to User 
                                TenderService Tn = new TenderService();
                        ...
                        ViewBag.SuccessMsg = DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/EnvelopUser Success Msg", "User created successfully and an email has been sent with details.");
                        ModelState.Clear();
                        return View(ReturnObj);
                    }
                    else
                ...
                catch (Exception ex)
                {
                    ...
                    ViewBag.ErrorMsg = DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/EnvelopUser Exception Msg", "Error in Insert Please try again");
                    return View(ReturnObj);
                }
            }
            else
        ...
        public ActionResult DisableEnvelopeUser(string id)
        {
            ...
            catch (Exception ex)
            {
                ...
                ViewBag.DisableUserError = DictionaryPhraseRepository.Current.Get("/AhmedabadAirport/Tender/DisableEnvelopeUser Exception Msg", "There is Error in Disable Users");
                return View();
            }
        ...
        public ActionResult AirportSurveyFeedback(AirportSurveyFeedback airportSurvey)
        {
            ...
            if (validationStatus == true)
            {
                ...
                {
                    TenderDBDataContext rdb = new TenderDBDataContext();
                    AMD_AirportSurveyFeedback ASF = new AMD_AirportSurveyFeedback();
                    ASF.Id = rdb.AMD_AirportSurveyFeedbacks.Select(x => x.Id).Max() + 1;
                    ...
                    #region Insert to DB
                    rdb.AMD_AirportSurveyFeedbacks.InsertOnSubmit(ASF);
                    rdb.SubmitChanges();
        ...
        [HttpGet]
        public ActionResult AhmedabadDepartureFlighSearchDetails(string Origin, string Airline, string DeparturFlightsDate, string FlightType, string SearchFlights)
        {
            DateTime filterdate = DateTime.Now;
            if (!String.IsNullOrEmpty(DeparturFlightsDate) && DeparturFlightsDate != "All")
            {
                filterdate = DateTime.Parse(DeparturFlightsDate);
            }
            ...
            {
                AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind == "Departure" && val.Airline.StartsWith(SearchFlights) || val.FlightNumber.StartsWith(SearchFlights)).OrderBy(val => val.Scheduled);
            }
            else
            {
                if (Origin == "All" && Airline == "All" && DeparturFlightsDate == "All")
                {
                    AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date >= filterdate.Date && val.flightkind == "Departure").OrderBy(val => val.Scheduled);
                }
                if (Origin == "All" && Airline == "All" && DeparturFlightsDate == "All" && FlightType == "All")
                {
                    AhmedabadFlightInformation = AhmedabadAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date >= filterdate.Date && val.flightkind == "Departure").OrderBy(val => val.Scheduled);
                }
                if (Origin == "All" && Airline == "All" && DeparturFlightsDate != "All" && FlightType == "All")
        ...
        //Contact Us Form
        [HttpPost]
        public ActionResult AirportContactUs(AmdAirportContactUs airportContactUs)
        {
            bool isValid = true;
            bool validationStatus = true;
            var result = new { status = "1" };
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    string securitytoken = DictionaryPhraseRepository.Current.Get("/AMDContactFormCRM/securitytoken", "");
                    string CRMAPI = DictionaryPhraseRepository.Current.Get("/AMDContactFormCRM/API", "");
                    Log.Info("CRM Start  AMD Contact Us form", this);
                    ...
                    Log.Info("CRM END  AMD Contact Us form", this);


                    Log.Info("Insert data into AMD Contact Us DB ", this);
                    TenderDBDataContext rdb = new TenderDBDataContext();
                    AMD_AirportContactUsForm contactUsForm = new AMD_AirportContactUsForm();
                    contactUsForm.Id = (rdb.AMD_AirportContactUsForms.Select(i => (long?)i.Id).Max() ?? 0) + 1;
                    ...
                    #region Insert to DB
                    rdb.AMD_AirportContactUsForms.InsertOnSubmit(contactUsForm);
                    rdb.SubmitChanges();
                    Log.Info("data inserted in  DB  sucessfully  AMD Contact Us form", this);
                ...
                {
                    result = new { status = "0" };
                    Log.Info("Exception thrown in AirportContactUs:" + ex.StackTrace, ex);
                    Console.WriteLine(ex);
                }
            }
            else
