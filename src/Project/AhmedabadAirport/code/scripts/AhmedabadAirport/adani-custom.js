...
$("#DateArrival").on("change", function () {
    ...
    $.ajax({
        type: "Get",
        data: { ArrivalTime: ArrivalFlights },
        url: "/api/AhmedabadAirport/AhmedabadFlightDetailsArrivalUpdate",
...
$('span#submit').click(function () {
    ...
    if (confirm("Do you really want to delete this User")) {
        $.ajax({
            type: 'POST',
            data: { id: Id },
            url: "/api/AhmedabadAirport/DisableEnvelopeUser",
            success: function (data) {
                location.reload();
...
$('span#submits').click(function () {
    ...
    if (confirm("Do you really want to delete this User")) {
        $.ajax({
            type: 'POST',
            data: { id: Id },
            url: "/api/AhmedabadAirport/DisableAdminUser",
            success: function (data) {
                location.reload();
...
/*$("#DateDeparture").on("change", function () {
    ...
    $.ajax({
        type: "Get",
        data: { DepartureFlight: DepartureFlights },
        url: "/api/AhmedabadAirport/AhmedabadFlightDetailsUpdateDeparture",
        contentType: "application/json",
        success: function (data) {
...
$("#btnSearch").click(function () {
    ...
    $.ajax({
        ...
        data: {
            ...
            Airline: $('#AirlineArrival').val(),
            Origin: $('#OriginArrival').val(),
            FlightType: $('#FlightType').val(),
            ArrivalFlights: ArrivalFlights
        },
        url: "/api/AhmedabadAirport/AhmedabadFlighSearchDetailsArival",
        contentType: "application/json",
        success: function (data) {
...
$("#btnSearchDeparture").click(function () {
    ...
    $.ajax({
        ...
        data: {
            ...
            Airline: $('#AirlineDeparture').val(),
            Origin: $('#OriginDeparture').val(),
            FlightType: $('#FlightTypes').val(),
            DepartureFlights: DepartureFlights
        },
        url: "/api/AhmedabadAirport/AhmedabadFlighSearchDetailsDeparture",
        contentType: "application/json",
        success: function (data) {
            $("#btnSearchDeparture").removeAttr("disabled");
            $('#departure2').html(data);
        },
        error: function () {
            alert("Failed! Please try again.");
...
$(document).ready(function () {
    ...
    if (window.location.href.indexOf('/PranaamAdaniGuestServices') > 0) {
        var d = new Date();
...
//    jQuery.ajax(
//        {
//            url: "/api/ahmedabadAirport/GetTenderDocument",
//            method: "GET",
//            data: {
//                Id: id
//            },
...
function loadWeather() {
    var url = "https://api.weatherapi.com/v1/current.json?key=e6f018f1071b4a2c9c392224212408&q=ahmedabad&aqi=no";
    $.ajax({
        url: url,
            ...
            if (data != null) {
                if (data.current != null) {
                    if (data.current.temp_c != null) {
                        ...
                        $(".weather-plugin").html('<a ><span id="tempInfo"><span>' + data.current.temp_c + '&#8451;</span></span><img width="35px" src="' + data.current.condition.icon + '"></a>');
...
$("#BtnAPASubmit").click(function () {
    ...
    if (MobileNumber.length != 10) { alert("Please enter correct Mobile Number"); $("#MobileNumber").focus(); $('#BtnAPASubmit').removeAttr("disabled"); return false; }
    ...
    */
    ...
    $.ajax({
        ...
        url: "/api/AhmedabadAirport/BookAirportAssistance",
        contentType: "application/json",
        success: function (data) {
            //////////////

            if (data.status == "1") {
                window.location.href = "https://stage.adaniuat.com/svpia-ahmedabad-airport/Assistance-thankyou";
...
$("#BtnASFSubmit").click(function () {
    ...
    $('#BtnASFSubmit').attr("disabled", "disabled");
    ...
    if (Travel == "Business Traveller") { var TravelPurpose = document.getElementById("TravelPurpose").value; }
    else if (Travel == "Leisure Traveller") { var TravelPurpose = document.getElementById("TravelPurpose1").value; }
    ...
    $.ajax({
        type: "POST",
        data: JSON.stringify(model),
        url: "/api/AhmedabadAirport/AirportSurveyFeedback",
        contentType: "application/json",
        success: function (data) {
            //////////////

            if (data.status == "1") {
                //alert("Thank you for your Valuable Feedback. By completing the survey, you may be contacted by the airport to help improve the quality of service provided. Your personal details will not be used for promotional messages");
                window.location.href = "https://stage.adaniuat.com/svpia-ahmedabad-airport/Assistance-thankyou";
                //$('#contact_form1').submit();
            }
            else if (data.status == "3") {
...
$("#Message").blur(function () {
    var MessageRegex = /^[A-Za-z0-9;,.?_\-!@&:""'\/\\ ]{0,500}$/g;
    ...
    grecaptcha.ready(function () {
        grecaptcha.execute('6LdoDj4hAAAAAD7gqiPJEsAy96GJBkiNRu9qohiz', { action: 'AirportContactUs' }).then(function (token) {
            ...
            $.ajax({
                ...
                url: "/api/AhmedabadAirport/AirportContactUs",
                contentType: "application/json",
                success: function (data) {
                    if (data.status == "1") {
                        window.location.href = "/svpia-ahmedabad-airport/thank-you";
                    }
