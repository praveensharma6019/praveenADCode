...
namespace Sitecore.AhmedabadAirport.Website.Services
{
    public class CorrigendumRepository
    {
        ...
        public void ActivateCorrigendum(Guid id)
        {
            ...
            {
                AMD_Corrigendum corrigendumItem = (
                    from c in dbcontext.AMD_Corrigendums
                    where c.Id == id
                    select c).Single<AMD_Corrigendum>();
        ...
        public void DeleteCorrigendumDocument(Guid id, string DocumentPath)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                AMD_CorrigendumDocument doc = (
                    from x in dbcontext.AMD_CorrigendumDocuments
                    where x.Id == id
                    select x).Single<AMD_CorrigendumDocument>();
                dbcontext.AMD_CorrigendumDocuments.DeleteOnSubmit(doc);
                try
                {
                    dbcontext.SubmitChanges();
                }
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                if (UserType != "Admin")
                {
                    IQueryable<CorrigendumModel> list =
                        from r in dbcontext.AMD_Corrigendums
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                IQueryable<CorrigedumEditModel> list =
                    ...
                    select new CorrigedumEditModel()
                    {
                        ...
                        CorrigendumDocument = r.AMD_CorrigendumDocuments.Where<AMD_CorrigendumDocument>((AMD_CorrigendumDocument x) => x.CorrigendumId == (Guid?)r.Id).ToList<AMD_CorrigendumDocument>(),
                        CorrigendumTenderMapping = r.AMD_CorrigendumTenderMappings.Where<AMD_CorrigendumTenderMapping>((AMD_CorrigendumTenderMapping x) => x.CorrigendumId == (Guid?)r.Id).ToList<AMD_CorrigendumTenderMapping>()
                    };
        ...
        public Guid InsertCorrigendum(CorrigendumModel obj)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                ...
                AMD_Corrigendum list = new AMD_Corrigendum()
                {
                    Id = Guid.NewGuid(),
                    Title = obj.Title,
                ...
                dbcontext.AMD_Corrigendums.InsertOnSubmit(list);
                dbcontext.SubmitChanges();
                id = list.Id;
            }
            return id;
        ...
        public void InsertCorrigendumDocument(CorrigendumModel obj)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                AMD_CorrigendumDocument aMDCorrigendumDocument = new AMD_CorrigendumDocument()
                {
                    Id = Guid.NewGuid(),
                ...
                dbcontext.AMD_CorrigendumDocuments.InsertOnSubmit(aMDCorrigendumDocument);
                dbcontext.SubmitChanges();
            }
        }
        ...
        public void InsertCorrigendumTenderMapping(Guid tenderId, Guid corrigId)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                AMD_CorrigendumTenderMapping aMDCorrigendumTenderMapping = new AMD_CorrigendumTenderMapping()
                {
                    Id = Guid.NewGuid(),
                ...
                dbcontext.AMD_CorrigendumTenderMappings.InsertOnSubmit(aMDCorrigendumTenderMapping);
                dbcontext.SubmitChanges();
            }
        }
        ...
        public void UpdateCorrigendumMapping(CorrigedumEditModel obj, List<Check> selectedRecords)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                (
                    ...
                    select x).ToList<AMD_CorrigendumTenderMapping>();
                IQueryable<AMD_CorrigendumTenderMapping> del =
                    from x in dbcontext.AMD_CorrigendumTenderMappings
                    where x.CorrigendumId == (Guid?)obj.Id
                    select x;
                dbcontext.AMD_CorrigendumTenderMappings.DeleteAllOnSubmit<AMD_CorrigendumTenderMapping>(del);
                dbcontext.SubmitChanges();
