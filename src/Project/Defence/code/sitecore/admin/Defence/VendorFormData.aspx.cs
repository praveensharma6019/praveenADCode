...
namespace Sitecore.Defence.Website.sitecore.admin.Defence
{
    public partial class VendorFormData : System.Web.UI.Page
    {
        ...
        {
            VendorEnrollmentFormDataContext rdb = new VendorEnrollmentFormDataContext();
            try
            {
                lblErroMsg.Text = "";
                ...
                var datarecord = from rc in rdb.DefenceVendorEnrollmentForms
                                 where ((rc.SubmitOnDate.Value.Date >= fromdate) && (rc.SubmitOnDate.Value.Date <= todate))
                                 select rc;
                

                if (System.Convert.ToDateTime(TextBoxTo.Text) == System.Convert.ToDateTime(TextBoxFrom.Text))
                {
                    datarecord = from rc in rdb.DefenceVendorEnrollmentForms
                                 where rc.SubmitOnDate.Value.Date == fromdate
                                 select rc;                    
                }
                DataTable dt = ToDataTable(datarecord.ToList());
                string savefilepath = Server.MapPath("~/sitecore/admin/Defence/SampleFile/Form-Records.csv");
