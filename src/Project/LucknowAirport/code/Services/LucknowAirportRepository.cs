...
{
    public class LucknowAirportRepository
    {
        public LucknowAirportRepository()
        {
        }
        ...
        public void ActivateTender(Guid id)
        {
            ...
            {
                LKO_TenderList tenderlist = (
                    from x in dbcontext.LKO_TenderLists
                    where x.Id == id
                    select x).Single<LKO_TenderList>();
        ...
        public void DeleteTenderDocument(Guid id, string DocumentPath)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                LKO_TenderDocument docFile = (
                    from x in dbcontext.LKO_TenderDocuments
                    where x.Id == id
                    select x).Single<LKO_TenderDocument>();
                dbcontext.LKO_TenderDocuments.DeleteOnSubmit(docFile);
                try
                {
                    dbcontext.SubmitChanges();
                }
        ...
        public int DeleteUserTenderDocument(Guid id, string userId, string tenderId)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                LKO_UserTenderDocument docFile = (
                    from x in dbcontext.LKO_UserTenderDocuments
                    where x.Id == id
                    select x).Single<LKO_UserTenderDocument>();
                ...
                {
                    dbcontext.LKO_UserTenderDocuments.DeleteOnSubmit(docFile);
                    try
                    {
                        dbcontext.SubmitChanges();
                        num = 0;
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                IQueryable<TenderEditModel> list =
                    ...
                    select new TenderEditModel()
                    {
                        ...
                        Location = r.Location,
                        Status = r.Staus,
                        TenderDocuments = r.LKO_TenderDocuments.Where<LKO_TenderDocument>((LKO_TenderDocument x) => (x.TenderId == (Guid?)r.Id) && x.IsPQDoc == (bool?)false).ToList<LKO_TenderDocument>(),
                        PQDocuments = r.LKO_TenderDocuments.Where<LKO_TenderDocument>((LKO_TenderDocument x) => (x.TenderId == (Guid?)r.Id) && x.IsPQDoc == (bool?)true).ToList<LKO_TenderDocument>()
                    };
        ...
        public void InsertDocumentList(TenderCreateModel obj)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                ...
                {
                    LKO_TenderDocument lKOTenderDocument = new LKO_TenderDocument()
                    {
                        Id = Guid.NewGuid(),
                    ...
                    dbcontext.LKO_TenderDocuments.InsertOnSubmit(lKOTenderDocument);
                }
                dbcontext.SubmitChanges();
            }
        }
        ...
        public Guid InsertTenderList(TenderCreateModel obj)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                LKO_TenderList Tlist = new LKO_TenderList()
                {
                    Id = Guid.NewGuid(),
                    NITNo = obj.NITNo,
                ...
                dbcontext.LKO_TenderLists.InsertOnSubmit(Tlist);
                dbcontext.SubmitChanges();
                id = Tlist.Id;
            }
            return id;
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                ...
                for (int j = 0; j < (int)pQFiles.Length; j++)
                {
                    ...
                    if (file != null && this.CheckExtension(file.FileName))
                    {
                        ...
                        Path.GetFileNameWithoutExtension(file.FileName);
                        Path.GetExtension(file.FileName);
                        LKO_TenderDocument lKOTenderDocument1 = new LKO_TenderDocument()
                        {
                            Id = Guid.NewGuid(),
                            TenderId = new Guid?(tenderlist.Id),
                            FileName = file.FileName,
                        ...
                        dbcontext.LKO_TenderDocuments.InsertOnSubmit(lKOTenderDocument1);
                        dbcontext.SubmitChanges();
                    }
                }
            }
