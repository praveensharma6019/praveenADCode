...
namespace Sitecore.LucknowAirport.Website.Services
{
    public class CorrigendumRepository
    {
        ...
        public void ActivateCorrigendum(Guid id)
        {
            ...
            {
                LKO_Corrigendum corrigendumItem = (
                    from c in dbcontext.LKO_Corrigendums
                    where c.Id == id
                    select c).Single<LKO_Corrigendum>();
        ...
        public void DeleteCorrigendumDocument(Guid id, string DocumentPath)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                LKO_CorrigendumDocument doc = (
                    from x in dbcontext.LKO_CorrigendumDocuments
                    where x.Id == id
                    select x).Single<LKO_CorrigendumDocument>();
                dbcontext.LKO_CorrigendumDocuments.DeleteOnSubmit(doc);
                try
                {
                    dbcontext.SubmitChanges();
                }
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                if (UserType != "Admin")
                {
                    IQueryable<CorrigendumModel> list =
                        from r in dbcontext.LKO_Corrigendums
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                IQueryable<CorrigedumEditModel> list =
                    ...
                    select new CorrigedumEditModel()
                    {
                        ...
                        CorrigendumDocument = r.LKO_CorrigendumDocuments.Where<LKO_CorrigendumDocument>((LKO_CorrigendumDocument x) => x.CorrigendumId == (Guid?)r.Id).ToList<LKO_CorrigendumDocument>(),
                        CorrigendumTenderMapping = r.LKO_CorrigendumTenderMappings.Where<LKO_CorrigendumTenderMapping>((LKO_CorrigendumTenderMapping x) => x.CorrigendumId == (Guid?)r.Id).ToList<LKO_CorrigendumTenderMapping>()
                    };
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                if (userType != "Admin")
                {
                    IQueryable<Check> list =
                        from r in dbcontext.LKO_TenderLists
                        select new Check()
                        {
                            Id = r.Id,
                            NITNo = r.NITNo,
        ...
        public Guid InsertCorrigendum(CorrigendumModel obj)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                ...
                LKO_Corrigendum list = new LKO_Corrigendum()
                {
                    Id = Guid.NewGuid(),
                    Title = obj.Title,
                ...
                dbcontext.LKO_Corrigendums.InsertOnSubmit(list);
                dbcontext.SubmitChanges();
                id = list.Id;
            }
            return id;
        ...
        public void InsertCorrigendumDocument(CorrigendumModel obj)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                LKO_CorrigendumDocument lKOCorrigendumDocument = new LKO_CorrigendumDocument()
                {
                    Id = Guid.NewGuid(),
                ...
                dbcontext.LKO_CorrigendumDocuments.InsertOnSubmit(lKOCorrigendumDocument);
                dbcontext.SubmitChanges();
            }
        }
        ...
        public void InsertCorrigendumTenderMapping(Guid tenderId, Guid corrigId)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                LKO_CorrigendumTenderMapping lKOCorrigendumTenderMapping = new LKO_CorrigendumTenderMapping()
                {
                    Id = Guid.NewGuid(),
                ...
                dbcontext.LKO_CorrigendumTenderMappings.InsertOnSubmit(lKOCorrigendumTenderMapping);
                dbcontext.SubmitChanges();
            }
        }
        ...
        public List<TenderDetails> NITRNCorrigendum(Guid corrigenudmID)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                ...
                List<Guid?> data = (
                    from x in dbcontext.LKO_CorrigendumTenderMappings
                ...
                foreach (Guid? nullable in data)
                {
                    List<TenderDetails> tendersNITList = (
                        from x in dbcontext.LKO_TenderLists
        ...
        public void UpdateCorrigendumMapping(CorrigedumEditModel obj, List<Check> selectedRecords)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                (
                    ...
                    select x).ToList<LKO_CorrigendumTenderMapping>();
                IQueryable<LKO_CorrigendumTenderMapping> del =
                    from x in dbcontext.LKO_CorrigendumTenderMappings
                    where x.CorrigendumId == (Guid?)obj.Id
                    select x;
                dbcontext.LKO_CorrigendumTenderMappings.DeleteAllOnSubmit<LKO_CorrigendumTenderMapping>(del);
                dbcontext.SubmitChanges();
        ...
        public void UpdateCorrrigendum(CorrigedumEditModel obj)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                LKO_Corrigendum CorrigenList = (
                    from x in dbcontext.LKO_Corrigendums
                    where x.Id == obj.Id
                    select x).Single<LKO_Corrigendum>();
