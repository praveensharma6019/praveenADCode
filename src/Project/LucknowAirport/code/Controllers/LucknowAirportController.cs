...
using Sitecore.LucknowAirport.Website.Services;
using Sitecore.LucknowAirport.Website.Utility;
...
using Sitecore.LucknowAirport.Website.Models;
using System.Data.Linq;
using System.Text.RegularExpressions;
using System.Net.Http;
using System.Net.Http.Headers;

namespace Sitecore.LucknowAirport.Website.Controllers
{
    public class LucknowAirportController : Controller
    {
        ...
        // GET: Lucknow Airport Contact Form
        public ActionResult Index()
        {
            return View();
        }

        TenderDBDataContext lkodb = new TenderDBDataContext();
        [HttpGet]
        public ActionResult LkoFlightDetails(string ArrivalTime)
        {
            ...
            LucknowAirportFlightModel lucknowairportmodel = new LucknowAirportFlightModel();
            List<LucknowFlight> LucknowFlights = new List<LucknowFlight>();
            Table<LKO_AirportFlight> LucknowAirportFlightDetail = this.lkodb.LKO_AirportFlights;
            var StatusList = Sitecore.Context.Database.GetItem("{C4F365B2-1863-4B6C-9DBB-CA58319DFD47}");
            IQueryable<LKO_AirportFlight> LucknowFlightInformation = LucknowAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date).OrderBy(val => val.Scheduled);
            
                foreach (LKO_AirportFlight LucknowAirportFlightData in LucknowFlightInformation)
                {

                    DateTime? schDate = LucknowAirportFlightData.ScheduledTime;

                    var schTime = schDate.Value.ToShortTimeString();
                    var schTimeFormat = DateTime.Parse(schTime).ToString("HH:mm");

                    LucknowFlight lucknowflights = new LucknowFlight()

                    {
                        Scheduled = schTimeFormat,
                        FlightNumber = LucknowAirportFlightData.FlightNumber.ToString(),
                        ArrivingFrom = LucknowAirportFlightData.ArrivingFrom.ToString(),
                        DepartureFrom = LucknowAirportFlightData.DepartureFrom.ToString(),
                        Airline = LucknowAirportFlightData.Airline,
                        terminal = LucknowAirportFlightData.Terminal,
                        Flightkind = LucknowAirportFlightData.flightkind,
                    };
                    if (StatusList != null)
                    {
                        if (StatusList.HasChildren)
                        {
                            ...
                            {
                                if (LucknowAirportFlightData.Status == CodeName["CodeValue"])
                                {
                                    var remarkdescription = CodeName["RemarkDescription"];
                                    LucknowAirportFlightData.Status = remarkdescription;
                                    lucknowflights.Status = LucknowAirportFlightData.Status;
                                }
                                else
                                {
                                    lucknowflights.Status = LucknowAirportFlightData.Status;
                    ...
                    if (LucknowAirportFlightData.EstimationTime != null)
                    {
                        DateTime? estimateDate = LucknowAirportFlightData.EstimationTime;
                        var estimate = estimateDate.Value.ToShortTimeString();
                        var estimateFormat = DateTime.Parse(estimate).ToString("HH:mm");
                        lucknowflights.EstimationTime = estimateFormat;
                    }

                    LucknowFlights.Add(lucknowflights);
                }
          
           


            lucknowairportmodel.LucknowAirportList = LucknowFlights;


            return base.View(lucknowairportmodel.LucknowAirportList.ToList<LucknowFlight>());
        }


        [HttpGet]
        public ActionResult LucknowFlightDetailsArrivalUpdate(string ArrivalTime)
        {
            DateTime filterdate = DateTime.Now;
            if (ArrivalTime != null || ArrivalTime == "")
            {
            ...
            Table<LKO_AirportFlight> LucknowAirportFlightDetail = this.lkodb.LKO_AirportFlights;
            var StatusList = Sitecore.Context.Database.GetItem("{C4F365B2-1863-4B6C-9DBB-CA58319DFD47}");
            IQueryable<LKO_AirportFlight> LucknowFlightInformation = LucknowAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind== "Arrival").OrderBy(val => val.Scheduled);
            if (LucknowFlightInformation.Count() >= 1)
            {
                foreach (LKO_AirportFlight LucknowAirportFlightData in LucknowFlightInformation)
            {
            ...
            return base.View("/views/LucknowAirport/LucknowFlightDetailsArrivalUpdate.cshtml", lucknowairportmodel.LucknowAirportList.ToList<LucknowFlight>());
        }

        [HttpGet]
        public ActionResult LucknowFlightDetailsUpdateDeparture(string DepartureFlight)
        {
            DateTime filterdate = DateTime.Now;
           ...
           LucknowAirportFlightModel lucknowairportmodel = new LucknowAirportFlightModel();
            ...
            Table<LKO_AirportFlight> LucknowAirportFlightDetail = this.lkodb.LKO_AirportFlights;
            var StatusList = Sitecore.Context.Database.GetItem("{C4F365B2-1863-4B6C-9DBB-CA58319DFD47}");
            IQueryable<LKO_AirportFlight> LucknowFlightInformation = LucknowAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind== "Departure").OrderBy(val => val.Scheduled);
            if (LucknowFlightInformation.Count() >= 1)
            {
                foreach (LKO_AirportFlight LucknowAirportFlightData in LucknowFlightInformation)
            {
            ...
            return base.View("/views/LucknowAirport/LucknowFlightDetailsUpdateDeparture.cshtml", lucknowairportmodel.LucknowAirportList.ToList<LucknowFlight>());
        }


        [HttpGet]
        public ActionResult LucknowFlighSearchDetailsArival(string Search, string ArrivalFlights)
        {
            DateTime filterdate = DateTime.Now;
            ...
            List<LucknowFlight> LucknowFlights = new List<LucknowFlight>();
            Table<LKO_AirportFlight> LucknowAirportFlightDetail = this.lkodb.LKO_AirportFlights;
            IQueryable<LKO_AirportFlight> LucknowFlightInformation = LucknowAirportFlightDetail.Where(val => val.SpecialAction != "Delete" && val.ServiceType == "J" && val.ScheduledTime != null && val.ScheduledTime.Value.Date == filterdate.Date && val.flightkind== "Arrival" && (val.Airline.StartsWith(Search) || val.FlightNumber.StartsWith(Search))).OrderBy(val => val.Scheduled);
            if (LucknowFlightInformation.Count() >= 1)
            {
                foreach (LKO_AirportFlight LucknowAirportFlightData in LucknowFlightInformation)
            {
        ...
        [HttpGet]
        public ActionResult LucknowFlighSearchDetailsDeparture(string Search, string DepartureFlights)
        {
            DateTime filterdate = DateTime.Now;
        ...
        public ActionResult FLIGHTS()
        {
            ...
            clsEncryptAES objcrypt = new clsEncryptAES("@LkoAdaniAirport");

            string userName_ = objcrypt.EncryptText("LKOAirport");
            string Password_ = objcrypt.EncryptText("LKO@AdaniAirport");
            ...
            if (username != "LKOAirport" || password != "LKO@AdaniAirport")
            {
                result = new { status = "3" };
                return Json(result, JsonRequestBehavior.AllowGet);
            }
            ...
            try
            {
                ...
                if (OBJ != null)
                {
                    ...
                    try
                    {
                        ...
                        {
                            //if (dbcontext.LKO_AirportFlights.Where(t => t.Scheduled.Value == DateTime.Parse(Schedule.ToString()) && t.FlightNumber.ToString() == fligthNumber).Any())
                            if (dbcontext.LKO_AirportFlights.Where(t => t.OriginDate.Value == DateTime.Parse(Schedule.Value.Date.ToString()) && t.FlightNumber.ToString() == fligthNumber && t.Airline.ToString() == Airline).Any())
                            {
                                // LKO_AirportFlight flight = dbcontext.LKO_AirportFlights.Where(t => t.Scheduled.Value == DateTime.Parse(Schedule.ToString()) && t.FlightNumber.ToString() == fligthNumber).FirstOrDefault();
                                LKO_AirportFlight flight = dbcontext.LKO_AirportFlights.Where(t => t.OriginDate.Value == DateTime.Parse(Schedule.Value.Date.ToString()) && t.FlightNumber.ToString() == fligthNumber && t.Airline.ToString() == Airline).FirstOrDefault();
                            ...
                            {
                           LKO_AirportFlight r1 = new LKO_AirportFlight();
                                ...
                                #region Insert to DB
                                ////
                                dbcontext.LKO_AirportFlights.InsertOnSubmit(r1);
                                // rdb.SubmitChanges();
                                try
                                {
                                    dbcontext.SubmitChanges();
        ...
        public ActionResult InsertContact(AdaniAirportsContactUsForm m)
        {
            ...
            if (validationStatus == true)
            {
                ...
                try
                {
                    ...
                    LKO_AdaniAirportsContactForm r1 = new LKO_AdaniAirportsContactForm();
                    ...
                    #region Insert to DB
                    rdb.LKO_AdaniAirportsContactForms.InsertOnSubmit(r1);
                    rdb.SubmitChanges();
                    //  Session["validate"] = Request.Cookies["SIDCC"].Value;
                }
                catch (Exception ex)
                ...
                try
                {
                    var msgTpye = Sitecore.Context.Database.GetItem("{93E3516A-B3E4-4407-8202-88C89A9B1A5B}");
        ...
        public ActionResult BookAirportAssistance(AirportAssistance m)
        {
            ...
            if (validationStatus == true)
            {
                ...
                /*try
                {
                    ...
                    {
                        if (objcontext.LKO_AirportAssistances.Where(x => x.ContactNumber.ToString() == m.ContactNumber.ToString()).Any())
                        {
                            result = new { status = "3" };
                            return Json(result, JsonRequestBehavior.AllowGet);
                        }
                ...
                if (ModelState.IsValid)
                {
                    ...
                    {
                        TenderDBDataContext rdb = new TenderDBDataContext();
                        LKO_AirportAssistance r1 = new LKO_AirportAssistance();
                        ...
                        #region Insert to DB
                        rdb.LKO_AirportAssistances.InsertOnSubmit(r1);
                        rdb.SubmitChanges();
                        //  Session["validate"] = Request.Cookies["SIDCC"].Value;
                    }
                    catch (Exception ex)
        ...
        private void SendMeetandGreetEmail_customer(AirportAssistance m)
        {
            ...
            try
            {
                getEmailTo = m.EmailAddress;


                var settingsItem = Context.Database.GetItem("{B443DF1E-3A1B-48DA-9BC3-44F10DC446F6}");
        ...
        private void SendMeetandGreetEmail_Adani(AirportAssistance m)
        {
            ...
            try
            {
                var msgTpye = Sitecore.Context.Database.GetItem("{34FB93DE-0FDD-41E9-8E67-54BB0E376726}");
                getEmailTo = msgTpye.Fields["EmailTo"].Value;


                var settingsItem = Context.Database.GetItem("{37B13A70-1D30-473C-A76D-6E5D5F1C2F2D}");
                var mailTemplateItem = settingsItem;
                var fromMail = mailTemplateItem.Fields[Templates.MailTemplate.Fields.From];
        ...
        public ActionResult TenderListing()
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                List<LKO_TenderList> tenderdata;
                if (!string.IsNullOrEmpty(TenderType))
                    tenderdata = dbcontext.LKO_TenderLists.Where(x => x.TenderType == TenderType).ToList();
                else
                    tenderdata = dbcontext.LKO_TenderLists.ToList();
                ...
                List<LKO_Corrigendum> Corr = dbcontext.LKO_Corrigendums.Where(x => x.Status == true).ToList();
        ...
        public ActionResult UserRegistration(UserDetails model)
        {
            ...
            {
                listPage = Sitecore.Context.Database.GetItem("{CDC9F5E7-44A8-4DBC-851E-5B67FB479EF1}");
            }
            // var ccsiaPage = Sitecore.Context.Database.GetItem("{CDC9F5E7-44A8-4DBC-851E-5B67FB479EF1}");
            ...
            if (validationStatus)
            {
                if (ModelState.IsValid)
                {
                    ...
                    try
                    {
                        ...
                        using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                        {
                            var user = dbcontext.LKO_Registrations.Where(x => x.UniqueId == model.UniqueId && x.TenderId == model.TenderID).FirstOrDefault();
                            if (user == null)
                            {
                                LKO_Registration ObjReg = new LKO_Registration();
                                ...
                                dbcontext.LKO_Registrations.InsertOnSubmit(ObjReg);
                                dbcontext.SubmitChanges();
                            }
                            else
                            {
                        ...
                        if (TempData["Error"] == null)
                        {
                            ...
                            if (tenderLoginitem != null)
                            {
                                ...
                                {
                                    var baseU = Sitecore.Context.Database.GetItem("{FCE658C4-0A89-4929-8FE1-06A2EB33B4CF}").Name;
                                    var baseur = Sitecore.Context.Database.GetItem("{A2E4458F-CEB7-4780-B568-B8A0C3AA2FA3}").Name;
                            ...
                            try
                            {
                                if (!string.IsNullOrEmpty(RedirectUrl))
                                {
                                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                                    {
                                        var tender = dbcontext.LKO_TenderLists.Where(x => x.Id == model.TenderID).FirstOrDefault();
            ...
            // if (currentUrl.Contains("adaniairports"))
            //  return Redirect(listPage.Url());
            //  else
            //     return Redirect(ccsiaPage.Url());
        ...
        public ActionResult TenderLogin(Login model)
        {
            ...
            else
            {
                try
                {
                    ...
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        var registerUser = dbcontext.LKO_Registrations.Where(x => x.UserId == model.LoginName && x.Password == model.Password && x.status == true).FirstOrDefault();
                        if (registerUser != null && registerUser.UserType != "SuperAdmin" && registerUser.UserType != "Admin")
                        {
                            DateTime tenderClosingDate = dbcontext.LKO_TenderLists.Where(t => t.Id == registerUser.TenderId).FirstOrDefault().Closing_Date.Value;
                            if (tenderClosingDate < DateTime.Now && registerUser.UserType.ToLower() == "visitor")
                            {
                                ModelState.AddModelError(nameof(model.Password), DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Login tender Error", "This Tender is closed, you won't be able to login now."));
                                return this.View(model);
                            }
                            else if (registerUser.UserType.ToLower() == "visitor" && registerUser.AllowLogin == false)
                            {
                                ModelState.AddModelError(nameof(model.Password), DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Login tender Error", "You won't be able to login now."));
                                return this.View(model);
                            }
                        }
                        ...
                        {
                            var loginhistories = dbcontext.LKO_LoginHistories.Where(x => x.Userid == model.LoginName && x.CreatedDate.GetValueOrDefault().Date == DateTime.Now.Date).Count();
                            if (loginhistories <= 5)
                            {
                                LKO_LoginHistory entity = new LKO_LoginHistory()
                                {
                                    Userid = model.LoginName,
                                    CreatedDate = DateTime.Now.Date
                                };
                                dbcontext.LKO_LoginHistories.InsertOnSubmit(entity);
                                dbcontext.SubmitChanges();
                            }
                            else
                            {
                            ...
                            ModelState.AddModelError(nameof(model.Password), DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Login User Error", "User and password is not valid. Please enter valid credential."));
                            return this.View(model);
                        }
                ...
                catch (System.Exception ex)
                {
                    ...
                    ModelState.AddModelError(nameof(model.Password), DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Login Technical Error", "There is technical problem. Please try after sometime."));
                    return this.View(model);
                }
            }
        }
        public bool OpenTenderDocPAth(Guid tenderid, string NITPRNo)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                ...
                bool IsCorrigendumPresent = dbcontext.LKO_CorrigendumTenderMappings.Any(x => x.TenderId == tenderid && x.LKO_Corrigendum.Status == true);
        ...
        public ActionResult Logout()
        {
            ...
            if (TenderUserSession.TenderUserSessionContext != null && !string.IsNullOrEmpty(TenderUserSession.TenderUserSessionContext.userId))
            {
                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                {
                    var result = (from user in dbcontext.LKO_Registrations where user.UserId == TenderUserSession.TenderUserSessionContext.userId select user).Single();
        ...
        private string IsUploadingInSequesnce(string envelopeType, string userid)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                if (envelopeType == "2")
                {
                    if (!dbcontext.LKO_UserTenderDocuments.Any(x => x.UserId == userid && x.EnvelopeType == "1"))
                    {
                ...
                {
                    if (!dbcontext.LKO_UserTenderDocuments.Any(x => x.UserId == userid && x.EnvelopeType == "1") || !dbcontext.LKO_UserTenderDocuments.Any(x => x.UserId == userid && x.EnvelopeType == "2"))
                    {
        ...
        public FileResult DownloadUserFile(Guid id)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                var fileToDownload = dbcontext.LKO_UserTenderDocuments.Where(i => i.Id == id).FirstOrDefault();
        ...
        public FileResult DownloadFile(Guid id)
        {
            ...
            else
            {
                ...
                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                var fileToDownload = dbcontext.LKO_TenderDocuments.Where(i => i.Id == id).FirstOrDefault();
                return File(fileToDownload.DocData.ToArray(), fileToDownload.ContentType, fileToDownload.FileName);
            }
        }
        ...
        public FileResult DownloadCorrigendumFile(Guid id)
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                var fileToDownload = dbcontext.LKO_CorrigendumDocuments.Where(i => i.Id == id).FirstOrDefault();
                return File(fileToDownload.DocData.ToArray(), fileToDownload.ContentType, fileToDownload.FileName);
            }
        }
        ...
        public ActionResult TenderDetails()
        {
            ...
            try
            {
                ...
                if (TenderUserSession.TenderUserSessionContext != null && !string.IsNullOrEmpty(TenderUserSession.TenderUserSessionContext.TenderId))
                {
                    ...
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        var tenderdetails = dbcontext.LKO_TenderLists.Where(x => x.Id == new System.Guid(tenderId)).FirstOrDefault();
                        var userdocument = dbcontext.LKO_UserTenderDocuments.Where(x => x.UserId == UserId).ToList();
                        var userDetail = dbcontext.LKO_Registrations.Where(x => x.UserId == UserId).FirstOrDefault();
                        ...
                        Tender.tenderdocument = tenderdetails.LKO_TenderDocuments.Where(t => t.IsPQDoc != true).ToList();
                        Tender.tenderPQdocument = tenderdetails.LKO_TenderDocuments.Where(t => t.IsPQDoc == true).ToList();
        ...
        public ActionResult TenderDetails(Tender Model, string saveDoc = null, string uploadDoc = null, string changePassword = null)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(uploadDoc) || !string.IsNullOrEmpty(saveDoc))
                {
                    if (Model != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(uploadDoc))
                        {
                            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                            {
                                var user = dbcontext.LKO_Registrations.Where(x => x.UserId == TenderUserSession.TenderUserSessionContext.userId).FirstOrDefault();
                                if (user != null)
                                {
                                    ...
                                    // List<Registration> UserEmailList = dbcontext.LKO_Registrations.Where(x => x.UserType.ToLower() == "admin" || x.UserType.ToLower() == "superadmin" || (x.UserType.ToLower() == "envelopeadmin" && x.TenderId.ToString() == Model.TenderId.ToString())).ToList();
                                    List<LKO_Registration> UserEmailList;
                                    ...
                                    {
                                        UserEmailList = dbcontext.LKO_Registrations.Where(x => (x.UserType.ToLower() == "envelopeadmin" && x.TenderId.ToString() == Model.TenderId.ToString())).ToList();
                                    }
                                    else
                                    {
                                        UserEmailList = dbcontext.LKO_Registrations.Where(x => x.UserType.ToLower() == "admin" || x.UserType.ToLower() == "superadmin" || (x.UserType.ToLower() == "envelopeadmin" && x.TenderId.ToString() == Model.TenderId.ToString())).ToList();
                                    }
                ...
                if (!string.IsNullOrEmpty(changePassword))
                {
                    ...
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        var user = dbcontext.LKO_Registrations.Where(x => x.UserId == TenderUserSession.TenderUserSessionContext.userId && x.Password == Model.OldPassword).FirstOrDefault();
                        if (user != null)
                        {
        ...
        public string saveUserTenderFile(HttpPostedFileBase file, string envelopeType, string userid)
        {
            if (file != null && file.ContentLength > 0)
            {
                ...
                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                {
                    if (dbcontext.LKO_UserTenderDocuments.Any(x => x.UserId == userid && x.EnvelopeType == envelopeType && x.FileName == file.FileName))
                    {
                    ...
                    else
                    {
                        ...
                        LKO_UserTenderDocument dbdoc = new LKO_UserTenderDocument();
                        ...
                        dbcontext.LKO_UserTenderDocuments.InsertOnSubmit(dbdoc);
                        dbcontext.SubmitChanges();
        ...
        public Tender GetTenderDetails()
        {
            ...
            try
            {
                ...
                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                {
                    ...
                    tender.AdvDate = tenderdetails.Adv_Date;
                    tender.ClosingDate = tenderdetails.Closing_Date;
                    //tender.tenderdocument = tenderdetails.LKO_TenderDocuments.ToList();
                    tender.userUpladdocument = userdocument;
                    tender.PQDocumentStatus = userdocument != null && userdocument.Where(d => d.IsPQDoc == true || d.EnvelopeType == "1").Count() > 0 ? "Uploaded" : "Pending";
                    tender.DocumentStatus = userdocument != null && userdocument.Where(d => d.IsPQDoc == false || d.EnvelopeType == "2").Count() > 0 ? "Uploaded" : "Pending";
                    tender.IsPQVerified = userDetail.IsPQVerified == null ? false : userDetail.IsPQVerified;
        ...
        public ActionResult GetTenderDocument(Guid Id)
        {
            try
            {
                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                {
                    List<LKO_TenderDocument> TD = dbcontext.LKO_TenderDocuments.Where(x => x.TenderId == Id).ToList();
        ...
        {
            LucknowAirportRepository ElecRepo = new LucknowAirportRepository();
        ...
        public ActionResult AdminTenderListing()
        {
            ...
            try
            {
                ...
                List<LKO_TenderList> tenderdata = new List<LKO_TenderList>();
                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                {
                    LKO_TenderList objtender = new LKO_TenderList();
                    if (UserType == "SuperAdmin")
                    {
                        tenderdata = dbcontext.LKO_TenderLists.OrderBy(x => x.Closing_Date).ToList();
                    }
                    else if (UserType == "Admin")
                    {
                        tenderdata = (from tnderlist in dbcontext.LKO_TenderLists orderby tnderlist.Closing_Date ascending where tnderlist.CreatedBy == UserId select tnderlist).ToList();
                    }
                    else // Envelope Admin
                    {
                        tenderdata = (from tdruser in dbcontext.LKO_UserTenderMappings join tnderlist in dbcontext.LKO_TenderLists on tdruser.TenderId equals tnderlist.Id orderby tnderlist.Closing_Date ascending where tdruser.UserId == UserId select tnderlist).ToList();
                    }
        ...
        public ActionResult AdminTenderListing(string Status)
        {
            ...
            try
            {
                ...
                using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                {
                    ...
                    if (UserType == "SuperAdmin")
                    {
                        tenderdata = dbcontext.LKO_TenderLists.OrderByDescending(x => x.Created_Date).ToList();
                    }
                    else // Envelope Admin
                    {
                        tenderdata = (from tdruser in dbcontext.LKO_UserTenderMappings join tnderlist in dbcontext.LKO_TenderLists on tdruser.TenderId equals tnderlist.Id where tdruser.UserId == UserId select tnderlist).ToList();
                    }
        ...
        public ActionResult AdminTenderUserListing()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(Request.QueryString["id"]))
                {
                    ...
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        datalist = dbcontext.LKO_Registrations.Where(x => x.TenderId == tenderId && x.status == true && x.UserType.ToLower() == "visitor").Select(s => new RegistedUserHasDocument
                        {
                            Name = s.Name,
                            ...
                            Mobile = s.Mobile,
                            UserId = s.UserId,
                            Description = s.LKO_TenderList.Description,
                            HasDocumentUploaded = dbcontext.LKO_UserTenderDocuments.Where(x => x.UserId == s.UserId && (x.IsPQDoc == false || x.EnvelopeType == "2")).Any() ? "Yes" : "No",
                            HasPQDocumentUploaded = dbcontext.LKO_UserTenderDocuments.Where(x => x.UserId == s.UserId && (x.IsPQDoc == true || x.EnvelopeType == "2")).Any() ? "Yes" : "No",
                            TenderId = s.LKO_TenderList.Id.ToString(),
        ...
        public ActionResult PQVerificationApprove(string uid, Guid tenderid)
        {
            ...
            try
            {
                ...
                if (UserType == "EnvelopeAdmin")
                {
                    ...
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        var tenderDetail = dbcontext.LKO_TenderLists.Where(x => x.Id == tenderid).FirstOrDefault();
                        if (tenderDetail.PQApprovalRequired == true)
                        {
                            var usertoapprove = dbcontext.LKO_Registrations.Where(i => i.UserId == uid).FirstOrDefault();
                            if (usertoapprove.PQEnvelopeReviewStatus == false || usertoapprove.PQEnvelopeReviewStatus == null)
                            {
                                ...
                                List<LKO_Registration> UserEmailList = dbcontext.LKO_Registrations.Where(x => x.UserType.ToLower() == "admin" || x.UserType.ToLower() == "superadmin").ToList();
                ...
                if (adminAproval)
                {
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        ...
                        if (tenderLoginitem != null)
                        {
                            ...
                            var subsiteURL = Sitecore.Context.Database.GetItem("{FCE658C4-0A89-4929-8FE1-06A2EB33B4CF}").Name;
                        ...
                        try
                        {
                            if (!string.IsNullOrEmpty(RedirectUrl))
                            {
                                var tenderDetails = dbcontext.LKO_TenderLists.Where(t => t.Id == tenderid).FirstOrDefault();
                                TenderService Tn = new TenderService();
                ...
                if (!string.IsNullOrEmpty(Request.QueryString["id"]))
                {
                    ...
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        datalist = dbcontext.LKO_Registrations.Where(x => x.TenderId == tenderId && x.status == true && x.UserType.ToLower() == "visitor").Select(s => new RegistedUserHasDocument
                        {
                            ...
                            UserId = s.UserId,
                            Description = s.LKO_TenderList.Description,
                            HasDocumentUploaded = dbcontext.LKO_UserTenderDocuments.Where(x => x.UserId == s.UserId).Any() ? "Yes" : "No",
                            TenderId = s.LKO_TenderList.Id.ToString()
                        }).ToList();
                        var TenderNITNo = dbcontext.LKO_TenderLists.Where(x => x.Id == tenderId).FirstOrDefault().NITNo;
        ...
        public ActionResult AdminTenderDetails()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(Request.QueryString["id"]) && !string.IsNullOrEmpty(Request.QueryString["uid"]))
                {
                    ...
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        var tenderdetails = dbcontext.LKO_TenderLists.Where(x => x.Id == tenderId).FirstOrDefault();
                        var userdocument = dbcontext.LKO_UserTenderDocuments.Where(x => x.UserId == tenderuserid).ToList();

                        var userdetails = dbcontext.LKO_Registrations.Where(x => x.TenderId == tenderId && x.UserId == tenderuserid).FirstOrDefault();
                        ...
                        tender.userUpladdocument = userdocument;
                        tender.PQApprovalRequired = tenderdetails.PQApprovalRequired == null ? false : tenderdetails.PQApprovalRequired;
                        ViewBag.Envelopetype = UserType == "SuperAdmin" ? "0" : dbcontext.LKO_UserTenderMappings.Where(x => x.UserId == UserId && x.TenderId == tenderId).FirstOrDefault().Envelope;
        ...
        public ActionResult AdminTenderCreate(TenderCreateModel obj)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    {
                        ModelState.AddModelError("Adv_Date", DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Date Invalid formate", "Please Input Date in dd-MM-yyyy HH:mm:ss format"));
                    ...
                    {
                        ModelState.AddModelError("Closing_Date", DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Date Invalid formate", "Please Input Date in dd-MM-yyyy HH:mm:ss format"));
                        return View(ReturnObj(obj));
                    }
                    if (obj.Files.Count() > 0 && obj.PQFiles.Count() > 0)
                    {
                        ...
                        if (obj.Files[0] == null)
                        {
                            ModelState.AddModelError("Files", DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Date Invalid formate", "Please upload tender document"));
                            err = true;
                        }
                        if (obj.PQFiles[0] == null && obj.PQApprovalRequired == true)
                        {
                            ModelState.AddModelError("PQFiles", DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Date Invalid formate", "Please upload PQ document"));
                            err = true;
                        }
                        if (err)
                        {
                    ...
                    foreach (HttpPostedFileBase file in obj.Files)
                    {
                        if (file != null)
                        {
                            ...
                            if (size > 10240)
                            {
                                ModelState.AddModelError("Files", DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Date Invalid formate", "Tender document size can not exceed then 10 MB"));
                                return View(ReturnObj(obj));
                            }
                        }
                    }
                    ...
                    foreach (HttpPostedFileBase file in obj.PQFiles)
                    {
                        if (file != null)
                        {
                            ...
                            if (size > 10000)
                            {
                                ModelState.AddModelError("PQFiles", DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Date Invalid formate", "PQ document size can not exceed then 10 MB"));
                                return View(ReturnObj(obj));
                            }
                        }
                    }
                    ...
                    if (string.IsNullOrEmpty(obj.EnvelopUser1email) == false || string.IsNullOrEmpty(obj.EnvelopUser1Mobile) == false || string.IsNullOrEmpty(obj.EnvelopUser1Name) == false || obj.EnvelopUser1EnvelopNameCheckboxs.Where(x => x.IsChecked == true).Count() > 0)
                    {
                        ...
                        {
                            ModelState.AddModelError("EnvelopUser1Mobile", DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Date Invalid formate", "All Fields are mandatory for Envelop user."));
                            return View(ReturnObj(obj));
                        }
                    }
                    if (string.IsNullOrEmpty(obj.EnvelopUser2email) == false || string.IsNullOrEmpty(obj.EnvelopUser2Mobile) == false || string.IsNullOrEmpty(obj.EnvelopUser2Name) == false || obj.EnvelopUser2EnvelopNameCheckboxs.Where(x => x.IsChecked == true).Count() > 0)
                    {
                        ...
                        {
                            ModelState.AddModelError("EnvelopUser2Mobile", DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Date Invalid formate", "All Fields are mandatory for Envelop user."));
                            return View(ReturnObj(obj));
                        }
                    }
                    if (string.IsNullOrEmpty(obj.EnvelopUser3email) == false || string.IsNullOrEmpty(obj.EnvelopUser3Mobile) == false || string.IsNullOrEmpty(obj.EnvelopUser3Name) == false)
                    {
                        ...
                        {
                            ModelState.AddModelError("EnvelopUser3Mobile", DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Date Invalid formate", "All Fields are mandatory for Envelop user."));
                            return View(ReturnObj(obj));
                        }
                    }
                    if (string.IsNullOrEmpty(obj.EnvelopUser4email) == false || string.IsNullOrEmpty(obj.EnvelopUser4Mobile) == false || string.IsNullOrEmpty(obj.EnvelopUser4Name) == false)
                    {
                        ...
                        {
                            ModelState.AddModelError("EnvelopUser4Mobile", DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Date Invalid formate", "All Fields are mandatory for Envelop user."));
                            return View(ReturnObj(obj));
                        }
                    }
                    if (string.IsNullOrEmpty(obj.EnvelopUser5email) == false || string.IsNullOrEmpty(obj.EnvelopUser5Mobile) == false || string.IsNullOrEmpty(obj.EnvelopUser5Name) == false)
                    {
                        ...
                        {
                            ModelState.AddModelError("EnvelopUser5Mobile", DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Date Invalid formate", "All Fields are mandatory for Envelop user."));
                            return View(ReturnObj(obj));
                        }
                    }
                    ...
                    if (Envelop1User)
                    {
                        ...
                        using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                        {
                            var tender = dbcontext.LKO_TenderLists.Where(x => x.Id == new Guid(EnpObj.SelectTenderId)).FirstOrDefault();
                            if (!string.IsNullOrEmpty(RedirectUrl))
                            {
                                //Note : Send Mail to User 
                                TenderService Tn = new TenderService();
                    ...
                    ViewBag.SuccessMsg = DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Tender Successful Message", "Tender Uploaded Successfully");
                    ModelState.Clear();
                    return View(ReturnObj(obj));
            ...
            catch (Exception ex)
            {
                ...
                ViewBag.ErrorMsg = DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/AdminTender Exception", "Error in uploading Please try again");
                return View(ReturnObj(obj));
            }
        ...
        public ActionResult AdminTenderUpdate(TenderEditModel obj, string Inactivate_tender = null, string Activate_tender = null)
        {
            ...
            try
            {
                ...
                else if (ModelState.IsValid)
                {
                    ...
                    else
                    {
                        ...
                        ViewBag.Message = DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Tender Successful Message", "Tender Uploaded Successfully");
                        return Redirect(listingitem.Url());
                    }
                }
                return View(obj);
            ...
            catch (Exception ex)
            {
                ...
                ViewBag.EditError = DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/TenderUpdate Exception", "Error in uploading Please try again");
                return View(obj);
            }
        }
        ...
        public ActionResult TenderCorrigendumList()
        {
            ...
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                //var CorrigendumIdList = dbcontext.LKO_CorrigendumTenderMappings.Where(x => x.TenderId == TenderId).Select(x => x.Corrigendum).ToList();
                var TenderDetail = dbcontext.LKO_TenderLists.Where(x => x.Id == TenderId).FirstOrDefault();
                ViewBag.TenderDetail = TenderDetail.NITNo + " - " + TenderDetail.Business;

                List<LKO_Corrigendum> Corr = dbcontext.LKO_CorrigendumTenderMappings.Where(x => x.TenderId == TenderId && x.LKO_Corrigendum.Status == true).Select(x => x.LKO_Corrigendum).ToList();
        ...
        public List<TenderDetails> NITRNoFunction(Guid corrigenudmID)
        {
            using (TenderDBDataContext dbcontext = new TenderDBDataContext())
            {
                ...
                var data = dbcontext.LKO_CorrigendumTenderMappings.Where(x => x.CorrigendumId == corrigenudmID).Select(x => x.TenderId).ToList();
                foreach (var item in data)
                {
                    var tendersNITList = dbcontext.LKO_TenderLists.Where(x => x.Id == item).Select(s => new TenderDetails() { NITPRNo = s.NITNo }).ToList();
        ...
        public List<TenderDetails> TenderDocFunction(Guid corrigenudmID)
        {
            ...
            {
                List<TenderDetails> list = new List<TenderDetails>();
                var TenderDocList = dbcontext.LKO_CorrigendumDocuments.Where(x => x.CorrigendumId == corrigenudmID).Select(x => new TenderDetails() { Id = x.Id, FileName = x.FileName }).ToList();
            ...
            if (validationStatus)
            {
                try
                {
                    ...
                    if (ModelState.IsValid)
                    {
                        ...
                        {
                            ViewBag.selectedRecordNull = DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Tender Selection Validation", "Please Select Tender for Corrigendum List");
                            return View(obj);
                        }
                        ...
                        {
                            ModelState.AddModelError("Date", DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Date Invalid formate", "Please Input Date in dd-MM-yyyy HH:mm:ss format"));
                            return View(obj);
                        }
                        else
                        {
                            ...
                            foreach (var ls in selectedRecords)
                            {
                                ...
                                try
                                {
                                    ...
                                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                                    {
                                        var tender = dbcontext.LKO_TenderLists.Where(x => x.Id == TenderId).FirstOrDefault();
                                        var tenderBidders = dbcontext.LKO_Registrations.Where(x => x.TenderId == TenderId && x.UserType.ToLower() == "visitor").Select(s => s.Email).Distinct().ToList();
                        ...
                        ViewBag.SuccessMsg = DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Corrigendum Success Msg", "Corrigendum Uploaded Successfully");
                        ModelState.Clear();
                        CorrigendumModel model = new CorrigendumModel();
                ...
                catch (Exception ex)
                {
                    ...
                    ViewBag.ErrorMsg = DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Corrigendum exception msg", "Error in Insert Please try again");
                    return View(obj);
                }
            }
            else
        ...
        public ActionResult CorrigenudumUpdate(CorrigedumEditModel obj, string Inactivate_Corrigendum = null, string Activate_Corrigendum = null)
        {
            ...
            try
            {
                ...
                else if (ModelState.IsValid)
                {
                    ...
                    {
                        ViewBag.selectedRecordNull = DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Tender Selection Validation", "Please Select Tender List");
                        return View(obj);
                    }
                    else
                    {
            ...
            catch (Exception ex)
            {
                ...
                ViewBag.EditError = DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Corrigendum exception msg", "Error in Insert Please try again");
                return View(obj);
            }
        }
        ...
        public ActionResult DeleteCorrigendumFile(Guid id, string DocumentPath)
        {
            ...
            catch (Exception ex)
            {
                ...
                //return RedirectToAction("EditCorrigendum", "LucknowAirport", new { @id = Edit_Id });
            }
            //  Guid EditId = (Guid)TempData["id"];
            //return RedirectToAction("EditCorrigendum", "LucknowAirport", new { @id = EditId });
            return Redirect(updateitem.Url() + "?id=" + TempData["id"]);
        }
        #endregion
        ...
        public ActionResult AdminEnvelopeCreate(EnvelopUserDetails obj)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        ViewBag.SelectTenderId = DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Selection Validation msg", "Please Select Tender From List");
                        return View(ReturnObj);
                    }
                    if (SelectedEnvelope.Count <= 0)
                    {
                        ViewBag.SelectedEnvelope = DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/Envelop selection msg", "Please Select Minimum 1 Envelope");
                        return View(ReturnObj);
                    }
                    ...
                    using (TenderDBDataContext dbcontext = new TenderDBDataContext())
                    {
                        var tender = dbcontext.LKO_TenderLists.Where(x => x.Id == new Guid(obj.SelectTenderId)).FirstOrDefault();
                        if (!string.IsNullOrEmpty(RedirectUrl))
                        {
                            //Note : Send Mail to User 
                            TenderService Tn = new TenderService();
                    ...
                    ViewBag.SuccessMsg = DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/EnvelopUser Success Msg", "User created successfully and an email has been sent with details.");
                    ModelState.Clear();
                    return View(ReturnObj);
                }
                else
            ...
            catch (Exception ex)
            {
                ...
                ViewBag.ErrorMsg = DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/EnvelopUser Exception Msg", "Error in Insert Please try again");
                return View(ReturnObj);
            }
        ...
        public ActionResult DisableEnvelopeUser(string id)
        {
            ...
            catch (Exception ex)
            {
                ...
                ViewBag.DisableUserError = DictionaryPhraseRepository.Current.Get("/LucknowAirport/Tender/DisableEnvelopeUser Exception Msg", "There is Error in Disable Users");
                return View();
            }
        ...
        public ActionResult AirportSurveyFeedback(AirportSurveyFeedback airportSurvey)
        {
            ...
            if (validationStatus == true)
            {
                ...
                if (isValid == false)
                {
                    ...
                    {
                        TenderDBDataContext rdb = new TenderDBDataContext();
                        LKO_AirportSurveyFeedback ASF = new LKO_AirportSurveyFeedback();
                        ASF.Id = (rdb.LKO_AirportSurveyFeedbacks.Select(x => (long?)x.Id).Max() ?? 0) + 1;
                        ...
                        #region Insert to DB
                        rdb.LKO_AirportSurveyFeedbacks.InsertOnSubmit(ASF);
                        rdb.SubmitChanges();
        ...
        //Contact Us Form
        [HttpPost]
        public ActionResult AirportContactUs(LKOAirportContactUs airportContactUs)
        {
            bool isValid = true;
            bool validationStatus = true;
            var result = new { status = "1" };
            Log.Info("Airport Contact Us Form Start ", this);
            ...
            if (ModelState.IsValid)
            {
                Log.Info("Airport Contact Us Form Validation Start ", this);
                var flag = this.IsReCaptchValidV3(airportContactUs.reResponse);
                if (flag == false)
                {
                    Log.Error("Captcha verification failed " + flag, this);
                    result = new { status = "2" };
                    return Json(result, JsonRequestBehavior.AllowGet);
                }
                Log.Info("Captcha verified ", this);
                ...
                try
                {
                    string securitytoken = DictionaryPhraseRepository.Current.Get("/LKOContactFormCRM/securitytoken", "");
                    string CRMAPI = DictionaryPhraseRepository.Current.Get("/LKOContactFormCRM/API", "");
                    Log.Info("CRM Start  LKO Contact Us form", this);
                    ...
                    Log.Info("CRM END  LKO Contact Us form", this);


                    Log.Info("Insert data into LKO Contact Us DB ", this);
                    TenderDBDataContext rdb = new TenderDBDataContext();
                    LKO_AirportContactUsForm contactUsForm = new LKO_AirportContactUsForm();
                    contactUsForm.Id = (rdb.LKO_AirportContactUsForms.Select(i => (long?)i.Id).Max() ?? 0) + 1;
                    ...
                    #region Insert to DB
                    rdb.LKO_AirportContactUsForms.InsertOnSubmit(contactUsForm);
                    rdb.SubmitChanges();
                    Log.Info("data inserted in  DB  sucessfully  LKO Contact Us form", this);
