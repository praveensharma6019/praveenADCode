...
namespace Sitecore.BelvedereClubAhmedabad.Website.sitecore.admin.BelvedereClubAhmedabad
{
    public partial class ClubAhmedabad_FormsRecord : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else if (ddl1.SelectedIndex == 2)
                {
                    ...
                    var datarecord = from rc in rdb.ClubAhmedabadEnquiries
                                     where ((rc.FormSubmitOn.Value.Date >= fromdate) && (rc.FormSubmitOn.Value.Date <= todate))
                                     select rc;
                    if (System.Convert.ToDateTime(TextBoxTo.Text) == System.Convert.ToDateTime(TextBoxFrom.Text))
                    {
                        datarecord = from rc in rdb.ClubAhmedabadEnquiries
                                     where rc.FormSubmitOn.Value.Date == fromdate
                                     select rc;
                    }
                    DataTable dt = ToDataTable(datarecord.ToList());
                    string savefilepath = Server.MapPath("~/sitecore/admin/BelvedereClubAhmedabad/SampleFile/ClubAhmedabad-EnquiryRecord.csv");
                    DatatableToCSV(dt, savefilepath);
                }
                else if (ddl1.SelectedIndex == 1)
                {
                    ClubAhmdRoomAvailabilityDataContext adc = new ClubAhmdRoomAvailabilityDataContext();
                    var datarecord = from rc in adc.ClubAhmedabadRoomAvailabilities
                                     where ((rc.FormSubmitOn.Value.Date >= fromdate) && (rc.FormSubmitOn.Value.Date <= todate))
                                     select rc;
                    if (System.Convert.ToDateTime(TextBoxTo.Text) == System.Convert.ToDateTime(TextBoxFrom.Text))
                    {
                        datarecord = from rc in adc.ClubAhmedabadRoomAvailabilities
                                     where rc.FormSubmitOn.Value.Date == fromdate
                                     select rc;
                    }
                    DataTable dt = ToDataTable(datarecord.ToList());
                    string savefilepath = Server.MapPath("~/sitecore/admin/BelvedereClubAhmedabad/SampleFile/ClubAhmedabad-BookingRecord.csv");
                    DatatableToCSV(dt, savefilepath);
                }
        ...
        public void DownLoad(string FName)
        {
            ...
            {
                Response.Clear();
                Response.AddHeader("Content-Disposition", "attachment; filename=ClubAhmedabad-Records.csv");
