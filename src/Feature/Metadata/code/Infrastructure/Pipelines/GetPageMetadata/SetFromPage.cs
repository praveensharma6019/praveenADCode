namespace Sitecore.Feature.Metadata.Infrastructure.Pipelines.GetPageMetadata
{
    ...
    public class SetFromPage
    {
        public void Process(GetPageMetadataArgs args)
        {
            ...
            this.SetCustomCanonicalURL(args.Item, args.Metadata);
        ...
        private void SetCustomCanonicalURL(Item item, IMetadata metadata)
        {
            var CustomCanonicalURL = item[Templates.PageMetadata.Fields.CustomCanonicalURL];
            if (string.IsNullOrWhiteSpace(CustomCanonicalURL))
                return;
            metadata.CustomCanonicalURL = CustomCanonicalURL;
        }
