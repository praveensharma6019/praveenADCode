...
{
    public class FAQSearchResolver : Sitecore.LayoutService.ItemRendering.ContentsResolvers.RenderingContentsResolver
    {
        private readonly IFAQSearchService _faqsearch;
        public FAQSearchResolver(IFAQSearchService faqSearch)
        {
            this._faqsearch = faqSearch;
        }
        public override object ResolveContents(Rendering rendering, IRenderingConfiguration renderingConfig)
        {
            Item contextItem = RenderingContext.Current.ContextItem;
            return _faqsearch.GetFAQSearchData(rendering,contextItem);
        }
    }
}