namespace Sitecore.Feature.Accounts.Controllers
{
    ...
    public class AEMLChangeOfNameController : Controller
    {
        ...
        #region LEC User Login and Registration and Logout

        [HttpGet]
        public ActionResult ChangeOfNameLECUserRegisteration()
        {
            ChangeOfNameLECUserRegistrationModel model = new ChangeOfNameLECUserRegistrationModel();
        ...
        public ActionResult ChangeOfNameLECUserRegisteration(ChangeOfNameLECUserRegistrationModel model, string ValidateLEC = null, string ValidateOTP = null, string Submit = null)
        {
            if (!string.IsNullOrEmpty(ValidateLEC))
            {
                ...
                Log.Info("ChangeOfNameLECUserRegisteration validate LECRegistrationNumber from DB and SAP", this);
                ChangeOfNameService changeOfNameService = new ChangeOfNameService();

                //validate 
                //1. Registration number call sap service
                Log.Info("ChangeOfNameLECUserRegisteration validate LECRegistrationNumber from SAP", this);
                var LECDetails = SapPiService.Services.RequestHandler.ValidateLECAndFetchDetails(model.InputByUser, null, null, false);
                if (LECDetails.IsValid)
                {
                    model.IsValidatedByRegistrationNumber = true;
                    model.IsValidatedByMobileNumber = false;
                    model.IsValidatedByEmailId = false;
                }
                else
                {
                    //2. validate using mobile number 
                    LECDetails = SapPiService.Services.RequestHandler.ValidateLECAndFetchDetails(null, model.InputByUser, null, false);
                    if (LECDetails.IsValid)
                    {
                        model.IsValidatedByMobileNumber = true;
                        model.IsValidatedByEmailId = false;
                        model.IsValidatedByRegistrationNumber = false;
                    }
                    else
                    {
                        //3. validate using Email Id 
                        LECDetails = SapPiService.Services.RequestHandler.ValidateLECAndFetchDetails(null, null, model.InputByUser, false);
                        if (LECDetails.IsValid)
                        {
                            model.IsValidatedByEmailId = true;
                            model.IsValidatedByRegistrationNumber = false;
                            model.IsValidatedByMobileNumber = false;
                        }
                    }
                ...
                if (model.IsValidatedByEmailId || model.IsValidatedByMobileNumber || model.IsValidatedByRegistrationNumber)
                {
                    //check for validity
                    DateTime LECValidity = DateTime.Parse(LECDetails.LEC_Validity_Info);
                    if (LECValidity.Date < DateTime.Now.Date)
                    {
                        this.ModelState.AddModelError(nameof(model.InputByUser), DictionaryPhraseRepository.Current.Get("/CON/LEC Registration/LEC Validation expired Exists", "LEC validity period expired."));
                        return this.View(model);
                    }

                    //check in database
                    if (changeOfNameService.IsLECRegistered(LECDetails.LEC_License_NO))
                    {
                        this.ModelState.AddModelError(nameof(model.InputByUser), DictionaryPhraseRepository.Current.Get("/CON/LEC Registration/Registration Number Exists", "LEC is already registered for this Registration Number"));
                        return this.View(model);
                    }

                    ChangeOfNameLECRegistrationDetails LECRegistrationDetails = new ChangeOfNameLECRegistrationDetails
                    {
                        InputByUser = model.InputByUser,
                        EmailId = LECDetails.LEC_Email,
                        IsValidatedByEmail = model.IsValidatedByEmailId,
                        IsValidatedByMobileNumber = model.IsValidatedByMobileNumber,
                        IsValidatedByRegistrationNumber = model.IsValidatedByRegistrationNumber,
                        MobileNumber = LECDetails.LEC_MOBILE_NO,
                        Name = LECDetails.LEC_Name,
                        RegistrationNumber = LECDetails.LEC_License_NO
                    };
                    Session["LECRegistrationDetails"] = LECRegistrationDetails;

                    ChangeOfNameService objService = new ChangeOfNameService();
                    //model.IsValidatedByRegistrationNumber = true;
                    //model.IsValidatedByMobileNumber = false;
                    //model.IsValidatedByMobileNumber = false;
                    model.LECRegistrationNumber = LECDetails.LEC_License_NO;
                    model.LECName = LECDetails.LEC_Name;
                    model.LECMobileNumber = LECDetails.LEC_MOBILE_NO.Substring(0, 1) + "xxxxxxx" + LECDetails.LEC_MOBILE_NO.Substring(LECDetails.LEC_MOBILE_NO.Length - 2);
                    model.LECEmailId = string.IsNullOrEmpty(LECDetails.LEC_Email) ? LECDetails.LEC_Email : LECDetails.LEC_Email.Substring(0, 2) + "xxxxxxxxxx" + LECDetails.LEC_Email.Substring(LECDetails.LEC_Email.Length - 3);
                    model.LECValidity = LECDetails.LEC_Validity_Info;

                    //Send OTP 
                    if (objService.IsLECOTPMaxLimitExceed(model.LECRegistrationNumber, LECDetails.LEC_MOBILE_NO, "LECRegistration"))
                    {
                        Log.Info("ChangeOfNameLECUserRegisteration: Number of attempt to get OTP reached for Registration Number and Mobile Number." + model.LECRegistrationNumber + ", " + LECDetails.LEC_MOBILE_NO, this);
                        this.ModelState.AddModelError(nameof(model.LECRegistrationNumber), DictionaryPhraseRepository.Current.Get("/CON/LEC Registration/Max20OTPPerLECMobile", "Number of attempt to get OTP reached for Entered Registration Number."));
                        return this.View(model);
                    }
                    ...
                    string generatedotp = registrationRepo.GenerateOTPRegistration(model.LECRegistrationNumber, null, "LECRegistration", LECDetails.LEC_MOBILE_NO);
                    //string generatedotp = "12345";
                    #endregion

                    if (model.IsValidatedByMobileNumber || model.IsValidatedByRegistrationNumber)
                    {
                        #region Api call to send SMS of OTP
                        try
                        {
                            var apiurl = string.Format(DictionaryPhraseRepository.Current.Get("/CON/LEC Registration/OTP API URL",
                                "https://hapibk.connectbox.in:8181/SendSMS.aspx?UserName=Web_OTP&password=SM78ysAu&MobileNo={0}&SenderID=ADANIE&CDMAHeader=ADANIE&Message=Welcome to Adani Electricity. You have initiated a request for registration on the LEC portal for License no. {1}. OTP for validation is {2}.&dlt_peid=1001756674011432608&dlt_tmid=120210000043&dlt_templateid=1707160794894603725"), LECDetails.LEC_MOBILE_NO, model.LECRegistrationNumber, generatedotp);
                            ...
                            if (response.IsSuccessStatusCode)
                            {
                                ...
                                model.ValidateRegistrationNumber = true;
                                return this.View(model);
                            }
                            else
                            {
                                Log.Error("ChangeOfNameLECUserRegisteration OTP Api call failed for registration", this);
                                this.ModelState.AddModelError(nameof(model.InputByUser), DictionaryPhraseRepository.Current.Get("/CON/LEC Registration/OTP sending error", "Unable to send OTP."));
                                return this.View(model);
                            }
                            //model.ValidateRegistrationNumber = true;
                            //return this.View(model);
                    ...
                    {
                        //send email 
                        bool isEmailSent = objService.SendOTPViaEmail(LECDetails.LEC_Email, LECDetails.LEC_License_NO, generatedotp);
                        if (isEmailSent)
                        {
                            model.ValidateRegistrationNumber = true;
                        }
                        ...
                        {
                            model.ValidateRegistrationNumber = false;
                            Log.Error("ChangeOfNameLECUserRegisteration OTP Api call failed for registration", this);
                            this.ModelState.AddModelError(nameof(model.InputByUser), DictionaryPhraseRepository.Current.Get("/CON/LEC Registration/OTP Email sending error", "Unable to send OTP."));
                            return this.View(model);
                        }
                        return this.View(model);
                    }
                ...
                {
                    Log.Info("ChangeOfNameLECUserRegisteration validate LECRegistrationNumber from SAP - not valid:" + LECDetails.Message, this);
                    this.ModelState.AddModelError(nameof(model.InputByUser), DictionaryPhraseRepository.Current.Get("/CON/LEC Registration/Input not valid", "Entered details not found associated with any LEC, please enter valid LEC License Number / Mobile Number / Email ID."));
                    return this.View(model);
                }
            }
            else if (!string.IsNullOrEmpty(ValidateOTP))
            {
                if (Session["LECRegistrationDetails"] == null)
                {
                    this.ModelState.AddModelError(nameof(model.InputByUser), DictionaryPhraseRepository.Current.Get("/CON/LEC Registration/Enter OTP", "Please validate."));
                    model.ValidateOTP = false;
                    model.ValidateRegistrationNumber = false;
                    return this.View(model);
                }
                ...
                var LECDetails = (ChangeOfNameLECRegistrationDetails)Session["LECRegistrationDetails"];
                //if (model.IsValidatedByMobileNumber && LECDetails.InputByUser != model.LECMobileNumber)
                //{
                //    ModelState.AddModelError(nameof(model.LECRegistrationNumber), DictionaryPhraseRepository.Current.Get("/Accounts/PANUpdate/Registration Number incorrect", "Registration Number is invalid."));
                //    model.ValidateOTP = false;
                //    model.ValidateRegistrationNumber = false;
                //    return this.View(model);
                //}
                //if (model.IsValidatedByRegistrationNumber && LECDetails.InputByUser != model.LECRegistrationNumber)
                //{
                //    ModelState.AddModelError(nameof(model.LECRegistrationNumber), DictionaryPhraseRepository.Current.Get("/Accounts/PANUpdate/Registration Number incorrect", "Registration Number is invalid."));
                //    model.ValidateOTP = false;
                //    model.ValidateRegistrationNumber = false;
                //    return this.View(model);
                //}
                //if (model.IsValidatedByEmailId && LECDetails.InputByUser != model.LECEmailId)
                //{
                //    ModelState.AddModelError(nameof(model.LECRegistrationNumber), DictionaryPhraseRepository.Current.Get("/Accounts/PANUpdate/Registration Number incorrect", "Registration Number is invalid."));
                //    model.ValidateOTP = false;
                //    model.ValidateRegistrationNumber = false;
                ...
                //}
                RegistrationRepository registrationRepo = new RegistrationRepository();
                string generatedOTP = registrationRepo.GetOTPRegistrationByMobAndCA(LECDetails.MobileNumber, LECDetails.RegistrationNumber, "LECRegistration");
                //generatedOTP = "12345";
                ...
                {
                    model.IsValidatedByEmailId = LECDetails.IsValidatedByEmail;
                    model.IsValidatedByMobileNumber = LECDetails.IsValidatedByMobileNumber;
                    model.IsValidatedByRegistrationNumber = LECDetails.IsValidatedByRegistrationNumber;
                    model.InputByUser = LECDetails.InputByUser;
                    model.ValidateOTP = true;
                    model.ValidateRegistrationNumber = true;
                    //model.LECUserName = LECDetails.InputByUser;
                    return View(model);
                }
            }
            else if (!string.IsNullOrEmpty(Submit))
            {
                ...
                try
                {
                    ...
                    string userName = LECDetails.RegistrationNumber;
                    if (model.UserNameType == "2")
                        userName = LECDetails.MobileNumber;
                    else if (model.UserNameType == "3")
                        userName = LECDetails.EmailId;
                    ...
                    {
                        CONLECRegistrationDetail ObjReg = new CONLECRegistrationDetail();
                        ObjReg.LECRegistrationNumber = LECDetails.RegistrationNumber;
                        ObjReg.LECPassword = objcrypt.encodetext(model.LECPassword);
                        ObjReg.LECName = LECDetails.Name;
                        ObjReg.LECEmailId = LECDetails.EmailId;
                        ObjReg.LECMobileNumber = LECDetails.MobileNumber;
                        ObjReg.LECUserName = userName;
                        ObjReg.LECCreatedDate = System.DateTime.Now;
                        ObjReg.LECIsActive = true;
                        dbcontext.CONLECRegistrationDetails.InsertOnSubmit(ObjReg);
                        dbcontext.SubmitChanges();
                    }
                    //email to be sent from here - username and password
                    ViewBag.Message = DictionaryPhraseRepository.Current.Get("/CON/LEC Registration/Success", "LEC Registration is successful, please login to access the Portal and Services.");
                    Session["Message"] = DictionaryPhraseRepository.Current.Get("/CON/LEC Registration/Success", "LEC Registration is successful, please login to access the Portal and Services.");

                    return RedirectPermanent(this.UserProfileService.GetPageURL(Templates.LECPortal.LECUserLoginPage));
                }
                catch (MembershipCreateUserException ex)
                {
                    Log.Error("ChangeOfNameLECUserRegisteration error in registration", ex, this);
                    this.ModelState.AddModelError(model.LECRegistrationNumber, ex.Message);
                }
            ...
            else
            {
                ...
                var LECDetails = (ChangeOfNameLECRegistrationDetails)Session["LECRegistrationDetails"];
                if (model.InputByUser != LECDetails.InputByUser)
                {
                    ModelState.AddModelError(nameof(model.LECRegistrationNumber), DictionaryPhraseRepository.Current.Get("/Accounts/PANUpdate/Registration Number incorrect", "Registration Number is invalid."));
                    model.ValidateOTP = false;
                    model.ValidateRegistrationNumber = false;
                ...
                if (model.UserNameType == "1")
                {
                    model.LECUserName = LECDetails.RegistrationNumber;
                    model.ValidateOTP = true;
                    model.ValidateRegistrationNumber = true;
                }
                else if (model.UserNameType == "2")
                {
                    model.LECUserName = LECDetails.MobileNumber;
                    model.ValidateOTP = true;
                    model.ValidateRegistrationNumber = true;
                }
                else if (model.UserNameType == "3")
                {
                    model.LECUserName = LECDetails.EmailId;
                    model.ValidateOTP = true;
                    model.ValidateRegistrationNumber = true;
                }
                ...
                model.IsValidatedByRegistrationNumber = LECDetails.IsValidatedByRegistrationNumber;
                model.InputByUser = LECDetails.InputByUser;
                if (this.ModelState.ContainsKey("LECPassword"))
                    this.ModelState["LECPassword"].Errors.Clear();
                if (this.ModelState.ContainsKey("LECConfirmPassword"))
                    this.ModelState["LECConfirmPassword"].Errors.Clear();
            }
            return View(model);
        }

        [HttpGet]
        public ActionResult ChangeOfNameLECUserLogin()
        {
            if (Session["LECUserLogin"] != null)
            {
                var item = Context.Database.GetItem(Templates.LECPortal.LECPortalHomePage);
                return this.Redirect(item.Url());
            }
            ChangeOfNameLECUserLoginModel model = new ChangeOfNameLECUserLoginModel();
            return this.View(model);
        }
        ...
        [ValidateRenderingId]
        [ValidateAntiForgeryToken]
        public ActionResult ChangeOfNameLECUserLogin(ChangeOfNameLECUserLoginModel model, string SignInWithCredentials = null, string SignInWithOTP = null, string ValidateOTP = null)
        {
            if (!string.IsNullOrEmpty(SignInWithCredentials))
            {
                if (string.IsNullOrEmpty(model.UserName) && string.IsNullOrEmpty(model.Password))
                {
                    ModelState.AddModelError(nameof(model.UserName), DictionaryPhraseRepository.Current.Get("/CON/LEC Login/Enter User Name password", "Please enter valid User Name and Password."));
                    return this.View(model);
                }
                if (string.IsNullOrEmpty(model.UserName))
                {
                    ModelState.AddModelError(nameof(model.UserName), DictionaryPhraseRepository.Current.Get("/CON/LEC Login/Enter User Name", "Please enter valid User Name."));
                    return this.View(model);
                }
                if (string.IsNullOrEmpty(model.Password))
                {
                    ModelState.AddModelError(nameof(model.Password), DictionaryPhraseRepository.Current.Get("/CON/LEC Login/Enter password", "Please enter valid Password."));
                    return this.View(model);
                }
                if (string.IsNullOrWhiteSpace(Request.Form["g-recaptcha-response"]))
                {
                ...
                if (ModelState.IsValid)
                {
                    try
                    {
                        ...
                        string EncryptionKey = itemInfo.Fields[Templates.CONAndEncryptionSettings.CONSettings.EncryptionKey].Value; // "@Aeml#2020";
                        clsEncryptAES objcrypt = new clsEncryptAES(EncryptionKey);
                        string encodedPassword = objcrypt.encodetext(model.Password);
                        using (PaymentHistoryDataContext dbcontext = new PaymentHistoryDataContext())
                        {
                            var registerUser = dbcontext.CONLECRegistrationDetails.Where(x => x.LECUserName == model.UserName && x.LECPassword == encodedPassword && x.LECIsActive == true).FirstOrDefault();

                            if (registerUser != null)
                            {
                                UserSession.AEMLCONLECUserSessionContext = new ChangeOfNameLECUserLoginSessionModel
                                {
                                    UserName = model.UserName,
                                    Password = model.Password,
                                    LECName = registerUser.LECName,
                                    EmailId = registerUser.LECEmailId,
                                    MobileNumber = registerUser.LECMobileNumber,
                                    RegistrationNumber = registerUser.LECRegistrationNumber
                                };
                            }
                            else
                            {
                                ModelState.AddModelError(nameof(model.Password), DictionaryPhraseRepository.Current.Get("/CON/LECLogin/Login User Error", "User Name and password is not valid. Please enter valid credential."));
                                return this.View(model);
                            }

                            return RedirectPermanent(this.UserProfileService.GetPageURL(Templates.LECPortal.LECPortalHomePage));
                        }
                    }
                    catch (System.Exception ex)
                    {
                        Sitecore.Diagnostics.Log.Error("ChangeOfNameLECUserLogin error in login" + ex.Message, this);
                        ModelState.AddModelError(nameof(model.Password), DictionaryPhraseRepository.Current.Get("/CON/LECLogin/Login Technical Error", "There is technical problem. Please try after sometime."));
                        return this.View(model);
                    }
                }
            }
            else if (!string.IsNullOrEmpty(SignInWithOTP))
            {
                if (string.IsNullOrEmpty(model.InputLoginWithOTP))
                {
                    ModelState.AddModelError(nameof(model.InputLoginWithOTP), DictionaryPhraseRepository.Current.Get("/CON/LEC Login/Input required", "Please enter required value."));
                    return this.View(model);
                }
                ...
                try
                {
                    using (PaymentHistoryDataContext dbcontext = new PaymentHistoryDataContext())
                    {
                        var registerUser = dbcontext.CONLECRegistrationDetails.Where(x => (x.LECMobileNumber == model.InputLoginWithOTP || x.LECEmailId == model.InputLoginWithOTP) && x.LECIsActive == true).FirstOrDefault();

                        if (registerUser != null)
                        {
                            ChangeOfNameService objService = new ChangeOfNameService();

                            UserSession.AEMLCONLECUserSessionOTPContext = new ChangeOfNameLECUserLoginSessionForOTPModel
                            {
                                LECName = registerUser.LECName,
                                EmailId = registerUser.LECEmailId,
                                MobileNumber = registerUser.LECMobileNumber,
                                IsMobileNumberInput = registerUser.LECMobileNumber == model.InputLoginWithOTP ? true : false,
                                LECInput = model.InputLoginWithOTP,
                                LECRegistrationNumber = registerUser.LECRegistrationNumber
                            };

                            //Send OTP
                            if (objService.IsLECOTPMaxLimitExceed(registerUser.LECRegistrationNumber, model.InputLoginWithOTP, "LECLogin"))
                            {
                                Log.Info("ChangeOfNameLECUserLogin: Number of attempt to get OTP reached for Input and Mobile Number." + model.InputLoginWithOTP + ", " + registerUser.LECMobileNumber, this);
                                this.ModelState.AddModelError(nameof(model.InputLoginWithOTP), DictionaryPhraseRepository.Current.Get("/CON/LEC Login/Max20OTPPerLECMobile", "Number of attempt to get OTP reached for Entered value."));
                                return this.View(model);
                            }
                            ...
                            RegistrationRepository registrationRepo = new RegistrationRepository();
                            string generatedotp = registrationRepo.GenerateOTPRegistration(registerUser.LECRegistrationNumber, null, "LECLogin", model.InputLoginWithOTP);
                            //string generatedotp = "12345";
                            #endregion

                            if (registerUser.LECMobileNumber == model.InputLoginWithOTP)
                            {
                                //send otp via SMS
                                #region Api call to send SMS of OTP
                                try
                                {
                                    var apiurl = string.Format(DictionaryPhraseRepository.Current.Get("/CON/LEC Login/OTP API URL",
                                        "https://hapibk.connectbox.in:8181/SendSMS.aspx?UserName=Web_OTP&password=SM78ysAu&MobileNo={0}&SenderID=ADANIE&CDMAHeader=ADANIE&Message=Welcome to Adani Electricity. You have initiated a login request on the LEC portal for License No.{1}. OTP for validation is {2}.&dlt_peid=1001756674011432608&dlt_tmid=120210000043&dlt_templateid=1707160794909789128"), registerUser.LECMobileNumber, registerUser.LECRegistrationNumber, generatedotp);
                                    ...
                                    {
                                        Log.Error("ChangeOfNameLECUserRegisteration OTP Api call failed for registration", this);
                                        this.ModelState.AddModelError(nameof(model.InputLoginWithOTP), DictionaryPhraseRepository.Current.Get("/CON/LEC Login/OTP sending error", "Unable to send OTP."));
                                        return this.View(model);
                                    }
                                    //model.IsLoginViaOTP = true;
                                    //model.IsOTPSent = true;
                            ...
                            {
                                //send otp via email
                                bool isEmailSent = objService.SendOTPForLoginViaEmail(registerUser.LECEmailId, registerUser.LECRegistrationNumber, generatedotp);
                                if (isEmailSent)
                                {
                                    model.IsLoginwithEmailId = true;
                                    model.IsLoginViaOTP = true;
                                    model.IsOTPSent = true;
                                }
                                else
                                {
                                    ...
                                    Log.Error("ChangeOfNameLECUserLogin: OTP Api call failed for login", this);
                                    this.ModelState.AddModelError(nameof(model.InputLoginWithOTP), DictionaryPhraseRepository.Current.Get("/CON/LEC Login/OTP sending error", "Unable to send OTP."));
                                }
                                return this.View(model);
                            }
                        ...
                        {
                            ModelState.AddModelError(nameof(model.InputLoginWithOTP), DictionaryPhraseRepository.Current.Get("/CON/LECLogin/Login via OTP input Error", "User is not registered with entered value."));
                            return this.View(model);
                        }
            ...
            {
                //validated otp
                if (Session["LECUserLoginOTP"] == null)
                {
                    this.ModelState.AddModelError(nameof(model.InputLoginWithOTP), DictionaryPhraseRepository.Current.Get("/CON/LEC Registration/Enter OTP", "Please validate."));
                    model.IsLoginViaOTP = false;
                    return this.View(model);
                }
                if (string.IsNullOrEmpty(model.OTPNumber))
                {
                ...
                var LECDetails = (ChangeOfNameLECUserLoginSessionForOTPModel)Session["LECUserLoginOTP"];

                RegistrationRepository registrationRepo = new RegistrationRepository();
                string generatedOTP = registrationRepo.GetOTPRegistrationByMobAndCA(LECDetails.LECInput, LECDetails.LECRegistrationNumber, "LECLogin");
                ...
                UserSession.AEMLCONLECUserSessionContext = new ChangeOfNameLECUserLoginSessionModel
                {
                    UserName = model.UserName,
                    Password = model.Password,
                    LECName = LECDetails.LECName,
                    EmailId = LECDetails.EmailId,
                    MobileNumber = LECDetails.MobileNumber,
                    RegistrationNumber = LECDetails.LECRegistrationNumber
                };
                return RedirectPermanent(this.UserProfileService.GetPageURL(Templates.LECPortal.LECPortalHomePage));
            }
            return View(model);
        ...
        [HttpPost]
        public ActionResult LogoutLEC()
        {
            var item = Context.Database.GetItem(Templates.LECPortal.LECUserLoginPage);
            this.Session["LECUserLogin"] = null;
            this.Session["LECUserLoginOTP"] = null;
            UserSession.AEMLCONLECUserSessionContext = null;
            this.Session.Abandon();
            return this.Redirect(item.Url());
        }

        [HttpGet]
        public ActionResult LECPortalHome()
        {
            if (Session["LECUserLogin"] == null)
            {
                var item = Context.Database.GetItem(Templates.LECPortal.LECUserLoginPage);
        ...
        [HttpGet]
        public ActionResult LECProfileUpdate()
        {
            if (Session["LECUserLogin"] == null)
            {
                var item = Context.Database.GetItem(Templates.LECPortal.LECUserLoginPage);
        ...
        [ValidateRenderingId]
        [ValidateAntiForgeryToken]
        public ActionResult LECProfileUpdate(ChangeOfNameLECUserProfileModel model, string submit = null, string ValidateOTP = null)
        {
            if (Session["LECUserLogin"] == null)
            {
                var item = Context.Database.GetItem(Templates.LECPortal.LECUserLoginPage);
            ...
            if (UserSession.AEMLCONLECUserSessionContext.RegistrationNumber != model.LECRegistrationNumber)
            {
                Log.Info("Error at ChangeOfNameLECProfileManagement UserSession.AEMLCONLECUserSessionContext.RegistrationNumber != model.LECRegistrationNumber:" + UserSession.AEMLCONLECUserSessionContext.RegistrationNumber + ":" + model.LECRegistrationNumber, this);
                this.Session["UpdateMessage"] = new InfoMessage(DictionaryPhraseRepository.Current.Get("/CON/LEC Profile/Profile update error", "Profile was not updated, please try again!"), InfoMessage.MessageType.Error);
            ...
            if (!string.IsNullOrEmpty(model.LECMobileNumber))
            {
                if (submit != null)
                {
                    RegistrationRepository registrationRepo = new RegistrationRepository();
                    ...
                    if (objService.IsLECOTPMaxLimitExceed(model.LECRegistrationNumber, model.LECMobileNumber, "LECUpdateProfile"))
                    {
                        Log.Info("Number of attempt to get OTP reached for LEC Profile update." + model.LECRegistrationNumber, this);
                        this.ModelState.AddModelError(nameof(model.LECMobileNumber), DictionaryPhraseRepository.Current.Get("/CON/Max20OTPPerCA", "Number of attempt to get OTP reached for LEC."));
                        model.isOTPSent = false;
                        return this.View(model);
                    }

                    #region Generate New Otp for given mobile number and save to database
                    string generatedotp = registrationRepo.GenerateOTPRegistration(model.LECRegistrationNumber, null, "LECUpdateProfile", model.LECMobileNumber);
                    #endregion

                    #region Api call to send SMS of OTP
                    try
                    {
                        var apiurl = string.Format(DictionaryPhraseRepository.Current.Get("/CON/LEC Profile/OTP API URL",
                                        "https://hapibk.connectbox.in:8181/SendSMS.aspx?UserName=Web_OTP&password=SM78ysAu&MobileNo={0}&SenderID=ADANIE&CDMAHeader=ADANIE&Message=Welcome to Adani Electricity. You have initiated a login request on the LEC portal for License No.{1}. OTP for validation is {2}.&dlt_peid=1001756674011432608&dlt_tmid=120210000043&dlt_templateid=1707160794909789128"), model.LECMobileNumber, model.LECRegistrationNumber, generatedotp);
                        ...
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            this.ModelState.AddModelError(nameof(model.LECMobileNumber), DictionaryPhraseRepository.Current.Get("/CON/LEC Profile/Mobile numberOTP", "OTP Sent."));
                            model.isOTPSent = true;
                            return this.View(model);
                        }
                        else
                        {
                            ...
                            this.ModelState.AddModelError(nameof(model.LECMobileNumber), DictionaryPhraseRepository.Current.Get("/CON/LEC Profile/Mobile numberOTPFailed", "Unable to send OTP."));
                            model.isOTPSent = false;
                            return this.View(model);
                        }
                    }
                ...
                else
                {
                    ...
                    RegistrationRepository registrationRepo = new RegistrationRepository();
                    string generatedOTP = registrationRepo.GetOTPRegistrationByMobAndCA(model.LECMobileNumber, model.LECRegistrationNumber, "LECUpdateProfile");
                    if (!string.Equals(generatedOTP, model.OTPNumber))
                    {
                        this.ModelState.AddModelError(nameof(model.OTPNumber), DictionaryPhraseRepository.Current.Get("/CON/LEC Profile/Mobile numberOTPmismatch", "OTP does not match. Please enter valid OTP."));
                        model.isOTPSent = true;
                        return this.View(model);
                    }
                    else
                    {
                        ...
                        var resultInfo = SapPiService.Services.RequestHandler.ValidateLECAndFetchDetails(model.LECRegistrationNumber, model.LECMobileNumber, model.LECEmailId, true);
                        if (resultInfo.IsUpdated && resultInfo.IsValid)
                        {
                            //update details in DB
                            ChangeOfNameService objService = new ChangeOfNameService();
                            var result = objService.UpdateLECProfile(model.LECRegistrationNumber, model.LECMobileNumber, model.LECEmailId);
                            if (result == true)
                                this.Session["UpdateMessage"] = new InfoMessage(DictionaryPhraseRepository.Current.Get("/CON/LEC Profile/Profile Success", "Profile was successfully updated"), InfoMessage.MessageType.Success);
                            else
                                this.Session["UpdateMessage"] = new InfoMessage(DictionaryPhraseRepository.Current.Get("/CON/LEC Profile/Profile update error", "Profile was not successfully updated, please try again!"), InfoMessage.MessageType.Error);
                        }
                        else
                        {
                            Sitecore.Diagnostics.Log.Error("Error at " + MethodBase.GetCurrentMethod().Name + ":" + resultInfo.Message, this);
                            this.Session["UpdateMessage"] = new InfoMessage(DictionaryPhraseRepository.Current.Get("/CON/LEC Profile/Profile update error", "Profile was not successfully updated, please try again!"), InfoMessage.MessageType.Error);
                        }
                        return this.Redirect(this.Request.RawUrl);
                    }
        ...
        [HttpGet]
        public ActionResult LECProfileChangePassword()
        {
            if (Session["LECUserLogin"] == null)
            {
                var item = Context.Database.GetItem(Templates.LECPortal.LECUserLoginPage);
            ...
            ChangeOfNameService conService = new ChangeOfNameService();
            ChangeOfNameLECChangePasswordModel model = new ChangeOfNameLECChangePasswordModel();
            return View(model);
        }
        ...
        [ValidateRenderingId]
        [ValidateAntiForgeryToken]
        public ActionResult LECProfileChangePassword(ChangeOfNameLECChangePasswordModel model)
        {
            try
            {
                if (Session["LECUserLogin"] == null)
                ...
                //if (UserSession.AEMLCONLECUserSessionContext.RegistrationNumber != model.LECRegistrationNumber)
                //{
                //    Log.Info("Error at LECProfileChangePassword UserSession.AEMLCONLECUserSessionContext.RegistrationNumber != model.LECRegistrationNumber:" + UserSession.AEMLCONLECUserSessionContext.RegistrationNumber + ":" + model.LECRegistrationNumber, this);
                //    this.Session["UpdateMessage"] = new InfoMessage(DictionaryPhraseRepository.Current.Get("/CON/LEC Profile/Password update error", "Password was not changed!"));
                //    return this.View(model);
                //}

                ChangeOfNameService objservice = new ChangeOfNameService();
                var isOldPasswordSame = objservice.CheckOldPasswordLEC(UserSession.AEMLCONLECUserSessionContext.RegistrationNumber, model.LECPassword);
                if (!isOldPasswordSame)
                {
                    //this.Session["UpdateMessage"] = new InfoMessage(DictionaryPhraseRepository.Current.Get("/CON/LEC Profile/Change Password Compare error", "Old Password is not correct!"));
                    this.ModelState.AddModelError(nameof(model.LECPassword), DictionaryPhraseRepository.Current.Get("/CON/LEC Profile/Change Password Compare error", "Password is not correct!"));
                    return this.View(model);
                }
                var isPasswordSame = objservice.ComparePasswordForUpdateLECPassword(UserSession.AEMLCONLECUserSessionContext.RegistrationNumber, model.LECNewPasssword);
                if (isPasswordSame)
                {
                    this.ModelState.AddModelError(nameof(model.LECNewPasssword), DictionaryPhraseRepository.Current.Get("/CON/LEC Profile/Change Password same error", "New Password should be different!"));
                    //this.Session["UpdateMessage"] = new InfoMessage(DictionaryPhraseRepository.Current.Get("/CON/LEC Profile/Change Password same error", "New Password should be different!"));
                    return this.View(model);
                }
                var result = objservice.UpdateLECPassword(UserSession.AEMLCONLECUserSessionContext.RegistrationNumber, model.LECNewPasssword);
                if (result == true)
                    this.Session["UpdateMessage"] = new InfoMessage(DictionaryPhraseRepository.Current.Get("/CON/LEC Profile/Password Update Success", "Password was successfully updated"));
                else
                    this.Session["UpdateMessage"] = new InfoMessage(DictionaryPhraseRepository.Current.Get("/CON/LEC Profile/Password update error", "Password was not successfully updated, please try again!"));
            }
            catch (Exception e)
            {
        ...
        [HttpGet]
        public ActionResult LECProfileDeregister()
        {
            if (Session["LECUserLogin"] == null)
            {
                var item = Context.Database.GetItem(Templates.LECPortal.LECUserLoginPage);
            ...
            ChangeOfNameService conService = new ChangeOfNameService();
            ChangeOfNameLECDeregisterModel model = new ChangeOfNameLECDeregisterModel();
            return View(model);
        }
        ...
        [ValidateRenderingId]
        [ValidateAntiForgeryToken]
        public ActionResult LECProfileDeregister(ChangeOfNameLECDeregisterModel model)
        {
            try
            {
                if (Session["LECUserLogin"] == null)
                ...
                //if (UserSession.AEMLCONLECUserSessionContext.RegistrationNumber != model.LECRegistrationNumber)
                //{
                //    Log.Info("Error at LECProfileDeregister UserSession.AEMLCONLECUserSessionContext.RegistrationNumber != model.LECRegistrationNumber:" + UserSession.AEMLCONLECUserSessionContext.RegistrationNumber + ":" + model.LECRegistrationNumber, this);
                //    this.Session["UpdateMessage"] = new InfoMessage(DictionaryPhraseRepository.Current.Get("/CON/LEC Profile/Password update error", "Profile was not Deactivated!"));
                //    return this.View(model);
                //}
                ...
                var result = objservice.DeregisterLECProfile(UserSession.AEMLCONLECUserSessionContext.RegistrationNumber);
                if (result == true)
                {
                    this.Session["UpdateMessage"] = new InfoMessage(DictionaryPhraseRepository.Current.Get("/CON/LEC Profile/Profile Deactivated", "Profile was successfully Deactivated"));
                    this.Session["LECUserLogin"] = null;
                    this.Session["LECUserLoginOTP"] = null;
                    UserSession.AEMLCONLECUserSessionContext = null;
                    this.Session.Abandon();
                    ...
                    this.Session["UpdateMessage"] = new InfoMessage(DictionaryPhraseRepository.Current.Get("/CON/LEC Profile/Profile Deactivated", "Password was not successfully Deactivated, please try again!"));
            }
            catch (Exception e)
            {
                Sitecore.Diagnostics.Log.Error("Error at " + MethodBase.GetCurrentMethod().Name + ":" + e.Message, this);
                this.Session["UpdateMessage"] = new InfoMessage(DictionaryPhraseRepository.Current.Get("/CON/LEC Profile/Profile Deactivated", "Profile was not successfully Deactivated, please try again!"));
            }
            return this.Redirect(this.Request.RawUrl);
        }
        ...
        [HttpGet]
        public ActionResult ChangeOfNameRegisterationLEC()
        {
            if (Session["LECUserLogin"] == null)
            {
                var item = Context.Database.GetItem(Templates.LECPortal.LECUserLoginPage);
        ...
        [HttpPost]
        [ValidateRenderingId]
        public ActionResult ChangeOfNameRegisterationLEC(ChangeOfNameRegistrationModel registrationInfo, string ValidateAccount = null, string sendOTP = null, string ValidateOTP = null, string submit = null, string CheckApplication = null)
        {
            try
            {
                if (Session["LECUserLogin"] == null)
                ...
                if (!string.IsNullOrEmpty(ValidateAccount))
                {
                    ...
                    {
                        registrationInfo.LECMobileNo = UserSession.AEMLCONLECUserSessionContext.MobileNumber;
                        registrationInfo.LECRegistrationNumber = UserSession.AEMLCONLECUserSessionContext.RegistrationNumber;
                        ...
                        registrationInfo.IsLEC = true;
                        Log.Info("ChangeOfNameRegisteration LEC successful LEc registration for :" + UserSession.AEMLCONLECUserSessionContext.RegistrationNumber, this);
                        Session["CONAccountDetails"] = registrationInfo;
                        return RedirectPermanent(this.UserProfileService.GetPageURL(Templates.LECPortal.ChangeOfNameLECApplicationFormPage));
                    }
                }
        ...
        [HttpGet]
        public ActionResult ChangeOfNameApplicationsStatusLEC()
        {
            if (Session["LECUserLogin"] == null)
            {
                var item = Context.Database.GetItem(Templates.LECPortal.LECUserLoginPage);
            ...
            ChangeOfNameApplicationsStatusModel model = new ChangeOfNameApplicationsStatusModel();
            ChangeOfNameService objservice = new ChangeOfNameService();
            //model.CONApplicationsList = objservice.GetApplicationsByLEC(null, null, null, UserSession.AEMLCONLECUserSessionContext.RegistrationNumber);

            model.StartDate = DateTime.Now.AddDays(-30).ToString("dd/MM/yyyy");
            model.EndDate = DateTime.Now.ToString("dd/MM/yyyy");
            model.CONApplicationsList = objservice.GetApplicationsByLEC(DateTime.Now.AddDays(-30), DateTime.Now, null, UserSession.AEMLCONLECUserSessionContext.RegistrationNumber);
        ...
        [ValidateRenderingId]
        [ValidateAntiForgeryToken]
        public ActionResult ChangeOfNameApplicationsStatusLEC(ChangeOfNameApplicationsStatusModel model, string Status)
        {
            if (Session["LECUserLogin"] == null)
            {
                var item = Context.Database.GetItem(Templates.LECPortal.LECUserLoginPage);
            ...
            try
            {
                ...
                model.CONApplicationsList = objservice.GetApplicationsByLEC(startDate, endDate, Status, UserSession.AEMLCONLECUserSessionContext.RegistrationNumber);
                return this.View(model);
            }
            catch (Exception ex)
            {
                Log.Error("Error at ChangeOfNameApplicationsStatusLEC Post " + ex.Message, this);
            }
            return this.View(model);
        }
        ...
        [HttpGet]
        public ActionResult ChangeOfNameRegisteration()
        {
            ChangeOfNameRegistrationModel model = new ChangeOfNameRegistrationModel();
            model.IsvalidatAccount = false;
            model.IsOTPSent = false;
        ...
        [HttpPost]
        [ValidateRenderingId]
        public ActionResult ChangeOfNameRegisteration(ChangeOfNameRegistrationModel registrationInfo, string ValidateAccount = null, string sendOTP = null, string ValidateOTP = null, string submit = null, string CheckApplication = null)
        {
            try
            {
                Session["Message"] = null;
                ...
                else if (!string.IsNullOrEmpty(sendOTP))
                {
                    ...
                    else
                    {
                        ...
                        catch (AggregateException err)
                        {
                            ...
                            Log.Error("CON OTP Api call failed for registration, catch: " + exceptionMessage, this);
                            this.ModelState.AddModelError(nameof(registrationInfo.MobileNo), DictionaryPhraseRepository.Current.Get("/Accounts/Controller Messages/Mobile numberOTPFailed", "Unable to send OTP."));
                            registrationInfo.IsOTPSent = false;
                            registrationInfo.IsvalidatAccount = true;
                            return this.View(registrationInfo);
        ...
        [HttpPost]
        [ValidateRenderingId]
        public ActionResult ChangeOfNameCheckApplication(string accountNumber, string registrationNumber, string OTPNumber)
        {
            bool isLEC = false;
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(accountNumber))
                {
                    if (!string.IsNullOrEmpty(OTPNumber))
                    {
                        ...
                        else
                        {
                            ...
                            Session["CONAccountDetails"] = registrationInfo;
                            string redirectUrl = this.UserProfileService.GetPageURL(Templates.AccountsSettings.Fields.ChangeOfNameApplicationFormPage);
                            result.Issuccess = true;
                    ...
                    {
                        message = DictionaryPhraseRepository.Current.Get("/CON/Controller Messages/Account Number Exists", "No application for Change of Name is registered for this Account Number");
                        result.Issuccess = false;
                        result.Message = message;
                        return Json(result, JsonRequestBehavior.AllowGet);
                    }
                    else
                    {
                        ...
                        catch (AggregateException err)
                        {
                            ...
                            Log.Error("CON OTP Api call failed for check application, catch: " + exceptionMessage, this);
                            result.IsOTPSent = false;
                ...
                if (!string.IsNullOrEmpty(registrationNumber))
                {
                    ...
                    {
                        message = DictionaryPhraseRepository.Current.Get("/CON/Controller Messages/Account Number Exists", "No application for Change of Name is registered for this Registration Number");
                        result.Issuccess = false;
                        result.Message = message;
                        return Json(result, JsonRequestBehavior.AllowGet);
                    }
                    else
                    {
                        ...
                        {
                            message = DictionaryPhraseRepository.Current.Get("/CON/Controller Messages/Account Number Exists", "No application submitted for Change of Name is registered for this Registration Number");
                            result.Issuccess = false;
                            result.Message = message;
                            return Json(result, JsonRequestBehavior.AllowGet);
                        }
        ...
        [HttpPost]
        [ValidateRenderingId]
        public ActionResult ChangeOfNameCheckApplicationLEC(string accountNumber, string registrationNumber, string OTPNumber)
        {
            bool isLEC = true;
            CheckApplicationModel result = new CheckApplicationModel
            {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(accountNumber))
                {
                    if (!changeOfNameService.IsApplicationExistsForCAByLEC(accountNumber))
                    {
                        message = DictionaryPhraseRepository.Current.Get("/CON/Controller Messages/Account Number Exists", "No application for Change of Name is registered for this Account Number");
                        result.Issuccess = false;
                        result.Message = message;
                    ...
                    {
                        var existingAPP = changeOfNameService.GetExistingApplicationForCAByLEC(accountNumber);

                        ChangeOfNameRegistrationModel registrationInfo = new ChangeOfNameRegistrationModel
                        {
                            AccountNoForCheckApplication = existingAPP.AccountNumber,
                            MobileNo = existingAPP.LECMobileNumber,
                            EmailId = "",
                            IsLEC = true
                        };
                        Session["CONAccountDetails"] = registrationInfo;
                        string redirectUrl = this.UserProfileService.GetPageURL(Templates.LECPortal.ChangeOfNameLECApplicationFormPage);
                        result.Issuccess = true;
                        result.IsVerified = true;
                        result.Message = redirectUrl;
                    }
                ...
                if (!string.IsNullOrEmpty(registrationNumber))
                {
                    if (!changeOfNameService.IsApplicationExistsForTempRegisterByLEC(registrationNumber))
                    {
                        message = DictionaryPhraseRepository.Current.Get("/CON/Controller Messages/Account Number Exists", "No application for Change of Name is registered for this Registration Number");
                        result.Issuccess = false;
                        result.Message = message;
                    ...
                    {
                        var existingApp = changeOfNameService.GetExistingApplicationByRegistrationNumberByLEC(registrationNumber);
                        if (existingApp != null && existingApp.ApplicationStatusCode == "1")
                        {
                            message = DictionaryPhraseRepository.Current.Get("/CON/Controller Messages/Account Number Exists", "No application submitted for Change of Name is registered for this Registration Number");
                            result.Issuccess = false;
                            result.Message = message;
        ...
        #region Export LEC Applications

        public ActionResult LECApplicationsExport(string sdate, string edate, string status)
        {
            if (Session["LECUserLogin"] == null)
            {
                var item = Context.Database.GetItem(Templates.LECPortal.LECUserLoginPage);
            ...
            var statuscheckpageurl = Context.Database.GetItem(Templates.LECPortal.ChangeOfNameLECStatusCheckPage).Url();
            try
            {
                if (Session["LECUserLogin"] == null)
                {
                    var lecLoginPageUrl = Context.Database.GetItem(Templates.LECPortal.LECUserLoginPage).Url();
                    return this.Redirect(lecLoginPageUrl);
                }
                ChangeOfNameService objservice = new ChangeOfNameService();
                DateTime startDate = (DateTime.ParseExact(sdate, "dd/MM/yyyy", CultureInfo.InvariantCulture));
                DateTime endDate = (DateTime.ParseExact(edate, "dd/MM/yyyy", CultureInfo.InvariantCulture));
                var datalist = objservice.GetApplicationsByLEC(startDate, endDate, status, UserSession.AEMLCONLECUserSessionContext.RegistrationNumber);
            ...
            return Redirect(statuscheckpageurl);
        }
        ...
        {
            string fileName = "CON_Applications_" + UserSession.AEMLCONLECUserSessionContext.RegistrationNumber;
        ...
        [HttpGet]
        public ActionResult ChangeOfNameApplicationForm()
        {
            Session["Message"] = null;
            ...
            try
            {
                ...
                {
                    return RedirectPermanent(this.UserProfileService.GetPageURL(Templates.AccountsSettings.Fields.ChangeOfNameRegistrationPage));
                }
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(accountDetailsForCON.AccountNoForCheckApplication))
                    {
                        ...
                        else
                        {
                            ...
                            string redirectUrl = this.UserProfileService.GetPageURL(Templates.AccountsSettings.Fields.ChangeOfNameSubmittedApplicationPage);
        ...
        [HttpPost]
        public ActionResult ChangeOfNameApplicationForm(ChangeOfNameApplicationFromModel model, FormCollection form, string SaveAsDraft = null, string SubmitApplication = null)
        {
            Session["Message"] = null;
        ...
        [HttpGet]
        public ActionResult ChangeOfNameApplicationFormLEC(string accountNumber = null)
        {
            if (Session["LECUserLogin"] == null)
            {
                var item = Context.Database.GetItem(Templates.LECPortal.LECUserLoginPage);
            ...
            try
            {
                ...
                {
                    ChangeOfNameService conservice = new ChangeOfNameService();
                    if (conservice.IsApplicationExistsForCAByLEC(accountNumber))
                    {
                        ChangeOfNameRegistrationModel CONAccountDetails = new ChangeOfNameRegistrationModel
                        {
                            AccountNoForCheckApplication = accountNumber,
                            IsLEC = true,
                        };
                        Session["CONAccountDetails"] = CONAccountDetails;
                    }
                }
                if (Session["CONAccountDetails"] == null)
                {
                    return RedirectPermanent(this.UserProfileService.GetPageURL(Templates.LECPortal.ChangeOfNameLECRegistrationPage));
                }
                else
                {
                    accountDetailsForCON = (ChangeOfNameRegistrationModel)Session["CONAccountDetails"];
                    if (!string.IsNullOrEmpty(accountDetailsForCON.AccountNoForCheckApplication))
                    {
                        ...
                        else
                        {
                            ...
                            //Get the values - submitted application display read only
                            Session["CONAccountDetails"] = accountDetailsForCON;
                            string redirectUrl = this.UserProfileService.GetPageURL(Templates.LECPortal.ChangeOfNameLECSubmittedApplicationPage);
                            return Redirect(redirectUrl);
                        }
                    }
                    else if (!string.IsNullOrEmpty(accountDetailsForCON.RegistrationNo))
        ...
        [HttpPost]
        public ActionResult ChangeOfNameApplicationFormLEC(ChangeOfNameApplicationFromModel model, FormCollection form, string SaveAsDraft = null, string SubmitApplication = null)
        {
            if (Session["LECUserLogin"] == null)
            {
                var item = Context.Database.GetItem(Templates.LECPortal.LECUserLoginPage);
                            ...
                            if (savedApplication != null)
                            {
                                ...
                                try
                                {
                                    ...
                                    foreach (var doc in listOfDocs.Where(d => d.FLAG_MANDATORY == "X" || d.FLAG_DOCNO != "X").ToList())
                                    {
                                        ...
                                        if (file != null && file.ContentLength > 0)
                                        {
                                            ...
                                            obj = CreateDocumentObject(bytes, file.FileName, file.ContentType, doc, model.AccountNo, savedApplication.RegistrationSerialNumber, isDocSaved, form["docnumber_" + doc.DocumentSerialNumber.ToString()]);
                                            dataContext.CONApplicationDocumentDetails.InsertOnSubmit(obj);
                                ...
                                if (!string.IsNullOrEmpty(SubmitApplication))
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        var apiurl = string.Format(DictionaryPhraseRepository.Current.Get("/ChangeOfName/SMS API form submission to LEC number",
                                       "https://hapibk.connectbox.in:8181/SendSMS.aspx?UserName=Web_OTP&password=SM78ysAu&MobileNo={0}&SenderID=ADANIE&CDMAHeader=ADANIE&Message=We confirm the receipt of the Change of Name application. The reference no. for the application is {1}. The request shall be reviewed and processed at the earliest. To check status of this application click here: https://www.adanielectricity.com/changeofnameregistration Adani Electricity&dlt_peid=1001756674011432608&dlt_tmid=120210000043&dlt_templateid=1707160766557724834"), model.LECMobileNumber, savedApplication.TempRegistrationSerialNumber);
        ...
        [HttpGet]
        public ActionResult ChangeOfNameSubmittedApplicationLEC()
        {
            if (Session["LECUserLogin"] == null)
            {
                var item = Context.Database.GetItem(Templates.LECPortal.LECUserLoginPage);
        ...
        [HttpGet]
        public ActionResult ChangeOfNameSubmittedApplication()
        {
            Session["Message"] = null;
            ChangeOfNameApplicationFromModel model = new ChangeOfNameApplicationFromModel();
            ChangeOfNameService changeOfNameService = new ChangeOfNameService();
        ...
        [HttpGet]
        public ActionResult ChangeOfNameViewApplicationLEC(string registrationNumber)
        {
            if (Session["LECUserLogin"] == null)
            {
                var item = Context.Database.GetItem(Templates.LECPortal.LECUserLoginPage);
            ...
            ChangeOfNameService changeOfNameService = new ChangeOfNameService();
            if (!changeOfNameService.IsApplicationExistsForRegistrationNumberByLEC(registrationNumber))
            {
                return RedirectPermanent(this.UserProfileService.GetPageURL(Templates.LECPortal.ChangeOfNameLECStatusCheckPage));
            }
            Session["Message"] = null;
            ChangeOfNameApplicationFromModel model = new ChangeOfNameApplicationFromModel();
            ...
            try
            {
                var CONApp = changeOfNameService.GetExistingApplicationByRegistrationNumberByLEC(registrationNumber);
                var CONAppDocuments = changeOfNameService.GetExistingDocument(CONApp.RegistrationSerialNumber);
