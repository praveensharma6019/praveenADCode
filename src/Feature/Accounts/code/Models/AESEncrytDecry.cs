...
namespace ClientsideEncryption
{
    public class AESEncrytDecry
    {
        ...
        public static string DecryptStringAES(string encryptedValue)
        {
            ...
            var encrypted = System.Convert.FromBase64String(encryptedValue);
            var decryptedFromJavascript = DecryptStringFromBytes(encrypted, keybytes, iv);

            return decryptedFromJavascript;
        }

        public static string GetEncriptedString(string stringToEncrypt)
        {
            var keybytes = Encoding.UTF8.GetBytes("8080808080808080");
            var iv = Encoding.UTF8.GetBytes("8080808080808080");

            var encryptedByte = EncryptStringToBytes(stringToEncrypt, keybytes, iv);

            var encrypted = System.Convert.ToBase64String(encryptedByte);
            return encrypted;
        }
