        ...
        private List<object> GetServiceListData(Rendering rendering, string queryString)
        {
            ...
            try
            {
                ...
                foreach (var child in childList)
                {
                    ...
                    serviceItem.isAirportSelectNeeded = _helper.GetCheckboxOption(child.Fields[Templates.ServicesListCollection.IsAirportSelectNeeded]);

                    switch (queryString)
                    {
                        case "app":
                            serviceItem.imageSrc = _helper.GetImageURL(child.Fields["MobileImage"]);
                            if (!String.IsNullOrEmpty(serviceItem.ctaUrl))
                            {
                                if (serviceItem.ctaUrl.Contains("?"))
                                {
                                    serviceItem.ctaUrl += "&isapp=true";
                                }
                                else
                                {
                                    serviceItem.ctaUrl += "?isapp=true";
