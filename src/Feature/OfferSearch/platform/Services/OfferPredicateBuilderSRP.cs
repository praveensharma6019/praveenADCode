...
{
    public static class OfferPredicateBuilderSRP
    {
        internal static Expression<Func<SearchResultItem, bool>> GetOfferPredicate(OfferFilters_old filter)
        {
            ...
            {
                predicate = predicate.And(x => x[offerFilterContent.OfferIsInternationalSolrField] == Convert.ToString(filter.isInternational).ToLower());
            }
            if(!filter.isInternational)
            {
                predicate = predicate.And(x => x[offerFilterContent.OfferIsInternationalSolrField] == Convert.ToString(filter.isInternational).ToLower());
