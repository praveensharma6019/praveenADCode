...
using static Adani.SuperApp.Realty.Foundation.SitecoreHelper.Platform.Helper.Templates;
...
namespace Adani.SuperApp.Realty.Foundation.SitecoreHelper.Platform.Helper
{
    public static class Helper
    {
        ...
        //this method is used for GSA only
        public static string GetRelativeImageURLbyField(Field field)
        {
            ...
            if (field != null)
            {
                ...
                if (imgField?.MediaItem != null)
                {
                    imageURL = Sitecore.Configuration.Settings.GetSetting("GSAdomainUrl").ToString() + Sitecore.StringUtil.EnsurePrefix('/', MediaManager.GetMediaUrl(imgField.MediaItem));
                }
            }
            if (imageURL == GetSitecoreDomain())
            {
        ...
        public static Item GetSolrImageDetails(SearchResultItem item, string fieldName, Item imageDetails = null)
        {
            Item simpleItem = item.GetItem();
            if (!String.IsNullOrEmpty(item.Fields[fieldName].ToString()))
            {
                ImageField imgField = (ImageField)simpleItem.Fields["{E4FBF9B5-927B-476C-A0B7-78308243F6D0}"];
                if (imgField.MediaItem != null)
                {
                    imageDetails = MediaManager.GetMedia(imgField.MediaItem).MediaData.MediaItem;
                }
