namespace Sitecore.Foundation.Assets.Pipelines.GetPageRendering
{
    ...
    public class AddRenderingAssets : GetPageRenderingProcessor
    {
        ...
        protected static void AddAssetsFromItem(Item renderingItem)
        {
            ...
            AddHeadScriptAssetsFromRendering(renderingItem);
            AddHeadInlineScriptFromRendering(renderingItem);
        ...
        private static void AddHeadInlineScriptFromRendering(Item renderingItem)
        {
            var javaScriptInline = renderingItem[Templates.RenderingAssets.Fields.HeadInlineScript];
            if (!string.IsNullOrEmpty(javaScriptInline))
            {
                var asset = AssetRepository.Current.AddHeadInlineScript(javaScriptInline, ScriptLocation.Head, true);
                asset.AddOnceToken = renderingItem.ID.ToString();
            }
        }
        ...
        private static void AddHeadScriptAssetsFromRendering(Item renderingItem)
        {
            var javaScriptAssets = renderingItem[Templates.RenderingAssets.Fields.HeadScriptFiles];
