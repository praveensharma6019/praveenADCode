...
{
    public class DutyfreeCategoryTitle : IComputedIndexField
    {
        ...
        public object ComputeFieldValue(IIndexable indexable)
        {
            ...
            try
            {
                ...
                GroupedDroplistField droplistField = item.Fields[Templates.DFCollection.Category_Name];
                ...
                if (categoryFolder.HasChildren)
                {
                    ...
                    {
                        Item categoryItem = categoryItems.FirstOrDefault(p => p.TemplateID == Templates.DFCollection.CategoryTemplateID && p.Name.ToLower().Equals(droplistField.Value.ToLower()));
                        if (categoryItem != null)
                        {
                            categoryTitle = categoryItem.Fields[Templates.DFCollection.Name].ToString();
                        }
                    }
                }
            }
            ...
            {
                Log.Warn(string.Format("{0} : unsupported Template type in categoryTitle  : {1} -- Error -- {2}", this, indexable.GetType(), ex), this);
            }
